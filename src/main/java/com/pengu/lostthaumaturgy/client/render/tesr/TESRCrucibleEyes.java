...
public class TESRCrucibleEyes extends TESR<TileCrucibleEyes>
{
	...
	public void renderTileEntityAt(TileCrucibleEyes te, double x, double y, double z, float partialTicks, ResourceLocation destroyStage, float alpha)
	{
		...
		try
		{
			...
			TextureAtlasSprite side_disconnected = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_side_disconnected_" + sub);
			TextureAtlasSprite bottom = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_bottom");
			TextureAtlasSprite top = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_top");
			...
			TextureAtlasSprite base = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_inner");
			TextureAtlasSprite side_connected = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_side_connected_" + sub);
	...
	public void renderFromNBT(NBTTagCompound nbt, double x, double y, double z, float partialTicks, ResourceLocation destroyStage)
	{
		...
		TextureAtlasSprite side_disconnected = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_side_disconnected_" + ((pureVis + taintedVis) >= maxVis * .9F ? "on" : "off"));
		TextureAtlasSprite bottom = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_bottom");
		TextureAtlasSprite top = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucibles/eyes/crucible_top");
		TextureAtlasSprite spill = ClientProxy.getSprite(Info.MOD_ID + ":blocks/crucible_spill");
		TextureAtlasSprite vis = ClientProxy.getSprite(Info.MOD_ID + ":blocks/fluid_vis");
