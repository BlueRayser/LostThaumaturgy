package com.pengu.lostthaumaturgy.util;

import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.TimeUnit;

import com.google.common.base.Stopwatch;
import com.pengu.lostthaumaturgy.LostThaumaturgy;

public class SymbolsLT
{
	private static transient final char[] chars;
	
	public static Character getSymbolForNum(Integer curr)
	{
		return new Character(chars[curr % chars.length]);
	}
	
	public static String convert(String chars)
	{
		String m = "";
		for(int i = 0; i < chars.length(); ++i)
			if(chars.charAt(i) == ' ')
				m += " ";
			else
				m += "" + getSymbolForNum(Math.abs(i * chars.length() + (chars.hashCode() * chars.hashCode())));
		return m;
	}
	
	static
	{
		LostThaumaturgy.LOG.info("[SYMBOLS] Loading symbols...");
		Stopwatch t = Stopwatch.createStarted();
		
		List<Character> charS = new ArrayList<Character>();
		
		charS.addAll(addCharEntryA());
		charS.addAll(addCharEntryB());
		charS.addAll(addCharEntryC());
		charS.addAll(addCharEntryD());
		
		chars = new char[charS.size()];
		for(int ind = 0; ind < chars.length; ++ind)
			chars[ind] = charS.get(ind);
		t.stop();
		
		LostThaumaturgy.LOG.info("[SYMBOLS] Loaded " + charS.size() + " symbols in " + t.elapsed(TimeUnit.MILLISECONDS) + " ms");
	}
	
	public static List<Character> addCharEntryA()
	{
		ArrayList<Character> charS = new ArrayList<Character>();
		
		charS.add('\uAC00');
		charS.add('\uAC01');
		charS.add('\uAC02');
		charS.add('\uAC03');
		charS.add('\uAC04');
		charS.add('\uAC05');
		charS.add('\uAC06');
		charS.add('\uAC07');
		charS.add('\uAC08');
		charS.add('\uAC09');
		charS.add('\uAC0A');
		charS.add('\uAC0B');
		charS.add('\uAC0C');
		charS.add('\uAC0D');
		charS.add('\uAC0E');
		charS.add('\uAC0F');
		charS.add('\uAC10');
		charS.add('\uAC11');
		charS.add('\uAC12');
		charS.add('\uAC13');
		charS.add('\uAC14');
		charS.add('\uAC15');
		charS.add('\uAC16');
		charS.add('\uAC17');
		charS.add('\uAC18');
		charS.add('\uAC19');
		charS.add('\uAC1A');
		charS.add('\uAC1B');
		charS.add('\uAC1C');
		charS.add('\uAC1D');
		charS.add('\uAC1E');
		charS.add('\uAC1F');
		charS.add('\uAC20');
		charS.add('\uAC21');
		charS.add('\uAC22');
		charS.add('\uAC23');
		charS.add('\uAC24');
		charS.add('\uAC25');
		charS.add('\uAC26');
		charS.add('\uAC27');
		charS.add('\uAC28');
		charS.add('\uAC29');
		charS.add('\uAC2A');
		charS.add('\uAC2B');
		charS.add('\uAC2C');
		charS.add('\uAC2D');
		charS.add('\uAC2E');
		charS.add('\uAC2F');
		charS.add('\uAC30');
		charS.add('\uAC31');
		charS.add('\uAC32');
		charS.add('\uAC33');
		charS.add('\uAC34');
		charS.add('\uAC35');
		charS.add('\uAC36');
		charS.add('\uAC37');
		charS.add('\uAC38');
		charS.add('\uAC39');
		charS.add('\uAC3A');
		charS.add('\uAC3B');
		charS.add('\uAC3C');
		charS.add('\uAC3D');
		charS.add('\uAC3E');
		charS.add('\uAC3F');
		charS.add('\uAC40');
		charS.add('\uAC41');
		charS.add('\uAC42');
		charS.add('\uAC43');
		charS.add('\uAC44');
		charS.add('\uAC45');
		charS.add('\uAC46');
		charS.add('\uAC47');
		charS.add('\uAC48');
		charS.add('\uAC49');
		charS.add('\uAC4A');
		charS.add('\uAC4B');
		charS.add('\uAC4C');
		charS.add('\uAC4D');
		charS.add('\uAC4E');
		charS.add('\uAC4F');
		charS.add('\uAC50');
		charS.add('\uAC51');
		charS.add('\uAC52');
		charS.add('\uAC53');
		charS.add('\uAC54');
		charS.add('\uAC55');
		charS.add('\uAC56');
		charS.add('\uAC57');
		charS.add('\uAC58');
		charS.add('\uAC59');
		charS.add('\uAC5A');
		charS.add('\uAC5B');
		charS.add('\uAC5C');
		charS.add('\uAC5D');
		charS.add('\uAC5E');
		charS.add('\uAC5F');
		charS.add('\uAC60');
		charS.add('\uAC61');
		charS.add('\uAC62');
		charS.add('\uAC63');
		charS.add('\uAC64');
		charS.add('\uAC65');
		charS.add('\uAC66');
		charS.add('\uAC67');
		charS.add('\uAC68');
		charS.add('\uAC69');
		charS.add('\uAC6A');
		charS.add('\uAC6B');
		charS.add('\uAC6C');
		charS.add('\uAC6D');
		charS.add('\uAC6E');
		charS.add('\uAC6F');
		charS.add('\uAC70');
		charS.add('\uAC71');
		charS.add('\uAC72');
		charS.add('\uAC73');
		charS.add('\uAC74');
		charS.add('\uAC75');
		charS.add('\uAC76');
		charS.add('\uAC77');
		charS.add('\uAC78');
		charS.add('\uAC79');
		charS.add('\uAC7A');
		charS.add('\uAC7B');
		charS.add('\uAC7C');
		charS.add('\uAC7D');
		charS.add('\uAC7E');
		charS.add('\uAC7F');
		charS.add('\uAC80');
		charS.add('\uAC81');
		charS.add('\uAC82');
		charS.add('\uAC83');
		charS.add('\uAC84');
		charS.add('\uAC85');
		charS.add('\uAC86');
		charS.add('\uAC87');
		charS.add('\uAC88');
		charS.add('\uAC89');
		charS.add('\uAC8A');
		charS.add('\uAC8B');
		charS.add('\uAC8C');
		charS.add('\uAC8D');
		charS.add('\uAC8E');
		charS.add('\uAC8F');
		charS.add('\uAC90');
		charS.add('\uAC91');
		charS.add('\uAC92');
		charS.add('\uAC93');
		charS.add('\uAC94');
		charS.add('\uAC95');
		charS.add('\uAC96');
		charS.add('\uAC97');
		charS.add('\uAC98');
		charS.add('\uAC99');
		charS.add('\uAC9A');
		charS.add('\uAC9B');
		charS.add('\uAC9C');
		charS.add('\uAC9D');
		charS.add('\uAC9E');
		charS.add('\uAC9F');
		charS.add('\uACA0');
		charS.add('\uACA1');
		charS.add('\uACA2');
		charS.add('\uACA3');
		charS.add('\uACA4');
		charS.add('\uACA5');
		charS.add('\uACA6');
		charS.add('\uACA7');
		charS.add('\uACA8');
		charS.add('\uACA9');
		charS.add('\uACAA');
		charS.add('\uACAB');
		charS.add('\uACAC');
		charS.add('\uACAD');
		charS.add('\uACAE');
		charS.add('\uACAF');
		charS.add('\uACB0');
		charS.add('\uACB1');
		charS.add('\uACB2');
		charS.add('\uACB3');
		charS.add('\uACB4');
		charS.add('\uACB5');
		charS.add('\uACB6');
		charS.add('\uACB7');
		charS.add('\uACB8');
		charS.add('\uACB9');
		charS.add('\uACBA');
		charS.add('\uACBB');
		charS.add('\uACBC');
		charS.add('\uAC3D');
		charS.add('\uACBE');
		charS.add('\uACBF');
		charS.add('\uACC0');
		charS.add('\uACC1');
		charS.add('\uACC2');
		charS.add('\uACC3');
		charS.add('\uACC4');
		charS.add('\uACC5');
		charS.add('\uACC6');
		charS.add('\uACC7');
		charS.add('\uACC8');
		charS.add('\uACC9');
		charS.add('\uACCA');
		charS.add('\uACCB');
		charS.add('\uACCC');
		charS.add('\uACCD');
		charS.add('\uACCE');
		charS.add('\uACCF');
		charS.add('\uACD0');
		charS.add('\uACD1');
		charS.add('\uACD2');
		charS.add('\uACD3');
		charS.add('\uACD4');
		charS.add('\uACD5');
		charS.add('\uACD6');
		charS.add('\uACD7');
		charS.add('\uACD8');
		charS.add('\uACD9');
		charS.add('\uACDA');
		charS.add('\uACDB');
		charS.add('\uACDC');
		charS.add('\uACDD');
		charS.add('\uACDE');
		charS.add('\uACDF');
		charS.add('\uACE0');
		charS.add('\uACE1');
		charS.add('\uACE2');
		charS.add('\uACE3');
		charS.add('\uACE4');
		charS.add('\uACE5');
		charS.add('\uACE6');
		charS.add('\uACE7');
		charS.add('\uACE8');
		charS.add('\uACE9');
		charS.add('\uACEA');
		charS.add('\uACEB');
		charS.add('\uACEC');
		charS.add('\uACED');
		charS.add('\uACEE');
		charS.add('\uACEF');
		charS.add('\uACF0');
		charS.add('\uACF1');
		charS.add('\uACF2');
		charS.add('\uACF3');
		charS.add('\uACF4');
		charS.add('\uACF5');
		charS.add('\uACF6');
		charS.add('\uACF7');
		charS.add('\uACF8');
		charS.add('\uACF9');
		charS.add('\uACFA');
		charS.add('\uACFB');
		charS.add('\uACFC');
		charS.add('\uACFD');
		charS.add('\uACFE');
		charS.add('\uACFF');
		
		return charS;
	}
	
	public static List<Character> addCharEntryB()
	{
		ArrayList<Character> charS = new ArrayList<Character>();
		
		charS.add('\uB000');
		charS.add('\uB001');
		charS.add('\uB002');
		charS.add('\uB003');
		charS.add('\uB004');
		charS.add('\uB005');
		charS.add('\uB006');
		charS.add('\uB007');
		charS.add('\uB008');
		charS.add('\uB009');
		charS.add('\uB00A');
		charS.add('\uB00B');
		charS.add('\uB00C');
		charS.add('\uB00D');
		charS.add('\uB00E');
		charS.add('\uB00F');
		charS.add('\uB010');
		charS.add('\uB011');
		charS.add('\uB012');
		charS.add('\uB013');
		charS.add('\uB014');
		charS.add('\uB015');
		charS.add('\uB016');
		charS.add('\uB017');
		charS.add('\uB018');
		charS.add('\uB019');
		charS.add('\uB01A');
		charS.add('\uB01B');
		charS.add('\uB01C');
		charS.add('\uB01D');
		charS.add('\uB01E');
		charS.add('\uB01F');
		charS.add('\uB020');
		charS.add('\uB021');
		charS.add('\uB022');
		charS.add('\uB023');
		charS.add('\uB024');
		charS.add('\uB025');
		charS.add('\uB026');
		charS.add('\uB027');
		charS.add('\uB028');
		charS.add('\uB029');
		charS.add('\uB02A');
		charS.add('\uB02B');
		charS.add('\uB02C');
		charS.add('\uB02D');
		charS.add('\uB02E');
		charS.add('\uB02F');
		charS.add('\uB030');
		charS.add('\uB031');
		charS.add('\uB032');
		charS.add('\uB033');
		charS.add('\uB034');
		charS.add('\uB035');
		charS.add('\uB036');
		charS.add('\uB037');
		charS.add('\uB038');
		charS.add('\uB039');
		charS.add('\uB03A');
		charS.add('\uB03B');
		charS.add('\uB03C');
		charS.add('\uB03D');
		charS.add('\uB03E');
		charS.add('\uB03F');
		charS.add('\uB040');
		charS.add('\uB041');
		charS.add('\uB042');
		charS.add('\uB043');
		charS.add('\uB044');
		charS.add('\uB045');
		charS.add('\uB046');
		charS.add('\uB047');
		charS.add('\uB048');
		charS.add('\uB049');
		charS.add('\uB04A');
		charS.add('\uB04B');
		charS.add('\uB04C');
		charS.add('\uB04D');
		charS.add('\uB04E');
		charS.add('\uB04F');
		charS.add('\uB050');
		charS.add('\uB051');
		charS.add('\uB052');
		charS.add('\uB053');
		charS.add('\uB054');
		charS.add('\uB055');
		charS.add('\uB056');
		charS.add('\uB057');
		charS.add('\uB058');
		charS.add('\uB059');
		charS.add('\uB05A');
		charS.add('\uB05B');
		charS.add('\uB05C');
		charS.add('\uB05D');
		charS.add('\uB05E');
		charS.add('\uB05F');
		charS.add('\uB060');
		charS.add('\uB061');
		charS.add('\uB062');
		charS.add('\uB063');
		charS.add('\uB064');
		charS.add('\uB065');
		charS.add('\uB066');
		charS.add('\uB067');
		charS.add('\uB068');
		charS.add('\uB069');
		charS.add('\uB06A');
		charS.add('\uB06B');
		charS.add('\uB06C');
		charS.add('\uB06D');
		charS.add('\uB06E');
		charS.add('\uB06F');
		charS.add('\uB070');
		charS.add('\uB071');
		charS.add('\uB072');
		charS.add('\uB073');
		charS.add('\uB074');
		charS.add('\uB075');
		charS.add('\uB076');
		charS.add('\uB077');
		charS.add('\uB078');
		charS.add('\uB079');
		charS.add('\uB07A');
		charS.add('\uB07B');
		charS.add('\uB07C');
		charS.add('\uB07D');
		charS.add('\uB07E');
		charS.add('\uB07F');
		charS.add('\uB080');
		charS.add('\uB081');
		charS.add('\uB082');
		charS.add('\uB083');
		charS.add('\uB084');
		charS.add('\uB085');
		charS.add('\uB086');
		charS.add('\uB087');
		charS.add('\uB088');
		charS.add('\uB089');
		charS.add('\uB08A');
		charS.add('\uB08B');
		charS.add('\uB08C');
		charS.add('\uB08D');
		charS.add('\uB08E');
		charS.add('\uB08F');
		charS.add('\uB090');
		charS.add('\uB091');
		charS.add('\uB092');
		charS.add('\uB093');
		charS.add('\uB094');
		charS.add('\uB095');
		charS.add('\uB096');
		charS.add('\uB097');
		charS.add('\uB098');
		charS.add('\uB099');
		charS.add('\uB09A');
		charS.add('\uB09B');
		charS.add('\uB09C');
		charS.add('\uB09D');
		charS.add('\uB09E');
		charS.add('\uB09F');
		charS.add('\uB0A0');
		charS.add('\uB0A1');
		charS.add('\uB0A2');
		charS.add('\uB0A3');
		charS.add('\uB0A4');
		charS.add('\uB0A5');
		charS.add('\uB0A6');
		charS.add('\uB0A7');
		charS.add('\uB0A8');
		charS.add('\uB0A9');
		charS.add('\uB0AA');
		charS.add('\uB0AB');
		charS.add('\uB0AC');
		charS.add('\uB0AD');
		charS.add('\uB0AE');
		charS.add('\uB0AF');
		charS.add('\uB0B0');
		charS.add('\uB0B1');
		charS.add('\uB0B2');
		charS.add('\uB0B3');
		charS.add('\uB0B4');
		charS.add('\uB0B5');
		charS.add('\uB0B6');
		charS.add('\uB0B7');
		charS.add('\uB0B8');
		charS.add('\uB0B9');
		charS.add('\uB0BA');
		charS.add('\uB0BB');
		charS.add('\uB0BC');
		charS.add('\uB03D');
		charS.add('\uB0BE');
		charS.add('\uB0BF');
		charS.add('\uB0C0');
		charS.add('\uB0C1');
		charS.add('\uB0C2');
		charS.add('\uB0C3');
		charS.add('\uB0C4');
		charS.add('\uB0C5');
		charS.add('\uB0C6');
		charS.add('\uB0C7');
		charS.add('\uB0C8');
		charS.add('\uB0C9');
		charS.add('\uB0CA');
		charS.add('\uB0CB');
		charS.add('\uB0CC');
		charS.add('\uB0CD');
		charS.add('\uB0CE');
		charS.add('\uB0CF');
		charS.add('\uB0D0');
		charS.add('\uB0D1');
		charS.add('\uB0D2');
		charS.add('\uB0D3');
		charS.add('\uB0D4');
		charS.add('\uB0D5');
		charS.add('\uB0D6');
		charS.add('\uB0D7');
		charS.add('\uB0D8');
		charS.add('\uB0D9');
		charS.add('\uB0DA');
		charS.add('\uB0DB');
		charS.add('\uB0DC');
		charS.add('\uB0DD');
		charS.add('\uB0DE');
		charS.add('\uB0DF');
		charS.add('\uB0E0');
		charS.add('\uB0E1');
		charS.add('\uB0E2');
		charS.add('\uB0E3');
		charS.add('\uB0E4');
		charS.add('\uB0E5');
		charS.add('\uB0E6');
		charS.add('\uB0E7');
		charS.add('\uB0E8');
		charS.add('\uB0E9');
		charS.add('\uB0EA');
		charS.add('\uB0EB');
		charS.add('\uB0EC');
		charS.add('\uB0ED');
		charS.add('\uB0EE');
		charS.add('\uB0EF');
		charS.add('\uB0F0');
		charS.add('\uB0F1');
		charS.add('\uB0F2');
		charS.add('\uB0F3');
		charS.add('\uB0F4');
		charS.add('\uB0F5');
		charS.add('\uB0F6');
		charS.add('\uB0F7');
		charS.add('\uB0F8');
		charS.add('\uB0F9');
		charS.add('\uB0FA');
		charS.add('\uB0FB');
		charS.add('\uB0FC');
		charS.add('\uB0FD');
		charS.add('\uB0FE');
		charS.add('\uB0FF');
		charS.add('\uB0E8');
		charS.add('\uB0E9');
		charS.add('\uB0EA');
		charS.add('\uB0EB');
		charS.add('\uB0EC');
		charS.add('\uB0ED');
		charS.add('\uB0EE');
		charS.add('\uB0EF');
		charS.add('\uB0F0');
		charS.add('\uB0F1');
		charS.add('\uB0F2');
		charS.add('\uB0F3');
		charS.add('\uB0F4');
		charS.add('\uB0F5');
		charS.add('\uB0F6');
		charS.add('\uB0F7');
		charS.add('\uB0F8');
		charS.add('\uB0F9');
		charS.add('\uB0FA');
		charS.add('\uB0FB');
		charS.add('\uB0FC');
		charS.add('\uB0FD');
		charS.add('\uB0FE');
		charS.add('\uB0FF');
		charS.add('\uB100');
		charS.add('\uB101');
		charS.add('\uB102');
		charS.add('\uB103');
		charS.add('\uB104');
		charS.add('\uB105');
		charS.add('\uB106');
		charS.add('\uB107');
		charS.add('\uB108');
		charS.add('\uB109');
		charS.add('\uB10A');
		charS.add('\uB10B');
		charS.add('\uB10C');
		charS.add('\uB10D');
		charS.add('\uB10E');
		charS.add('\uB10F');
		charS.add('\uB110');
		charS.add('\uB111');
		charS.add('\uB112');
		charS.add('\uB113');
		charS.add('\uB114');
		charS.add('\uB115');
		charS.add('\uB116');
		charS.add('\uB117');
		charS.add('\uB118');
		charS.add('\uB119');
		charS.add('\uB11A');
		charS.add('\uB11B');
		charS.add('\uB11C');
		charS.add('\uB11D');
		charS.add('\uB11E');
		charS.add('\uB11F');
		charS.add('\uB120');
		charS.add('\uB121');
		charS.add('\uB122');
		charS.add('\uB123');
		charS.add('\uB124');
		charS.add('\uB125');
		charS.add('\uB126');
		charS.add('\uB127');
		charS.add('\uB128');
		charS.add('\uB129');
		charS.add('\uB12A');
		charS.add('\uB12B');
		charS.add('\uB12C');
		charS.add('\uB12D');
		charS.add('\uB12E');
		charS.add('\uB12F');
		charS.add('\uB130');
		charS.add('\uB131');
		charS.add('\uB132');
		charS.add('\uB133');
		charS.add('\uB134');
		charS.add('\uB135');
		charS.add('\uB136');
		charS.add('\uB137');
		charS.add('\uB138');
		charS.add('\uB139');
		charS.add('\uB13A');
		charS.add('\uB13B');
		charS.add('\uB13C');
		charS.add('\uB13D');
		charS.add('\uB13E');
		charS.add('\uB13F');
		charS.add('\uB140');
		charS.add('\uB141');
		charS.add('\uB142');
		charS.add('\uB143');
		charS.add('\uB144');
		charS.add('\uB145');
		charS.add('\uB146');
		charS.add('\uB147');
		charS.add('\uB148');
		charS.add('\uB149');
		charS.add('\uB14A');
		charS.add('\uB14B');
		charS.add('\uB14C');
		charS.add('\uB14D');
		charS.add('\uB14E');
		charS.add('\uB14F');
		charS.add('\uB150');
		charS.add('\uB151');
		charS.add('\uB152');
		charS.add('\uB153');
		charS.add('\uB154');
		charS.add('\uB155');
		charS.add('\uB156');
		charS.add('\uB157');
		charS.add('\uB158');
		charS.add('\uB159');
		charS.add('\uB15A');
		charS.add('\uB15B');
		charS.add('\uB15C');
		charS.add('\uB15D');
		charS.add('\uB15E');
		charS.add('\uB15F');
		charS.add('\uB160');
		charS.add('\uB161');
		charS.add('\uB162');
		charS.add('\uB163');
		charS.add('\uB164');
		charS.add('\uB165');
		charS.add('\uB166');
		charS.add('\uB167');
		charS.add('\uB168');
		charS.add('\uB169');
		charS.add('\uB16A');
		charS.add('\uB16B');
		charS.add('\uB16C');
		charS.add('\uB16D');
		charS.add('\uB16E');
		charS.add('\uB16F');
		charS.add('\uB170');
		charS.add('\uB171');
		charS.add('\uB172');
		charS.add('\uB173');
		charS.add('\uB174');
		charS.add('\uB175');
		charS.add('\uB176');
		charS.add('\uB177');
		charS.add('\uB178');
		charS.add('\uB179');
		charS.add('\uB17A');
		charS.add('\uB17B');
		charS.add('\uB17C');
		charS.add('\uB17D');
		charS.add('\uB17E');
		charS.add('\uB17F');
		charS.add('\uB180');
		charS.add('\uB181');
		charS.add('\uB182');
		charS.add('\uB183');
		charS.add('\uB184');
		charS.add('\uB185');
		charS.add('\uB186');
		charS.add('\uB187');
		charS.add('\uB188');
		charS.add('\uB189');
		charS.add('\uB18A');
		charS.add('\uB18B');
		charS.add('\uB18C');
		charS.add('\uB18D');
		charS.add('\uB18E');
		charS.add('\uB18F');
		charS.add('\uB190');
		charS.add('\uB191');
		charS.add('\uB192');
		charS.add('\uB193');
		charS.add('\uB194');
		charS.add('\uB195');
		charS.add('\uB196');
		charS.add('\uB197');
		charS.add('\uB198');
		charS.add('\uB199');
		charS.add('\uB19A');
		charS.add('\uB19B');
		charS.add('\uB19C');
		charS.add('\uB19D');
		charS.add('\uB19E');
		charS.add('\uB19F');
		charS.add('\uB1A0');
		charS.add('\uB1A1');
		charS.add('\uB1A2');
		charS.add('\uB1A3');
		charS.add('\uB1A4');
		charS.add('\uB1A5');
		charS.add('\uB1A6');
		charS.add('\uB1A7');
		charS.add('\uB1A8');
		charS.add('\uB1A9');
		charS.add('\uB1AA');
		charS.add('\uB1AB');
		charS.add('\uB1AC');
		charS.add('\uB1AD');
		charS.add('\uB1AE');
		charS.add('\uB1AF');
		charS.add('\uB1B0');
		charS.add('\uB1B1');
		charS.add('\uB1B2');
		charS.add('\uB1B3');
		charS.add('\uB1B4');
		charS.add('\uB1B5');
		charS.add('\uB1B6');
		charS.add('\uB1B7');
		charS.add('\uB1B8');
		charS.add('\uB1B9');
		charS.add('\uB1BA');
		charS.add('\uB1BB');
		charS.add('\uB1BC');
		charS.add('\uB13D');
		charS.add('\uB1BE');
		charS.add('\uB1BF');
		charS.add('\uB1C0');
		charS.add('\uB1C1');
		charS.add('\uB1C2');
		charS.add('\uB1C3');
		charS.add('\uB1C4');
		charS.add('\uB1C5');
		charS.add('\uB1C6');
		charS.add('\uB1C7');
		charS.add('\uB1C8');
		charS.add('\uB1C9');
		charS.add('\uB1CA');
		charS.add('\uB1CB');
		charS.add('\uB1CC');
		charS.add('\uB1CD');
		charS.add('\uB1CE');
		charS.add('\uB1CF');
		charS.add('\uB1D0');
		charS.add('\uB1D1');
		charS.add('\uB1D2');
		charS.add('\uB1D3');
		charS.add('\uB1D4');
		charS.add('\uB1D5');
		charS.add('\uB1D6');
		charS.add('\uB1D7');
		charS.add('\uB1D8');
		charS.add('\uB1D9');
		charS.add('\uB1DA');
		charS.add('\uB1DB');
		charS.add('\uB1DC');
		charS.add('\uB1DD');
		charS.add('\uB1DE');
		charS.add('\uB1DF');
		charS.add('\uB1E0');
		charS.add('\uB1E1');
		charS.add('\uB1E2');
		charS.add('\uB1E3');
		charS.add('\uB1E4');
		charS.add('\uB1E5');
		charS.add('\uB1E6');
		charS.add('\uB1E7');
		charS.add('\uB1E8');
		charS.add('\uB1E9');
		charS.add('\uB1EA');
		charS.add('\uB1EB');
		charS.add('\uB1EC');
		charS.add('\uB1ED');
		charS.add('\uB1EE');
		charS.add('\uB1EF');
		charS.add('\uB1F0');
		charS.add('\uB1F1');
		charS.add('\uB1F2');
		charS.add('\uB1F3');
		charS.add('\uB1F4');
		charS.add('\uB1F5');
		charS.add('\uB1F6');
		charS.add('\uB1F7');
		charS.add('\uB1F8');
		charS.add('\uB1F9');
		charS.add('\uB1FA');
		charS.add('\uB1FB');
		charS.add('\uB1FC');
		charS.add('\uB1FD');
		charS.add('\uB1FE');
		charS.add('\uB1FF');
		charS.add('\uB200');
		charS.add('\uB201');
		charS.add('\uB202');
		charS.add('\uB203');
		charS.add('\uB204');
		charS.add('\uB205');
		charS.add('\uB206');
		charS.add('\uB207');
		charS.add('\uB208');
		charS.add('\uB209');
		charS.add('\uB20A');
		charS.add('\uB20B');
		charS.add('\uB20C');
		charS.add('\uB20D');
		charS.add('\uB20E');
		charS.add('\uB20F');
		charS.add('\uB210');
		charS.add('\uB211');
		charS.add('\uB212');
		charS.add('\uB213');
		charS.add('\uB214');
		charS.add('\uB215');
		charS.add('\uB216');
		charS.add('\uB217');
		charS.add('\uB218');
		charS.add('\uB219');
		charS.add('\uB21A');
		charS.add('\uB21B');
		charS.add('\uB21C');
		charS.add('\uB21D');
		charS.add('\uB21E');
		charS.add('\uB21F');
		charS.add('\uB220');
		charS.add('\uB221');
		charS.add('\uB222');
		charS.add('\uB223');
		charS.add('\uB224');
		charS.add('\uB225');
		charS.add('\uB226');
		charS.add('\uB227');
		charS.add('\uB228');
		charS.add('\uB229');
		charS.add('\uB22A');
		charS.add('\uB22B');
		charS.add('\uB22C');
		charS.add('\uB22D');
		charS.add('\uB22E');
		charS.add('\uB22F');
		charS.add('\uB230');
		charS.add('\uB231');
		charS.add('\uB232');
		charS.add('\uB233');
		charS.add('\uB234');
		charS.add('\uB235');
		charS.add('\uB236');
		charS.add('\uB237');
		charS.add('\uB238');
		charS.add('\uB239');
		charS.add('\uB23A');
		charS.add('\uB23B');
		charS.add('\uB23C');
		charS.add('\uB23D');
		charS.add('\uB23E');
		charS.add('\uB23F');
		charS.add('\uB240');
		charS.add('\uB241');
		charS.add('\uB242');
		charS.add('\uB243');
		charS.add('\uB244');
		charS.add('\uB245');
		charS.add('\uB246');
		charS.add('\uB247');
		charS.add('\uB248');
		charS.add('\uB249');
		charS.add('\uB24A');
		charS.add('\uB24B');
		charS.add('\uB24C');
		charS.add('\uB24D');
		charS.add('\uB24E');
		charS.add('\uB24F');
		charS.add('\uB250');
		charS.add('\uB251');
		charS.add('\uB252');
		charS.add('\uB253');
		charS.add('\uB254');
		charS.add('\uB255');
		charS.add('\uB256');
		charS.add('\uB257');
		charS.add('\uB258');
		charS.add('\uB259');
		charS.add('\uB25A');
		charS.add('\uB25B');
		charS.add('\uB25C');
		charS.add('\uB25D');
		charS.add('\uB25E');
		charS.add('\uB25F');
		charS.add('\uB260');
		charS.add('\uB261');
		charS.add('\uB262');
		charS.add('\uB263');
		charS.add('\uB264');
		charS.add('\uB265');
		charS.add('\uB266');
		charS.add('\uB267');
		charS.add('\uB268');
		charS.add('\uB269');
		charS.add('\uB26A');
		charS.add('\uB26B');
		charS.add('\uB26C');
		charS.add('\uB26D');
		charS.add('\uB26E');
		charS.add('\uB26F');
		charS.add('\uB270');
		charS.add('\uB271');
		charS.add('\uB272');
		charS.add('\uB273');
		charS.add('\uB274');
		charS.add('\uB275');
		charS.add('\uB276');
		charS.add('\uB277');
		charS.add('\uB278');
		charS.add('\uB279');
		charS.add('\uB27A');
		charS.add('\uB27B');
		charS.add('\uB27C');
		charS.add('\uB27D');
		charS.add('\uB27E');
		charS.add('\uB27F');
		charS.add('\uB280');
		charS.add('\uB281');
		charS.add('\uB282');
		charS.add('\uB283');
		charS.add('\uB284');
		charS.add('\uB285');
		charS.add('\uB286');
		charS.add('\uB287');
		charS.add('\uB288');
		charS.add('\uB289');
		charS.add('\uB28A');
		charS.add('\uB28B');
		charS.add('\uB28C');
		charS.add('\uB28D');
		charS.add('\uB28E');
		charS.add('\uB28F');
		charS.add('\uB290');
		charS.add('\uB291');
		charS.add('\uB292');
		charS.add('\uB293');
		charS.add('\uB294');
		charS.add('\uB295');
		charS.add('\uB296');
		charS.add('\uB297');
		charS.add('\uB298');
		charS.add('\uB299');
		charS.add('\uB29A');
		charS.add('\uB29B');
		charS.add('\uB29C');
		charS.add('\uB29D');
		charS.add('\uB29E');
		charS.add('\uB29F');
		charS.add('\uB2A0');
		charS.add('\uB2A1');
		charS.add('\uB2A2');
		charS.add('\uB2A3');
		charS.add('\uB2A4');
		charS.add('\uB2A5');
		charS.add('\uB2A6');
		charS.add('\uB2A7');
		charS.add('\uB2A8');
		charS.add('\uB2A9');
		charS.add('\uB2AA');
		charS.add('\uB2AB');
		charS.add('\uB2AC');
		charS.add('\uB2AD');
		charS.add('\uB2AE');
		charS.add('\uB2AF');
		charS.add('\uB2B0');
		charS.add('\uB2B1');
		charS.add('\uB2B2');
		charS.add('\uB2B3');
		charS.add('\uB2B4');
		charS.add('\uB2B5');
		charS.add('\uB2B6');
		charS.add('\uB2B7');
		charS.add('\uB2B8');
		charS.add('\uB2B9');
		charS.add('\uB2BA');
		charS.add('\uB2BB');
		charS.add('\uB2BC');
		charS.add('\uB23D');
		charS.add('\uB2BE');
		charS.add('\uB2BF');
		charS.add('\uB2C0');
		charS.add('\uB2C1');
		charS.add('\uB2C2');
		charS.add('\uB2C3');
		charS.add('\uB2C4');
		charS.add('\uB2C5');
		charS.add('\uB2C6');
		charS.add('\uB2C7');
		charS.add('\uB2C8');
		charS.add('\uB2C9');
		charS.add('\uB2CA');
		charS.add('\uB2CB');
		charS.add('\uB2CC');
		charS.add('\uB2CD');
		charS.add('\uB2CE');
		charS.add('\uB2CF');
		charS.add('\uB2D0');
		charS.add('\uB2D1');
		charS.add('\uB2D2');
		charS.add('\uB2D3');
		charS.add('\uB2D4');
		charS.add('\uB2D5');
		charS.add('\uB2D6');
		charS.add('\uB2D7');
		charS.add('\uB2D8');
		charS.add('\uB2D9');
		charS.add('\uB2DA');
		charS.add('\uB2DB');
		charS.add('\uB2DC');
		charS.add('\uB2DD');
		charS.add('\uB2DE');
		charS.add('\uB2DF');
		charS.add('\uB2E0');
		charS.add('\uB2E1');
		charS.add('\uB2E2');
		charS.add('\uB2E3');
		charS.add('\uB2E4');
		charS.add('\uB2E5');
		charS.add('\uB2E6');
		charS.add('\uB2E7');
		charS.add('\uB2E8');
		charS.add('\uB2E9');
		charS.add('\uB2EA');
		charS.add('\uB2EB');
		charS.add('\uB2EC');
		charS.add('\uB2ED');
		charS.add('\uB2EE');
		charS.add('\uB2EF');
		charS.add('\uB2F0');
		charS.add('\uB2F1');
		charS.add('\uB2F2');
		charS.add('\uB2F3');
		charS.add('\uB2F4');
		charS.add('\uB2F5');
		charS.add('\uB2F6');
		charS.add('\uB2F7');
		charS.add('\uB2F8');
		charS.add('\uB2F9');
		charS.add('\uB2FA');
		charS.add('\uB2FB');
		charS.add('\uB2FC');
		charS.add('\uB2FD');
		charS.add('\uB2FE');
		charS.add('\uB2FF');
		charS.add('\uB300');
		charS.add('\uB301');
		charS.add('\uB302');
		charS.add('\uB303');
		charS.add('\uB304');
		charS.add('\uB305');
		charS.add('\uB306');
		charS.add('\uB307');
		charS.add('\uB308');
		charS.add('\uB309');
		charS.add('\uB30A');
		charS.add('\uB30B');
		charS.add('\uB30C');
		charS.add('\uB30D');
		charS.add('\uB30E');
		charS.add('\uB30F');
		charS.add('\uB310');
		charS.add('\uB311');
		charS.add('\uB312');
		charS.add('\uB313');
		charS.add('\uB314');
		charS.add('\uB315');
		charS.add('\uB316');
		charS.add('\uB317');
		charS.add('\uB318');
		charS.add('\uB319');
		charS.add('\uB31A');
		charS.add('\uB31B');
		charS.add('\uB31C');
		charS.add('\uB31D');
		charS.add('\uB31E');
		charS.add('\uB31F');
		charS.add('\uB320');
		charS.add('\uB321');
		charS.add('\uB322');
		charS.add('\uB323');
		charS.add('\uB324');
		charS.add('\uB325');
		charS.add('\uB326');
		charS.add('\uB327');
		charS.add('\uB328');
		charS.add('\uB329');
		charS.add('\uB32A');
		charS.add('\uB32B');
		charS.add('\uB32C');
		charS.add('\uB32D');
		charS.add('\uB32E');
		charS.add('\uB32F');
		charS.add('\uB330');
		charS.add('\uB331');
		charS.add('\uB332');
		charS.add('\uB333');
		charS.add('\uB334');
		charS.add('\uB335');
		charS.add('\uB336');
		charS.add('\uB337');
		charS.add('\uB338');
		charS.add('\uB339');
		charS.add('\uB33A');
		charS.add('\uB33B');
		charS.add('\uB33C');
		charS.add('\uB33D');
		charS.add('\uB33E');
		charS.add('\uB33F');
		charS.add('\uB340');
		charS.add('\uB341');
		charS.add('\uB342');
		charS.add('\uB343');
		charS.add('\uB344');
		charS.add('\uB345');
		charS.add('\uB346');
		charS.add('\uB347');
		charS.add('\uB348');
		charS.add('\uB349');
		charS.add('\uB34A');
		charS.add('\uB34B');
		charS.add('\uB34C');
		charS.add('\uB34D');
		charS.add('\uB34E');
		charS.add('\uB34F');
		charS.add('\uB350');
		charS.add('\uB351');
		charS.add('\uB352');
		charS.add('\uB353');
		charS.add('\uB354');
		charS.add('\uB355');
		charS.add('\uB356');
		charS.add('\uB357');
		charS.add('\uB358');
		charS.add('\uB359');
		charS.add('\uB35A');
		charS.add('\uB35B');
		charS.add('\uB35C');
		charS.add('\uB35D');
		charS.add('\uB35E');
		charS.add('\uB35F');
		charS.add('\uB360');
		charS.add('\uB361');
		charS.add('\uB362');
		charS.add('\uB363');
		charS.add('\uB364');
		charS.add('\uB365');
		charS.add('\uB366');
		charS.add('\uB367');
		charS.add('\uB368');
		charS.add('\uB369');
		charS.add('\uB36A');
		charS.add('\uB36B');
		charS.add('\uB36C');
		charS.add('\uB36D');
		charS.add('\uB36E');
		charS.add('\uB36F');
		charS.add('\uB370');
		charS.add('\uB371');
		charS.add('\uB372');
		charS.add('\uB373');
		charS.add('\uB374');
		charS.add('\uB375');
		charS.add('\uB376');
		charS.add('\uB377');
		charS.add('\uB378');
		charS.add('\uB379');
		charS.add('\uB37A');
		charS.add('\uB37B');
		charS.add('\uB37C');
		charS.add('\uB37D');
		charS.add('\uB37E');
		charS.add('\uB37F');
		charS.add('\uB380');
		charS.add('\uB381');
		charS.add('\uB382');
		charS.add('\uB383');
		charS.add('\uB384');
		charS.add('\uB385');
		charS.add('\uB386');
		charS.add('\uB387');
		charS.add('\uB388');
		charS.add('\uB389');
		charS.add('\uB38A');
		charS.add('\uB38B');
		charS.add('\uB38C');
		charS.add('\uB38D');
		charS.add('\uB38E');
		charS.add('\uB38F');
		charS.add('\uB390');
		charS.add('\uB391');
		charS.add('\uB392');
		charS.add('\uB393');
		charS.add('\uB394');
		charS.add('\uB395');
		charS.add('\uB396');
		charS.add('\uB397');
		charS.add('\uB398');
		charS.add('\uB399');
		charS.add('\uB39A');
		charS.add('\uB39B');
		charS.add('\uB39C');
		charS.add('\uB39D');
		charS.add('\uB39E');
		charS.add('\uB39F');
		charS.add('\uB3A0');
		charS.add('\uB3A1');
		charS.add('\uB3A2');
		charS.add('\uB3A3');
		charS.add('\uB3A4');
		charS.add('\uB3A5');
		charS.add('\uB3A6');
		charS.add('\uB3A7');
		charS.add('\uB3A8');
		charS.add('\uB3A9');
		charS.add('\uB3AA');
		charS.add('\uB3AB');
		charS.add('\uB3AC');
		charS.add('\uB3AD');
		charS.add('\uB3AE');
		charS.add('\uB3AF');
		charS.add('\uB3B0');
		charS.add('\uB3B1');
		charS.add('\uB3B2');
		charS.add('\uB3B3');
		charS.add('\uB3B4');
		charS.add('\uB3B5');
		charS.add('\uB3B6');
		charS.add('\uB3B7');
		charS.add('\uB3B8');
		charS.add('\uB3B9');
		charS.add('\uB3BA');
		charS.add('\uB3BB');
		charS.add('\uB3BC');
		charS.add('\uB33D');
		charS.add('\uB3BE');
		charS.add('\uB3BF');
		charS.add('\uB3C0');
		charS.add('\uB3C1');
		charS.add('\uB3C2');
		charS.add('\uB3C3');
		charS.add('\uB3C4');
		charS.add('\uB3C5');
		charS.add('\uB3C6');
		charS.add('\uB3C7');
		charS.add('\uB3C8');
		charS.add('\uB3C9');
		charS.add('\uB3CA');
		charS.add('\uB3CB');
		charS.add('\uB3CC');
		charS.add('\uB3CD');
		charS.add('\uB3CE');
		charS.add('\uB3CF');
		charS.add('\uB3D0');
		charS.add('\uB3D1');
		charS.add('\uB3D2');
		charS.add('\uB3D3');
		charS.add('\uB3D4');
		charS.add('\uB3D5');
		charS.add('\uB3D6');
		charS.add('\uB3D7');
		charS.add('\uB3D8');
		charS.add('\uB3D9');
		charS.add('\uB3DA');
		charS.add('\uB3DB');
		charS.add('\uB3DC');
		charS.add('\uB3DD');
		charS.add('\uB3DE');
		charS.add('\uB3DF');
		charS.add('\uB3E0');
		charS.add('\uB3E1');
		charS.add('\uB3E2');
		charS.add('\uB3E3');
		charS.add('\uB3E4');
		charS.add('\uB3E5');
		charS.add('\uB3E6');
		charS.add('\uB3E7');
		charS.add('\uB3E8');
		charS.add('\uB3E9');
		charS.add('\uB3EA');
		charS.add('\uB3EB');
		charS.add('\uB3EC');
		charS.add('\uB3ED');
		charS.add('\uB3EE');
		charS.add('\uB3EF');
		charS.add('\uB3F0');
		charS.add('\uB3F1');
		charS.add('\uB3F2');
		charS.add('\uB3F3');
		charS.add('\uB3F4');
		charS.add('\uB3F5');
		charS.add('\uB3F6');
		charS.add('\uB3F7');
		charS.add('\uB3F8');
		charS.add('\uB3F9');
		charS.add('\uB3FA');
		charS.add('\uB3FB');
		charS.add('\uB3FC');
		charS.add('\uB3FD');
		charS.add('\uB3FE');
		charS.add('\uB3FF');
		charS.add('\uB400');
		charS.add('\uB401');
		charS.add('\uB402');
		charS.add('\uB403');
		charS.add('\uB404');
		charS.add('\uB405');
		charS.add('\uB406');
		charS.add('\uB407');
		charS.add('\uB408');
		charS.add('\uB409');
		charS.add('\uB40A');
		charS.add('\uB40B');
		charS.add('\uB40C');
		charS.add('\uB40D');
		charS.add('\uB40E');
		charS.add('\uB40F');
		charS.add('\uB410');
		charS.add('\uB411');
		charS.add('\uB412');
		charS.add('\uB413');
		charS.add('\uB414');
		charS.add('\uB415');
		charS.add('\uB416');
		charS.add('\uB417');
		charS.add('\uB418');
		charS.add('\uB419');
		charS.add('\uB41A');
		charS.add('\uB41B');
		charS.add('\uB41C');
		charS.add('\uB41D');
		charS.add('\uB41E');
		charS.add('\uB41F');
		charS.add('\uB420');
		charS.add('\uB421');
		charS.add('\uB422');
		charS.add('\uB423');
		charS.add('\uB424');
		charS.add('\uB425');
		charS.add('\uB426');
		charS.add('\uB427');
		charS.add('\uB428');
		charS.add('\uB429');
		charS.add('\uB42A');
		charS.add('\uB42B');
		charS.add('\uB42C');
		charS.add('\uB42D');
		charS.add('\uB42E');
		charS.add('\uB42F');
		charS.add('\uB430');
		charS.add('\uB431');
		charS.add('\uB432');
		charS.add('\uB433');
		charS.add('\uB434');
		charS.add('\uB435');
		charS.add('\uB436');
		charS.add('\uB437');
		charS.add('\uB438');
		charS.add('\uB439');
		charS.add('\uB43A');
		charS.add('\uB43B');
		charS.add('\uB43C');
		charS.add('\uB43D');
		charS.add('\uB43E');
		charS.add('\uB43F');
		charS.add('\uB440');
		charS.add('\uB441');
		charS.add('\uB442');
		charS.add('\uB443');
		charS.add('\uB444');
		charS.add('\uB445');
		charS.add('\uB446');
		charS.add('\uB447');
		charS.add('\uB448');
		charS.add('\uB449');
		charS.add('\uB44A');
		charS.add('\uB44B');
		charS.add('\uB44C');
		charS.add('\uB44D');
		charS.add('\uB44E');
		charS.add('\uB44F');
		charS.add('\uB450');
		charS.add('\uB451');
		charS.add('\uB452');
		charS.add('\uB453');
		charS.add('\uB454');
		charS.add('\uB455');
		charS.add('\uB456');
		charS.add('\uB457');
		charS.add('\uB458');
		charS.add('\uB459');
		charS.add('\uB45A');
		charS.add('\uB45B');
		charS.add('\uB45C');
		charS.add('\uB45D');
		charS.add('\uB45E');
		charS.add('\uB45F');
		charS.add('\uB460');
		charS.add('\uB461');
		charS.add('\uB462');
		charS.add('\uB463');
		charS.add('\uB464');
		charS.add('\uB465');
		charS.add('\uB466');
		charS.add('\uB467');
		charS.add('\uB468');
		charS.add('\uB469');
		charS.add('\uB46A');
		charS.add('\uB46B');
		charS.add('\uB46C');
		charS.add('\uB46D');
		charS.add('\uB46E');
		charS.add('\uB46F');
		charS.add('\uB470');
		charS.add('\uB471');
		charS.add('\uB472');
		charS.add('\uB473');
		charS.add('\uB474');
		charS.add('\uB475');
		charS.add('\uB476');
		charS.add('\uB477');
		charS.add('\uB478');
		charS.add('\uB479');
		charS.add('\uB47A');
		charS.add('\uB47B');
		charS.add('\uB47C');
		charS.add('\uB47D');
		charS.add('\uB47E');
		charS.add('\uB47F');
		charS.add('\uB480');
		charS.add('\uB481');
		charS.add('\uB482');
		charS.add('\uB483');
		charS.add('\uB484');
		charS.add('\uB485');
		charS.add('\uB486');
		charS.add('\uB487');
		charS.add('\uB488');
		charS.add('\uB489');
		charS.add('\uB48A');
		charS.add('\uB48B');
		charS.add('\uB48C');
		charS.add('\uB48D');
		charS.add('\uB48E');
		charS.add('\uB48F');
		charS.add('\uB490');
		charS.add('\uB491');
		charS.add('\uB492');
		charS.add('\uB493');
		charS.add('\uB494');
		charS.add('\uB495');
		charS.add('\uB496');
		charS.add('\uB497');
		charS.add('\uB498');
		charS.add('\uB499');
		charS.add('\uB49A');
		charS.add('\uB49B');
		charS.add('\uB49C');
		charS.add('\uB49D');
		charS.add('\uB49E');
		charS.add('\uB49F');
		charS.add('\uB4A0');
		charS.add('\uB4A1');
		charS.add('\uB4A2');
		charS.add('\uB4A3');
		charS.add('\uB4A4');
		charS.add('\uB4A5');
		charS.add('\uB4A6');
		charS.add('\uB4A7');
		charS.add('\uB4A8');
		charS.add('\uB4A9');
		charS.add('\uB4AA');
		charS.add('\uB4AB');
		charS.add('\uB4AC');
		charS.add('\uB4AD');
		charS.add('\uB4AE');
		charS.add('\uB4AF');
		charS.add('\uB4B0');
		charS.add('\uB4B1');
		charS.add('\uB4B2');
		charS.add('\uB4B3');
		charS.add('\uB4B4');
		charS.add('\uB4B5');
		charS.add('\uB4B6');
		charS.add('\uB4B7');
		charS.add('\uB4B8');
		charS.add('\uB4B9');
		charS.add('\uB4BA');
		charS.add('\uB4BB');
		charS.add('\uB4BC');
		charS.add('\uB43D');
		charS.add('\uB4BE');
		charS.add('\uB4BF');
		charS.add('\uB4C0');
		charS.add('\uB4C1');
		charS.add('\uB4C2');
		charS.add('\uB4C3');
		charS.add('\uB4C4');
		charS.add('\uB4C5');
		charS.add('\uB4C6');
		charS.add('\uB4C7');
		charS.add('\uB4C8');
		charS.add('\uB4C9');
		charS.add('\uB4CA');
		charS.add('\uB4CB');
		charS.add('\uB4CC');
		charS.add('\uB4CD');
		charS.add('\uB4CE');
		charS.add('\uB4CF');
		charS.add('\uB4D0');
		charS.add('\uB4D1');
		charS.add('\uB4D2');
		charS.add('\uB4D3');
		charS.add('\uB4D4');
		charS.add('\uB4D5');
		charS.add('\uB4D6');
		charS.add('\uB4D7');
		charS.add('\uB4D8');
		charS.add('\uB4D9');
		charS.add('\uB4DA');
		charS.add('\uB4DB');
		charS.add('\uB4DC');
		charS.add('\uB4DD');
		charS.add('\uB4DE');
		charS.add('\uB4DF');
		charS.add('\uB4E0');
		charS.add('\uB4E1');
		charS.add('\uB4E2');
		charS.add('\uB4E3');
		charS.add('\uB4E4');
		charS.add('\uB4E5');
		charS.add('\uB4E6');
		charS.add('\uB4E7');
		charS.add('\uB4E8');
		charS.add('\uB4E9');
		charS.add('\uB4EA');
		charS.add('\uB4EB');
		charS.add('\uB4EC');
		charS.add('\uB4ED');
		charS.add('\uB4EE');
		charS.add('\uB4EF');
		charS.add('\uB4F0');
		charS.add('\uB4F1');
		charS.add('\uB4F2');
		charS.add('\uB4F3');
		charS.add('\uB4F4');
		charS.add('\uB4F5');
		charS.add('\uB4F6');
		charS.add('\uB4F7');
		charS.add('\uB4F8');
		charS.add('\uB4F9');
		charS.add('\uB4FA');
		charS.add('\uB4FB');
		charS.add('\uB4FC');
		charS.add('\uB4FD');
		charS.add('\uB4FE');
		charS.add('\uB4FF');
		charS.add('\uB500');
		charS.add('\uB501');
		charS.add('\uB502');
		charS.add('\uB503');
		charS.add('\uB504');
		charS.add('\uB505');
		charS.add('\uB506');
		charS.add('\uB507');
		charS.add('\uB508');
		charS.add('\uB509');
		charS.add('\uB50A');
		charS.add('\uB50B');
		charS.add('\uB50C');
		charS.add('\uB50D');
		charS.add('\uB50E');
		charS.add('\uB50F');
		charS.add('\uB510');
		charS.add('\uB511');
		charS.add('\uB512');
		charS.add('\uB513');
		charS.add('\uB514');
		charS.add('\uB515');
		charS.add('\uB516');
		charS.add('\uB517');
		charS.add('\uB518');
		charS.add('\uB519');
		charS.add('\uB51A');
		charS.add('\uB51B');
		charS.add('\uB51C');
		charS.add('\uB51D');
		charS.add('\uB51E');
		charS.add('\uB51F');
		charS.add('\uB520');
		charS.add('\uB521');
		charS.add('\uB522');
		charS.add('\uB523');
		charS.add('\uB524');
		charS.add('\uB525');
		charS.add('\uB526');
		charS.add('\uB527');
		charS.add('\uB528');
		charS.add('\uB529');
		charS.add('\uB52A');
		charS.add('\uB52B');
		charS.add('\uB52C');
		charS.add('\uB52D');
		charS.add('\uB52E');
		charS.add('\uB52F');
		charS.add('\uB530');
		charS.add('\uB531');
		charS.add('\uB532');
		charS.add('\uB533');
		charS.add('\uB534');
		charS.add('\uB535');
		charS.add('\uB536');
		charS.add('\uB537');
		charS.add('\uB538');
		charS.add('\uB539');
		charS.add('\uB53A');
		charS.add('\uB53B');
		charS.add('\uB53C');
		charS.add('\uB53D');
		charS.add('\uB53E');
		charS.add('\uB53F');
		charS.add('\uB540');
		charS.add('\uB541');
		charS.add('\uB542');
		charS.add('\uB543');
		charS.add('\uB544');
		charS.add('\uB545');
		charS.add('\uB546');
		charS.add('\uB547');
		charS.add('\uB548');
		charS.add('\uB549');
		charS.add('\uB54A');
		charS.add('\uB54B');
		charS.add('\uB54C');
		charS.add('\uB54D');
		charS.add('\uB54E');
		charS.add('\uB54F');
		charS.add('\uB550');
		charS.add('\uB551');
		charS.add('\uB552');
		charS.add('\uB553');
		charS.add('\uB554');
		charS.add('\uB555');
		charS.add('\uB556');
		charS.add('\uB557');
		charS.add('\uB558');
		charS.add('\uB559');
		charS.add('\uB55A');
		charS.add('\uB55B');
		charS.add('\uB55C');
		charS.add('\uB55D');
		charS.add('\uB55E');
		charS.add('\uB55F');
		charS.add('\uB560');
		charS.add('\uB561');
		charS.add('\uB562');
		charS.add('\uB563');
		charS.add('\uB564');
		charS.add('\uB565');
		charS.add('\uB566');
		charS.add('\uB567');
		charS.add('\uB568');
		charS.add('\uB569');
		charS.add('\uB56A');
		charS.add('\uB56B');
		charS.add('\uB56C');
		charS.add('\uB56D');
		charS.add('\uB56E');
		charS.add('\uB56F');
		charS.add('\uB570');
		charS.add('\uB571');
		charS.add('\uB572');
		charS.add('\uB573');
		charS.add('\uB574');
		charS.add('\uB575');
		charS.add('\uB576');
		charS.add('\uB577');
		charS.add('\uB578');
		charS.add('\uB579');
		charS.add('\uB57A');
		charS.add('\uB57B');
		charS.add('\uB57C');
		charS.add('\uB57D');
		charS.add('\uB57E');
		charS.add('\uB57F');
		charS.add('\uB580');
		charS.add('\uB581');
		charS.add('\uB582');
		charS.add('\uB583');
		charS.add('\uB584');
		charS.add('\uB585');
		charS.add('\uB586');
		charS.add('\uB587');
		charS.add('\uB588');
		charS.add('\uB589');
		charS.add('\uB58A');
		charS.add('\uB58B');
		charS.add('\uB58C');
		charS.add('\uB58D');
		charS.add('\uB58E');
		charS.add('\uB58F');
		charS.add('\uB590');
		charS.add('\uB591');
		charS.add('\uB592');
		charS.add('\uB593');
		charS.add('\uB594');
		charS.add('\uB595');
		charS.add('\uB596');
		charS.add('\uB597');
		charS.add('\uB598');
		charS.add('\uB599');
		charS.add('\uB59A');
		charS.add('\uB59B');
		charS.add('\uB59C');
		charS.add('\uB59D');
		charS.add('\uB59E');
		charS.add('\uB59F');
		charS.add('\uB5A0');
		charS.add('\uB5A1');
		charS.add('\uB5A2');
		charS.add('\uB5A3');
		charS.add('\uB5A4');
		charS.add('\uB5A5');
		charS.add('\uB5A6');
		charS.add('\uB5A7');
		charS.add('\uB5A8');
		charS.add('\uB5A9');
		charS.add('\uB5AA');
		charS.add('\uB5AB');
		charS.add('\uB5AC');
		charS.add('\uB5AD');
		charS.add('\uB5AE');
		charS.add('\uB5AF');
		charS.add('\uB5B0');
		charS.add('\uB5B1');
		charS.add('\uB5B2');
		charS.add('\uB5B3');
		charS.add('\uB5B4');
		charS.add('\uB5B5');
		charS.add('\uB5B6');
		charS.add('\uB5B7');
		charS.add('\uB5B8');
		charS.add('\uB5B9');
		charS.add('\uB5BA');
		charS.add('\uB5BB');
		charS.add('\uB5BC');
		charS.add('\uB53D');
		charS.add('\uB5BE');
		charS.add('\uB5BF');
		charS.add('\uB5C0');
		charS.add('\uB5C1');
		charS.add('\uB5C2');
		charS.add('\uB5C3');
		charS.add('\uB5C4');
		charS.add('\uB5C5');
		charS.add('\uB5C6');
		charS.add('\uB5C7');
		charS.add('\uB5C8');
		charS.add('\uB5C9');
		charS.add('\uB5CA');
		charS.add('\uB5CB');
		charS.add('\uB5CC');
		charS.add('\uB5CD');
		charS.add('\uB5CE');
		charS.add('\uB5CF');
		charS.add('\uB5D0');
		charS.add('\uB5D1');
		charS.add('\uB5D2');
		charS.add('\uB5D3');
		charS.add('\uB5D4');
		charS.add('\uB5D5');
		charS.add('\uB5D6');
		charS.add('\uB5D7');
		charS.add('\uB5D8');
		charS.add('\uB5D9');
		charS.add('\uB5DA');
		charS.add('\uB5DB');
		charS.add('\uB5DC');
		charS.add('\uB5DD');
		charS.add('\uB5DE');
		charS.add('\uB5DF');
		charS.add('\uB5E0');
		charS.add('\uB5E1');
		charS.add('\uB5E2');
		charS.add('\uB5E3');
		charS.add('\uB5E4');
		charS.add('\uB5E5');
		charS.add('\uB5E6');
		charS.add('\uB5E7');
		charS.add('\uB5E8');
		charS.add('\uB5E9');
		charS.add('\uB5EA');
		charS.add('\uB5EB');
		charS.add('\uB5EC');
		charS.add('\uB5ED');
		charS.add('\uB5EE');
		charS.add('\uB5EF');
		charS.add('\uB5F0');
		charS.add('\uB5F1');
		charS.add('\uB5F2');
		charS.add('\uB5F3');
		charS.add('\uB5F4');
		charS.add('\uB5F5');
		charS.add('\uB5F6');
		charS.add('\uB5F7');
		charS.add('\uB5F8');
		charS.add('\uB5F9');
		charS.add('\uB5FA');
		charS.add('\uB5FB');
		charS.add('\uB5FC');
		charS.add('\uB5FD');
		charS.add('\uB5FE');
		charS.add('\uB5FF');
		charS.add('\uB600');
		charS.add('\uB601');
		charS.add('\uB602');
		charS.add('\uB603');
		charS.add('\uB604');
		charS.add('\uB605');
		charS.add('\uB606');
		charS.add('\uB607');
		charS.add('\uB608');
		charS.add('\uB609');
		charS.add('\uB60A');
		charS.add('\uB60B');
		charS.add('\uB60C');
		charS.add('\uB60D');
		charS.add('\uB60E');
		charS.add('\uB60F');
		charS.add('\uB610');
		charS.add('\uB611');
		charS.add('\uB612');
		charS.add('\uB613');
		charS.add('\uB614');
		charS.add('\uB615');
		charS.add('\uB616');
		charS.add('\uB617');
		charS.add('\uB618');
		charS.add('\uB619');
		charS.add('\uB61A');
		charS.add('\uB61B');
		charS.add('\uB61C');
		charS.add('\uB61D');
		charS.add('\uB61E');
		charS.add('\uB61F');
		charS.add('\uB620');
		charS.add('\uB621');
		charS.add('\uB622');
		charS.add('\uB623');
		charS.add('\uB624');
		charS.add('\uB625');
		charS.add('\uB626');
		charS.add('\uB627');
		charS.add('\uB628');
		charS.add('\uB629');
		charS.add('\uB62A');
		charS.add('\uB62B');
		charS.add('\uB62C');
		charS.add('\uB62D');
		charS.add('\uB62E');
		charS.add('\uB62F');
		charS.add('\uB630');
		charS.add('\uB631');
		charS.add('\uB632');
		charS.add('\uB633');
		charS.add('\uB634');
		charS.add('\uB635');
		charS.add('\uB636');
		charS.add('\uB637');
		charS.add('\uB638');
		charS.add('\uB639');
		charS.add('\uB63A');
		charS.add('\uB63B');
		charS.add('\uB63C');
		charS.add('\uB63D');
		charS.add('\uB63E');
		charS.add('\uB63F');
		charS.add('\uB640');
		charS.add('\uB641');
		charS.add('\uB642');
		charS.add('\uB643');
		charS.add('\uB644');
		charS.add('\uB645');
		charS.add('\uB646');
		charS.add('\uB647');
		charS.add('\uB648');
		charS.add('\uB649');
		charS.add('\uB64A');
		charS.add('\uB64B');
		charS.add('\uB64C');
		charS.add('\uB64D');
		charS.add('\uB64E');
		charS.add('\uB64F');
		charS.add('\uB650');
		charS.add('\uB651');
		charS.add('\uB652');
		charS.add('\uB653');
		charS.add('\uB654');
		charS.add('\uB655');
		charS.add('\uB656');
		charS.add('\uB657');
		charS.add('\uB658');
		charS.add('\uB659');
		charS.add('\uB65A');
		charS.add('\uB65B');
		charS.add('\uB65C');
		charS.add('\uB65D');
		charS.add('\uB65E');
		charS.add('\uB65F');
		charS.add('\uB660');
		charS.add('\uB661');
		charS.add('\uB662');
		charS.add('\uB663');
		charS.add('\uB664');
		charS.add('\uB665');
		charS.add('\uB666');
		charS.add('\uB667');
		charS.add('\uB668');
		charS.add('\uB669');
		charS.add('\uB66A');
		charS.add('\uB66B');
		charS.add('\uB66C');
		charS.add('\uB66D');
		charS.add('\uB66E');
		charS.add('\uB66F');
		charS.add('\uB670');
		charS.add('\uB671');
		charS.add('\uB672');
		charS.add('\uB673');
		charS.add('\uB674');
		charS.add('\uB675');
		charS.add('\uB676');
		charS.add('\uB677');
		charS.add('\uB678');
		charS.add('\uB679');
		charS.add('\uB67A');
		charS.add('\uB67B');
		charS.add('\uB67C');
		charS.add('\uB67D');
		charS.add('\uB67E');
		charS.add('\uB67F');
		charS.add('\uB680');
		charS.add('\uB681');
		charS.add('\uB682');
		charS.add('\uB683');
		charS.add('\uB684');
		charS.add('\uB685');
		charS.add('\uB686');
		charS.add('\uB687');
		charS.add('\uB688');
		charS.add('\uB689');
		charS.add('\uB68A');
		charS.add('\uB68B');
		charS.add('\uB68C');
		charS.add('\uB68D');
		charS.add('\uB68E');
		charS.add('\uB68F');
		charS.add('\uB690');
		charS.add('\uB691');
		charS.add('\uB692');
		charS.add('\uB693');
		charS.add('\uB694');
		charS.add('\uB695');
		charS.add('\uB696');
		charS.add('\uB697');
		charS.add('\uB698');
		charS.add('\uB699');
		charS.add('\uB69A');
		charS.add('\uB69B');
		charS.add('\uB69C');
		charS.add('\uB69D');
		charS.add('\uB69E');
		charS.add('\uB69F');
		charS.add('\uB6A0');
		charS.add('\uB6A1');
		charS.add('\uB6A2');
		charS.add('\uB6A3');
		charS.add('\uB6A4');
		charS.add('\uB6A5');
		charS.add('\uB6A6');
		charS.add('\uB6A7');
		charS.add('\uB6A8');
		charS.add('\uB6A9');
		charS.add('\uB6AA');
		charS.add('\uB6AB');
		charS.add('\uB6AC');
		charS.add('\uB6AD');
		charS.add('\uB6AE');
		charS.add('\uB6AF');
		charS.add('\uB6B0');
		charS.add('\uB6B1');
		charS.add('\uB6B2');
		charS.add('\uB6B3');
		charS.add('\uB6B4');
		charS.add('\uB6B5');
		charS.add('\uB6B6');
		charS.add('\uB6B7');
		charS.add('\uB6B8');
		charS.add('\uB6B9');
		charS.add('\uB6BA');
		charS.add('\uB6BB');
		charS.add('\uB6BC');
		charS.add('\uB63D');
		charS.add('\uB6BE');
		charS.add('\uB6BF');
		charS.add('\uB6C0');
		charS.add('\uB6C1');
		charS.add('\uB6C2');
		charS.add('\uB6C3');
		charS.add('\uB6C4');
		charS.add('\uB6C5');
		charS.add('\uB6C6');
		charS.add('\uB6C7');
		charS.add('\uB6C8');
		charS.add('\uB6C9');
		charS.add('\uB6CA');
		charS.add('\uB6CB');
		charS.add('\uB6CC');
		charS.add('\uB6CD');
		charS.add('\uB6CE');
		charS.add('\uB6CF');
		charS.add('\uB6D0');
		charS.add('\uB6D1');
		charS.add('\uB6D2');
		charS.add('\uB6D3');
		charS.add('\uB6D4');
		charS.add('\uB6D5');
		charS.add('\uB6D6');
		charS.add('\uB6D7');
		charS.add('\uB6D8');
		charS.add('\uB6D9');
		charS.add('\uB6DA');
		charS.add('\uB6DB');
		charS.add('\uB6DC');
		charS.add('\uB6DD');
		charS.add('\uB6DE');
		charS.add('\uB6DF');
		charS.add('\uB6E0');
		charS.add('\uB6E1');
		charS.add('\uB6E2');
		charS.add('\uB6E3');
		charS.add('\uB6E4');
		charS.add('\uB6E5');
		charS.add('\uB6E6');
		charS.add('\uB6E7');
		charS.add('\uB6E8');
		charS.add('\uB6E9');
		charS.add('\uB6EA');
		charS.add('\uB6EB');
		charS.add('\uB6EC');
		charS.add('\uB6ED');
		charS.add('\uB6EE');
		charS.add('\uB6EF');
		charS.add('\uB6F0');
		charS.add('\uB6F1');
		charS.add('\uB6F2');
		charS.add('\uB6F3');
		charS.add('\uB6F4');
		charS.add('\uB6F5');
		charS.add('\uB6F6');
		charS.add('\uB6F7');
		charS.add('\uB6F8');
		charS.add('\uB6F9');
		charS.add('\uB6FA');
		charS.add('\uB6FB');
		charS.add('\uB6FC');
		charS.add('\uB6FD');
		charS.add('\uB6FE');
		charS.add('\uB6FF');
		charS.add('\uB700');
		charS.add('\uB701');
		charS.add('\uB702');
		charS.add('\uB703');
		charS.add('\uB704');
		charS.add('\uB705');
		charS.add('\uB706');
		charS.add('\uB707');
		charS.add('\uB708');
		charS.add('\uB709');
		charS.add('\uB70A');
		charS.add('\uB70B');
		charS.add('\uB70C');
		charS.add('\uB70D');
		charS.add('\uB70E');
		charS.add('\uB70F');
		charS.add('\uB710');
		charS.add('\uB711');
		charS.add('\uB712');
		charS.add('\uB713');
		charS.add('\uB714');
		charS.add('\uB715');
		charS.add('\uB716');
		charS.add('\uB717');
		charS.add('\uB718');
		charS.add('\uB719');
		charS.add('\uB71A');
		charS.add('\uB71B');
		charS.add('\uB71C');
		charS.add('\uB71D');
		charS.add('\uB71E');
		charS.add('\uB71F');
		charS.add('\uB720');
		charS.add('\uB721');
		charS.add('\uB722');
		charS.add('\uB723');
		charS.add('\uB724');
		charS.add('\uB725');
		charS.add('\uB726');
		charS.add('\uB727');
		charS.add('\uB728');
		charS.add('\uB729');
		charS.add('\uB72A');
		charS.add('\uB72B');
		charS.add('\uB72C');
		charS.add('\uB72D');
		charS.add('\uB72E');
		charS.add('\uB72F');
		charS.add('\uB730');
		charS.add('\uB731');
		charS.add('\uB732');
		charS.add('\uB733');
		charS.add('\uB734');
		charS.add('\uB735');
		charS.add('\uB736');
		charS.add('\uB737');
		charS.add('\uB738');
		charS.add('\uB739');
		charS.add('\uB73A');
		charS.add('\uB73B');
		charS.add('\uB73C');
		charS.add('\uB73D');
		charS.add('\uB73E');
		charS.add('\uB73F');
		charS.add('\uB740');
		charS.add('\uB741');
		charS.add('\uB742');
		charS.add('\uB743');
		charS.add('\uB744');
		charS.add('\uB745');
		charS.add('\uB746');
		charS.add('\uB747');
		charS.add('\uB748');
		charS.add('\uB749');
		charS.add('\uB74A');
		charS.add('\uB74B');
		charS.add('\uB74C');
		charS.add('\uB74D');
		charS.add('\uB74E');
		charS.add('\uB74F');
		charS.add('\uB750');
		charS.add('\uB751');
		charS.add('\uB752');
		charS.add('\uB753');
		charS.add('\uB754');
		charS.add('\uB755');
		charS.add('\uB756');
		charS.add('\uB757');
		charS.add('\uB758');
		charS.add('\uB759');
		charS.add('\uB75A');
		charS.add('\uB75B');
		charS.add('\uB75C');
		charS.add('\uB75D');
		charS.add('\uB75E');
		charS.add('\uB75F');
		charS.add('\uB760');
		charS.add('\uB761');
		charS.add('\uB762');
		charS.add('\uB763');
		charS.add('\uB764');
		charS.add('\uB765');
		charS.add('\uB766');
		charS.add('\uB767');
		charS.add('\uB768');
		charS.add('\uB769');
		charS.add('\uB76A');
		charS.add('\uB76B');
		charS.add('\uB76C');
		charS.add('\uB76D');
		charS.add('\uB76E');
		charS.add('\uB76F');
		charS.add('\uB770');
		charS.add('\uB771');
		charS.add('\uB772');
		charS.add('\uB773');
		charS.add('\uB774');
		charS.add('\uB775');
		charS.add('\uB776');
		charS.add('\uB777');
		charS.add('\uB778');
		charS.add('\uB779');
		charS.add('\uB77A');
		charS.add('\uB77B');
		charS.add('\uB77C');
		charS.add('\uB77D');
		charS.add('\uB77E');
		charS.add('\uB77F');
		charS.add('\uB780');
		charS.add('\uB781');
		charS.add('\uB782');
		charS.add('\uB783');
		charS.add('\uB784');
		charS.add('\uB785');
		charS.add('\uB786');
		charS.add('\uB787');
		charS.add('\uB788');
		charS.add('\uB789');
		charS.add('\uB78A');
		charS.add('\uB78B');
		charS.add('\uB78C');
		charS.add('\uB78D');
		charS.add('\uB78E');
		charS.add('\uB78F');
		charS.add('\uB790');
		charS.add('\uB791');
		charS.add('\uB792');
		charS.add('\uB793');
		charS.add('\uB794');
		charS.add('\uB795');
		charS.add('\uB796');
		charS.add('\uB797');
		charS.add('\uB798');
		charS.add('\uB799');
		charS.add('\uB79A');
		charS.add('\uB79B');
		charS.add('\uB79C');
		charS.add('\uB79D');
		charS.add('\uB79E');
		charS.add('\uB79F');
		charS.add('\uB7A0');
		charS.add('\uB7A1');
		charS.add('\uB7A2');
		charS.add('\uB7A3');
		charS.add('\uB7A4');
		charS.add('\uB7A5');
		charS.add('\uB7A6');
		charS.add('\uB7A7');
		charS.add('\uB7A8');
		charS.add('\uB7A9');
		charS.add('\uB7AA');
		charS.add('\uB7AB');
		charS.add('\uB7AC');
		charS.add('\uB7AD');
		charS.add('\uB7AE');
		charS.add('\uB7AF');
		charS.add('\uB7B0');
		charS.add('\uB7B1');
		charS.add('\uB7B2');
		charS.add('\uB7B3');
		charS.add('\uB7B4');
		charS.add('\uB7B5');
		charS.add('\uB7B6');
		charS.add('\uB7B7');
		charS.add('\uB7B8');
		charS.add('\uB7B9');
		charS.add('\uB7BA');
		charS.add('\uB7BB');
		charS.add('\uB7BC');
		charS.add('\uB73D');
		charS.add('\uB7BE');
		charS.add('\uB7BF');
		charS.add('\uB7C0');
		charS.add('\uB7C1');
		charS.add('\uB7C2');
		charS.add('\uB7C3');
		charS.add('\uB7C4');
		charS.add('\uB7C5');
		charS.add('\uB7C6');
		charS.add('\uB7C7');
		charS.add('\uB7C8');
		charS.add('\uB7C9');
		charS.add('\uB7CA');
		charS.add('\uB7CB');
		charS.add('\uB7CC');
		charS.add('\uB7CD');
		charS.add('\uB7CE');
		charS.add('\uB7CF');
		charS.add('\uB7D0');
		charS.add('\uB7D1');
		charS.add('\uB7D2');
		charS.add('\uB7D3');
		charS.add('\uB7D4');
		charS.add('\uB7D5');
		charS.add('\uB7D6');
		charS.add('\uB7D7');
		charS.add('\uB7D8');
		charS.add('\uB7D9');
		charS.add('\uB7DA');
		charS.add('\uB7DB');
		charS.add('\uB7DC');
		charS.add('\uB7DD');
		charS.add('\uB7DE');
		charS.add('\uB7DF');
		charS.add('\uB7E0');
		charS.add('\uB7E1');
		charS.add('\uB7E2');
		charS.add('\uB7E3');
		charS.add('\uB7E4');
		charS.add('\uB7E5');
		charS.add('\uB7E6');
		charS.add('\uB7E7');
		charS.add('\uB7E8');
		charS.add('\uB7E9');
		charS.add('\uB7EA');
		charS.add('\uB7EB');
		charS.add('\uB7EC');
		charS.add('\uB7ED');
		charS.add('\uB7EE');
		charS.add('\uB7EF');
		charS.add('\uB7F0');
		charS.add('\uB7F1');
		charS.add('\uB7F2');
		charS.add('\uB7F3');
		charS.add('\uB7F4');
		charS.add('\uB7F5');
		charS.add('\uB7F6');
		charS.add('\uB7F7');
		charS.add('\uB7F8');
		charS.add('\uB7F9');
		charS.add('\uB7FA');
		charS.add('\uB7FB');
		charS.add('\uB7FC');
		charS.add('\uB7FD');
		charS.add('\uB7FE');
		charS.add('\uB7FF');
		charS.add('\uB800');
		charS.add('\uB801');
		charS.add('\uB802');
		charS.add('\uB803');
		charS.add('\uB804');
		charS.add('\uB805');
		charS.add('\uB806');
		charS.add('\uB807');
		charS.add('\uB808');
		charS.add('\uB809');
		charS.add('\uB80A');
		charS.add('\uB80B');
		charS.add('\uB80C');
		charS.add('\uB80D');
		charS.add('\uB80E');
		charS.add('\uB80F');
		charS.add('\uB810');
		charS.add('\uB811');
		charS.add('\uB812');
		charS.add('\uB813');
		charS.add('\uB814');
		charS.add('\uB815');
		charS.add('\uB816');
		charS.add('\uB817');
		charS.add('\uB818');
		charS.add('\uB819');
		charS.add('\uB81A');
		charS.add('\uB81B');
		charS.add('\uB81C');
		charS.add('\uB81D');
		charS.add('\uB81E');
		charS.add('\uB81F');
		charS.add('\uB820');
		charS.add('\uB821');
		charS.add('\uB822');
		charS.add('\uB823');
		charS.add('\uB824');
		charS.add('\uB825');
		charS.add('\uB826');
		charS.add('\uB827');
		charS.add('\uB828');
		charS.add('\uB829');
		charS.add('\uB82A');
		charS.add('\uB82B');
		charS.add('\uB82C');
		charS.add('\uB82D');
		charS.add('\uB82E');
		charS.add('\uB82F');
		charS.add('\uB830');
		charS.add('\uB831');
		charS.add('\uB832');
		charS.add('\uB833');
		charS.add('\uB834');
		charS.add('\uB835');
		charS.add('\uB836');
		charS.add('\uB837');
		charS.add('\uB838');
		charS.add('\uB839');
		charS.add('\uB83A');
		charS.add('\uB83B');
		charS.add('\uB83C');
		charS.add('\uB83D');
		charS.add('\uB83E');
		charS.add('\uB83F');
		charS.add('\uB840');
		charS.add('\uB841');
		charS.add('\uB842');
		charS.add('\uB843');
		charS.add('\uB844');
		charS.add('\uB845');
		charS.add('\uB846');
		charS.add('\uB847');
		charS.add('\uB848');
		charS.add('\uB849');
		charS.add('\uB84A');
		charS.add('\uB84B');
		charS.add('\uB84C');
		charS.add('\uB84D');
		charS.add('\uB84E');
		charS.add('\uB84F');
		charS.add('\uB850');
		charS.add('\uB851');
		charS.add('\uB852');
		charS.add('\uB853');
		charS.add('\uB854');
		charS.add('\uB855');
		charS.add('\uB856');
		charS.add('\uB857');
		charS.add('\uB858');
		charS.add('\uB859');
		charS.add('\uB85A');
		charS.add('\uB85B');
		charS.add('\uB85C');
		charS.add('\uB85D');
		charS.add('\uB85E');
		charS.add('\uB85F');
		charS.add('\uB860');
		charS.add('\uB861');
		charS.add('\uB862');
		charS.add('\uB863');
		charS.add('\uB864');
		charS.add('\uB865');
		charS.add('\uB866');
		charS.add('\uB867');
		charS.add('\uB868');
		charS.add('\uB869');
		charS.add('\uB86A');
		charS.add('\uB86B');
		charS.add('\uB86C');
		charS.add('\uB86D');
		charS.add('\uB86E');
		charS.add('\uB86F');
		charS.add('\uB870');
		charS.add('\uB871');
		charS.add('\uB872');
		charS.add('\uB873');
		charS.add('\uB874');
		charS.add('\uB875');
		charS.add('\uB876');
		charS.add('\uB877');
		charS.add('\uB878');
		charS.add('\uB879');
		charS.add('\uB87A');
		charS.add('\uB87B');
		charS.add('\uB87C');
		charS.add('\uB87D');
		charS.add('\uB87E');
		charS.add('\uB87F');
		charS.add('\uB880');
		charS.add('\uB881');
		charS.add('\uB882');
		charS.add('\uB883');
		charS.add('\uB884');
		charS.add('\uB885');
		charS.add('\uB886');
		charS.add('\uB887');
		charS.add('\uB888');
		charS.add('\uB889');
		charS.add('\uB88A');
		charS.add('\uB88B');
		charS.add('\uB88C');
		charS.add('\uB88D');
		charS.add('\uB88E');
		charS.add('\uB88F');
		charS.add('\uB890');
		charS.add('\uB891');
		charS.add('\uB892');
		charS.add('\uB893');
		charS.add('\uB894');
		charS.add('\uB895');
		charS.add('\uB896');
		charS.add('\uB897');
		charS.add('\uB898');
		charS.add('\uB899');
		charS.add('\uB89A');
		charS.add('\uB89B');
		charS.add('\uB89C');
		charS.add('\uB89D');
		charS.add('\uB89E');
		charS.add('\uB89F');
		charS.add('\uB8A0');
		charS.add('\uB8A1');
		charS.add('\uB8A2');
		charS.add('\uB8A3');
		charS.add('\uB8A4');
		charS.add('\uB8A5');
		charS.add('\uB8A6');
		charS.add('\uB8A7');
		charS.add('\uB8A8');
		charS.add('\uB8A9');
		charS.add('\uB8AA');
		charS.add('\uB8AB');
		charS.add('\uB8AC');
		charS.add('\uB8AD');
		charS.add('\uB8AE');
		charS.add('\uB8AF');
		charS.add('\uB8B0');
		charS.add('\uB8B1');
		charS.add('\uB8B2');
		charS.add('\uB8B3');
		charS.add('\uB8B4');
		charS.add('\uB8B5');
		charS.add('\uB8B6');
		charS.add('\uB8B7');
		charS.add('\uB8B8');
		charS.add('\uB8B9');
		charS.add('\uB8BA');
		charS.add('\uB8BB');
		charS.add('\uB8BC');
		charS.add('\uB83D');
		charS.add('\uB8BE');
		charS.add('\uB8BF');
		charS.add('\uB8C0');
		charS.add('\uB8C1');
		charS.add('\uB8C2');
		charS.add('\uB8C3');
		charS.add('\uB8C4');
		charS.add('\uB8C5');
		charS.add('\uB8C6');
		charS.add('\uB8C7');
		charS.add('\uB8C8');
		charS.add('\uB8C9');
		charS.add('\uB8CA');
		charS.add('\uB8CB');
		charS.add('\uB8CC');
		charS.add('\uB8CD');
		charS.add('\uB8CE');
		charS.add('\uB8CF');
		charS.add('\uB8D0');
		charS.add('\uB8D1');
		charS.add('\uB8D2');
		charS.add('\uB8D3');
		charS.add('\uB8D4');
		charS.add('\uB8D5');
		charS.add('\uB8D6');
		charS.add('\uB8D7');
		charS.add('\uB8D8');
		charS.add('\uB8D9');
		charS.add('\uB8DA');
		charS.add('\uB8DB');
		charS.add('\uB8DC');
		charS.add('\uB8DD');
		charS.add('\uB8DE');
		charS.add('\uB8DF');
		charS.add('\uB8E0');
		charS.add('\uB8E1');
		charS.add('\uB8E2');
		charS.add('\uB8E3');
		charS.add('\uB8E4');
		charS.add('\uB8E5');
		charS.add('\uB8E6');
		charS.add('\uB8E7');
		charS.add('\uB8E8');
		charS.add('\uB8E9');
		charS.add('\uB8EA');
		charS.add('\uB8EB');
		charS.add('\uB8EC');
		charS.add('\uB8ED');
		charS.add('\uB8EE');
		charS.add('\uB8EF');
		charS.add('\uB8F0');
		charS.add('\uB8F1');
		charS.add('\uB8F2');
		charS.add('\uB8F3');
		charS.add('\uB8F4');
		charS.add('\uB8F5');
		charS.add('\uB8F6');
		charS.add('\uB8F7');
		charS.add('\uB8F8');
		charS.add('\uB8F9');
		charS.add('\uB8FA');
		charS.add('\uB8FB');
		charS.add('\uB8FC');
		charS.add('\uB8FD');
		charS.add('\uB8FE');
		charS.add('\uB8FF');
		charS.add('\uB900');
		charS.add('\uB901');
		charS.add('\uB902');
		charS.add('\uB903');
		charS.add('\uB904');
		charS.add('\uB905');
		charS.add('\uB906');
		charS.add('\uB907');
		charS.add('\uB908');
		charS.add('\uB909');
		charS.add('\uB90A');
		charS.add('\uB90B');
		charS.add('\uB90C');
		charS.add('\uB90D');
		charS.add('\uB90E');
		charS.add('\uB90F');
		charS.add('\uB910');
		charS.add('\uB911');
		charS.add('\uB912');
		charS.add('\uB913');
		charS.add('\uB914');
		charS.add('\uB915');
		charS.add('\uB916');
		charS.add('\uB917');
		charS.add('\uB918');
		charS.add('\uB919');
		charS.add('\uB91A');
		charS.add('\uB91B');
		charS.add('\uB91C');
		charS.add('\uB91D');
		charS.add('\uB91E');
		charS.add('\uB91F');
		charS.add('\uB920');
		charS.add('\uB921');
		charS.add('\uB922');
		charS.add('\uB923');
		charS.add('\uB924');
		charS.add('\uB925');
		charS.add('\uB926');
		charS.add('\uB927');
		charS.add('\uB928');
		charS.add('\uB929');
		charS.add('\uB92A');
		charS.add('\uB92B');
		charS.add('\uB92C');
		charS.add('\uB92D');
		charS.add('\uB92E');
		charS.add('\uB92F');
		charS.add('\uB930');
		charS.add('\uB931');
		charS.add('\uB932');
		charS.add('\uB933');
		charS.add('\uB934');
		charS.add('\uB935');
		charS.add('\uB936');
		charS.add('\uB937');
		charS.add('\uB938');
		charS.add('\uB939');
		charS.add('\uB93A');
		charS.add('\uB93B');
		charS.add('\uB93C');
		charS.add('\uB93D');
		charS.add('\uB93E');
		charS.add('\uB93F');
		charS.add('\uB940');
		charS.add('\uB941');
		charS.add('\uB942');
		charS.add('\uB943');
		charS.add('\uB944');
		charS.add('\uB945');
		charS.add('\uB946');
		charS.add('\uB947');
		charS.add('\uB948');
		charS.add('\uB949');
		charS.add('\uB94A');
		charS.add('\uB94B');
		charS.add('\uB94C');
		charS.add('\uB94D');
		charS.add('\uB94E');
		charS.add('\uB94F');
		charS.add('\uB950');
		charS.add('\uB951');
		charS.add('\uB952');
		charS.add('\uB953');
		charS.add('\uB954');
		charS.add('\uB955');
		charS.add('\uB956');
		charS.add('\uB957');
		charS.add('\uB958');
		charS.add('\uB959');
		charS.add('\uB95A');
		charS.add('\uB95B');
		charS.add('\uB95C');
		charS.add('\uB95D');
		charS.add('\uB95E');
		charS.add('\uB95F');
		charS.add('\uB960');
		charS.add('\uB961');
		charS.add('\uB962');
		charS.add('\uB963');
		charS.add('\uB964');
		charS.add('\uB965');
		charS.add('\uB966');
		charS.add('\uB967');
		charS.add('\uB968');
		charS.add('\uB969');
		charS.add('\uB96A');
		charS.add('\uB96B');
		charS.add('\uB96C');
		charS.add('\uB96D');
		charS.add('\uB96E');
		charS.add('\uB96F');
		charS.add('\uB970');
		charS.add('\uB971');
		charS.add('\uB972');
		charS.add('\uB973');
		charS.add('\uB974');
		charS.add('\uB975');
		charS.add('\uB976');
		charS.add('\uB977');
		charS.add('\uB978');
		charS.add('\uB979');
		charS.add('\uB97A');
		charS.add('\uB97B');
		charS.add('\uB97C');
		charS.add('\uB97D');
		charS.add('\uB97E');
		charS.add('\uB97F');
		charS.add('\uB980');
		charS.add('\uB981');
		charS.add('\uB982');
		charS.add('\uB983');
		charS.add('\uB984');
		charS.add('\uB985');
		charS.add('\uB986');
		charS.add('\uB987');
		charS.add('\uB988');
		charS.add('\uB989');
		charS.add('\uB98A');
		charS.add('\uB98B');
		charS.add('\uB98C');
		charS.add('\uB98D');
		charS.add('\uB98E');
		charS.add('\uB98F');
		charS.add('\uB990');
		charS.add('\uB991');
		charS.add('\uB992');
		charS.add('\uB993');
		charS.add('\uB994');
		charS.add('\uB995');
		charS.add('\uB996');
		charS.add('\uB997');
		charS.add('\uB998');
		charS.add('\uB999');
		charS.add('\uB99A');
		charS.add('\uB99B');
		charS.add('\uB99C');
		charS.add('\uB99D');
		charS.add('\uB99E');
		charS.add('\uB99F');
		charS.add('\uB9A0');
		charS.add('\uB9A1');
		charS.add('\uB9A2');
		charS.add('\uB9A3');
		charS.add('\uB9A4');
		charS.add('\uB9A5');
		charS.add('\uB9A6');
		charS.add('\uB9A7');
		charS.add('\uB9A8');
		charS.add('\uB9A9');
		charS.add('\uB9AA');
		charS.add('\uB9AB');
		charS.add('\uB9AC');
		charS.add('\uB9AD');
		charS.add('\uB9AE');
		charS.add('\uB9AF');
		charS.add('\uB9B0');
		charS.add('\uB9B1');
		charS.add('\uB9B2');
		charS.add('\uB9B3');
		charS.add('\uB9B4');
		charS.add('\uB9B5');
		charS.add('\uB9B6');
		charS.add('\uB9B7');
		charS.add('\uB9B8');
		charS.add('\uB9B9');
		charS.add('\uB9BA');
		charS.add('\uB9BB');
		charS.add('\uB9BC');
		charS.add('\uB93D');
		charS.add('\uB9BE');
		charS.add('\uB9BF');
		charS.add('\uB9C0');
		charS.add('\uB9C1');
		charS.add('\uB9C2');
		charS.add('\uB9C3');
		charS.add('\uB9C4');
		charS.add('\uB9C5');
		charS.add('\uB9C6');
		charS.add('\uB9C7');
		charS.add('\uB9C8');
		charS.add('\uB9C9');
		charS.add('\uB9CA');
		charS.add('\uB9CB');
		charS.add('\uB9CC');
		charS.add('\uB9CD');
		charS.add('\uB9CE');
		charS.add('\uB9CF');
		charS.add('\uB9D0');
		charS.add('\uB9D1');
		charS.add('\uB9D2');
		charS.add('\uB9D3');
		charS.add('\uB9D4');
		charS.add('\uB9D5');
		charS.add('\uB9D6');
		charS.add('\uB9D7');
		charS.add('\uB9D8');
		charS.add('\uB9D9');
		charS.add('\uB9DA');
		charS.add('\uB9DB');
		charS.add('\uB9DC');
		charS.add('\uB9DD');
		charS.add('\uB9DE');
		charS.add('\uB9DF');
		charS.add('\uB9E0');
		charS.add('\uB9E1');
		charS.add('\uB9E2');
		charS.add('\uB9E3');
		charS.add('\uB9E4');
		charS.add('\uB9E5');
		charS.add('\uB9E6');
		charS.add('\uB9E7');
		charS.add('\uB9E8');
		charS.add('\uB9E9');
		charS.add('\uB9EA');
		charS.add('\uB9EB');
		charS.add('\uB9EC');
		charS.add('\uB9ED');
		charS.add('\uB9EE');
		charS.add('\uB9EF');
		charS.add('\uB9F0');
		charS.add('\uB9F1');
		charS.add('\uB9F2');
		charS.add('\uB9F3');
		charS.add('\uB9F4');
		charS.add('\uB9F5');
		charS.add('\uB9F6');
		charS.add('\uB9F7');
		charS.add('\uB9F8');
		charS.add('\uB9F9');
		charS.add('\uB9FA');
		charS.add('\uB9FB');
		charS.add('\uB9FC');
		charS.add('\uB9FD');
		charS.add('\uB9FE');
		charS.add('\uB9FF');
		charS.add('\uBA00');
		charS.add('\uBA01');
		charS.add('\uBA02');
		charS.add('\uBA03');
		charS.add('\uBA04');
		charS.add('\uBA05');
		charS.add('\uBA06');
		charS.add('\uBA07');
		charS.add('\uBA08');
		charS.add('\uBA09');
		charS.add('\uBA0A');
		charS.add('\uBA0B');
		charS.add('\uBA0C');
		charS.add('\uBA0D');
		charS.add('\uBA0E');
		charS.add('\uBA0F');
		charS.add('\uBA10');
		charS.add('\uBA11');
		charS.add('\uBA12');
		charS.add('\uBA13');
		charS.add('\uBA14');
		charS.add('\uBA15');
		charS.add('\uBA16');
		charS.add('\uBA17');
		charS.add('\uBA18');
		charS.add('\uBA19');
		charS.add('\uBA1A');
		charS.add('\uBA1B');
		charS.add('\uBA1C');
		charS.add('\uBA1D');
		charS.add('\uBA1E');
		charS.add('\uBA1F');
		charS.add('\uBA20');
		charS.add('\uBA21');
		charS.add('\uBA22');
		charS.add('\uBA23');
		charS.add('\uBA24');
		charS.add('\uBA25');
		charS.add('\uBA26');
		charS.add('\uBA27');
		charS.add('\uBA28');
		charS.add('\uBA29');
		charS.add('\uBA2A');
		charS.add('\uBA2B');
		charS.add('\uBA2C');
		charS.add('\uBA2D');
		charS.add('\uBA2E');
		charS.add('\uBA2F');
		charS.add('\uBA30');
		charS.add('\uBA31');
		charS.add('\uBA32');
		charS.add('\uBA33');
		charS.add('\uBA34');
		charS.add('\uBA35');
		charS.add('\uBA36');
		charS.add('\uBA37');
		charS.add('\uBA38');
		charS.add('\uBA39');
		charS.add('\uBA3A');
		charS.add('\uBA3B');
		charS.add('\uBA3C');
		charS.add('\uBA3D');
		charS.add('\uBA3E');
		charS.add('\uBA3F');
		charS.add('\uBA40');
		charS.add('\uBA41');
		charS.add('\uBA42');
		charS.add('\uBA43');
		charS.add('\uBA44');
		charS.add('\uBA45');
		charS.add('\uBA46');
		charS.add('\uBA47');
		charS.add('\uBA48');
		charS.add('\uBA49');
		charS.add('\uBA4A');
		charS.add('\uBA4B');
		charS.add('\uBA4C');
		charS.add('\uBA4D');
		charS.add('\uBA4E');
		charS.add('\uBA4F');
		charS.add('\uBA50');
		charS.add('\uBA51');
		charS.add('\uBA52');
		charS.add('\uBA53');
		charS.add('\uBA54');
		charS.add('\uBA55');
		charS.add('\uBA56');
		charS.add('\uBA57');
		charS.add('\uBA58');
		charS.add('\uBA59');
		charS.add('\uBA5A');
		charS.add('\uBA5B');
		charS.add('\uBA5C');
		charS.add('\uBA5D');
		charS.add('\uBA5E');
		charS.add('\uBA5F');
		charS.add('\uBA60');
		charS.add('\uBA61');
		charS.add('\uBA62');
		charS.add('\uBA63');
		charS.add('\uBA64');
		charS.add('\uBA65');
		charS.add('\uBA66');
		charS.add('\uBA67');
		charS.add('\uBA68');
		charS.add('\uBA69');
		charS.add('\uBA6A');
		charS.add('\uBA6B');
		charS.add('\uBA6C');
		charS.add('\uBA6D');
		charS.add('\uBA6E');
		charS.add('\uBA6F');
		charS.add('\uBA70');
		charS.add('\uBA71');
		charS.add('\uBA72');
		charS.add('\uBA73');
		charS.add('\uBA74');
		charS.add('\uBA75');
		charS.add('\uBA76');
		charS.add('\uBA77');
		charS.add('\uBA78');
		charS.add('\uBA79');
		charS.add('\uBA7A');
		charS.add('\uBA7B');
		charS.add('\uBA7C');
		charS.add('\uBA7D');
		charS.add('\uBA7E');
		charS.add('\uBA7F');
		charS.add('\uBA80');
		charS.add('\uBA81');
		charS.add('\uBA82');
		charS.add('\uBA83');
		charS.add('\uBA84');
		charS.add('\uBA85');
		charS.add('\uBA86');
		charS.add('\uBA87');
		charS.add('\uBA88');
		charS.add('\uBA89');
		charS.add('\uBA8A');
		charS.add('\uBA8B');
		charS.add('\uBA8C');
		charS.add('\uBA8D');
		charS.add('\uBA8E');
		charS.add('\uBA8F');
		charS.add('\uBA90');
		charS.add('\uBA91');
		charS.add('\uBA92');
		charS.add('\uBA93');
		charS.add('\uBA94');
		charS.add('\uBA95');
		charS.add('\uBA96');
		charS.add('\uBA97');
		charS.add('\uBA98');
		charS.add('\uBA99');
		charS.add('\uBA9A');
		charS.add('\uBA9B');
		charS.add('\uBA9C');
		charS.add('\uBA9D');
		charS.add('\uBA9E');
		charS.add('\uBA9F');
		charS.add('\uBAA0');
		charS.add('\uBAA1');
		charS.add('\uBAA2');
		charS.add('\uBAA3');
		charS.add('\uBAA4');
		charS.add('\uBAA5');
		charS.add('\uBAA6');
		charS.add('\uBAA7');
		charS.add('\uBAA8');
		charS.add('\uBAA9');
		charS.add('\uBAAA');
		charS.add('\uBAAB');
		charS.add('\uBAAC');
		charS.add('\uBAAD');
		charS.add('\uBAAE');
		charS.add('\uBAAF');
		charS.add('\uBAB0');
		charS.add('\uBAB1');
		charS.add('\uBAB2');
		charS.add('\uBAB3');
		charS.add('\uBAB4');
		charS.add('\uBAB5');
		charS.add('\uBAB6');
		charS.add('\uBAB7');
		charS.add('\uBAB8');
		charS.add('\uBAB9');
		charS.add('\uBABA');
		charS.add('\uBABB');
		charS.add('\uBABC');
		charS.add('\uBA3D');
		charS.add('\uBABE');
		charS.add('\uBABF');
		charS.add('\uBAC0');
		charS.add('\uBAC1');
		charS.add('\uBAC2');
		charS.add('\uBAC3');
		charS.add('\uBAC4');
		charS.add('\uBAC5');
		charS.add('\uBAC6');
		charS.add('\uBAC7');
		charS.add('\uBAC8');
		charS.add('\uBAC9');
		charS.add('\uBACA');
		charS.add('\uBACB');
		charS.add('\uBACC');
		charS.add('\uBACD');
		charS.add('\uBACE');
		charS.add('\uBACF');
		charS.add('\uBAD0');
		charS.add('\uBAD1');
		charS.add('\uBAD2');
		charS.add('\uBAD3');
		charS.add('\uBAD4');
		charS.add('\uBAD5');
		charS.add('\uBAD6');
		charS.add('\uBAD7');
		charS.add('\uBAD8');
		charS.add('\uBAD9');
		charS.add('\uBADA');
		charS.add('\uBADB');
		charS.add('\uBADC');
		charS.add('\uBADD');
		charS.add('\uBADE');
		charS.add('\uBADF');
		charS.add('\uBAE0');
		charS.add('\uBAE1');
		charS.add('\uBAE2');
		charS.add('\uBAE3');
		charS.add('\uBAE4');
		charS.add('\uBAE5');
		charS.add('\uBAE6');
		charS.add('\uBAE7');
		charS.add('\uBAE8');
		charS.add('\uBAE9');
		charS.add('\uBAEA');
		charS.add('\uBAEB');
		charS.add('\uBAEC');
		charS.add('\uBAED');
		charS.add('\uBAEE');
		charS.add('\uBAEF');
		charS.add('\uBAF0');
		charS.add('\uBAF1');
		charS.add('\uBAF2');
		charS.add('\uBAF3');
		charS.add('\uBAF4');
		charS.add('\uBAF5');
		charS.add('\uBAF6');
		charS.add('\uBAF7');
		charS.add('\uBAF8');
		charS.add('\uBAF9');
		charS.add('\uBAFA');
		charS.add('\uBAFB');
		charS.add('\uBAFC');
		charS.add('\uBAFD');
		charS.add('\uBAFE');
		charS.add('\uBAFF');
		charS.add('\uBB00');
		charS.add('\uBB01');
		charS.add('\uBB02');
		charS.add('\uBB03');
		charS.add('\uBB04');
		charS.add('\uBB05');
		charS.add('\uBB06');
		charS.add('\uBB07');
		charS.add('\uBB08');
		charS.add('\uBB09');
		charS.add('\uBB0A');
		charS.add('\uBB0B');
		charS.add('\uBB0C');
		charS.add('\uBB0D');
		charS.add('\uBB0E');
		charS.add('\uBB0F');
		charS.add('\uBB10');
		charS.add('\uBB11');
		charS.add('\uBB12');
		charS.add('\uBB13');
		charS.add('\uBB14');
		charS.add('\uBB15');
		charS.add('\uBB16');
		charS.add('\uBB17');
		charS.add('\uBB18');
		charS.add('\uBB19');
		charS.add('\uBB1A');
		charS.add('\uBB1B');
		charS.add('\uBB1C');
		charS.add('\uBB1D');
		charS.add('\uBB1E');
		charS.add('\uBB1F');
		charS.add('\uBB20');
		charS.add('\uBB21');
		charS.add('\uBB22');
		charS.add('\uBB23');
		charS.add('\uBB24');
		charS.add('\uBB25');
		charS.add('\uBB26');
		charS.add('\uBB27');
		charS.add('\uBB28');
		charS.add('\uBB29');
		charS.add('\uBB2A');
		charS.add('\uBB2B');
		charS.add('\uBB2C');
		charS.add('\uBB2D');
		charS.add('\uBB2E');
		charS.add('\uBB2F');
		charS.add('\uBB30');
		charS.add('\uBB31');
		charS.add('\uBB32');
		charS.add('\uBB33');
		charS.add('\uBB34');
		charS.add('\uBB35');
		charS.add('\uBB36');
		charS.add('\uBB37');
		charS.add('\uBB38');
		charS.add('\uBB39');
		charS.add('\uBB3A');
		charS.add('\uBB3B');
		charS.add('\uBB3C');
		charS.add('\uBB3D');
		charS.add('\uBB3E');
		charS.add('\uBB3F');
		charS.add('\uBB40');
		charS.add('\uBB41');
		charS.add('\uBB42');
		charS.add('\uBB43');
		charS.add('\uBB44');
		charS.add('\uBB45');
		charS.add('\uBB46');
		charS.add('\uBB47');
		charS.add('\uBB48');
		charS.add('\uBB49');
		charS.add('\uBB4A');
		charS.add('\uBB4B');
		charS.add('\uBB4C');
		charS.add('\uBB4D');
		charS.add('\uBB4E');
		charS.add('\uBB4F');
		charS.add('\uBB50');
		charS.add('\uBB51');
		charS.add('\uBB52');
		charS.add('\uBB53');
		charS.add('\uBB54');
		charS.add('\uBB55');
		charS.add('\uBB56');
		charS.add('\uBB57');
		charS.add('\uBB58');
		charS.add('\uBB59');
		charS.add('\uBB5A');
		charS.add('\uBB5B');
		charS.add('\uBB5C');
		charS.add('\uBB5D');
		charS.add('\uBB5E');
		charS.add('\uBB5F');
		charS.add('\uBB60');
		charS.add('\uBB61');
		charS.add('\uBB62');
		charS.add('\uBB63');
		charS.add('\uBB64');
		charS.add('\uBB65');
		charS.add('\uBB66');
		charS.add('\uBB67');
		charS.add('\uBB68');
		charS.add('\uBB69');
		charS.add('\uBB6A');
		charS.add('\uBB6B');
		charS.add('\uBB6C');
		charS.add('\uBB6D');
		charS.add('\uBB6E');
		charS.add('\uBB6F');
		charS.add('\uBB70');
		charS.add('\uBB71');
		charS.add('\uBB72');
		charS.add('\uBB73');
		charS.add('\uBB74');
		charS.add('\uBB75');
		charS.add('\uBB76');
		charS.add('\uBB77');
		charS.add('\uBB78');
		charS.add('\uBB79');
		charS.add('\uBB7A');
		charS.add('\uBB7B');
		charS.add('\uBB7C');
		charS.add('\uBB7D');
		charS.add('\uBB7E');
		charS.add('\uBB7F');
		charS.add('\uBB80');
		charS.add('\uBB81');
		charS.add('\uBB82');
		charS.add('\uBB83');
		charS.add('\uBB84');
		charS.add('\uBB85');
		charS.add('\uBB86');
		charS.add('\uBB87');
		charS.add('\uBB88');
		charS.add('\uBB89');
		charS.add('\uBB8A');
		charS.add('\uBB8B');
		charS.add('\uBB8C');
		charS.add('\uBB8D');
		charS.add('\uBB8E');
		charS.add('\uBB8F');
		charS.add('\uBB90');
		charS.add('\uBB91');
		charS.add('\uBB92');
		charS.add('\uBB93');
		charS.add('\uBB94');
		charS.add('\uBB95');
		charS.add('\uBB96');
		charS.add('\uBB97');
		charS.add('\uBB98');
		charS.add('\uBB99');
		charS.add('\uBB9A');
		charS.add('\uBB9B');
		charS.add('\uBB9C');
		charS.add('\uBB9D');
		charS.add('\uBB9E');
		charS.add('\uBB9F');
		charS.add('\uBBA0');
		charS.add('\uBBA1');
		charS.add('\uBBA2');
		charS.add('\uBBA3');
		charS.add('\uBBA4');
		charS.add('\uBBA5');
		charS.add('\uBBA6');
		charS.add('\uBBA7');
		charS.add('\uBBA8');
		charS.add('\uBBA9');
		charS.add('\uBBAA');
		charS.add('\uBBAB');
		charS.add('\uBBAC');
		charS.add('\uBBAD');
		charS.add('\uBBAE');
		charS.add('\uBBAF');
		charS.add('\uBBB0');
		charS.add('\uBBB1');
		charS.add('\uBBB2');
		charS.add('\uBBB3');
		charS.add('\uBBB4');
		charS.add('\uBBB5');
		charS.add('\uBBB6');
		charS.add('\uBBB7');
		charS.add('\uBBB8');
		charS.add('\uBBB9');
		charS.add('\uBBBA');
		charS.add('\uBBBB');
		charS.add('\uBBBC');
		charS.add('\uBB3D');
		charS.add('\uBBBE');
		charS.add('\uBBBF');
		charS.add('\uBBC0');
		charS.add('\uBBC1');
		charS.add('\uBBC2');
		charS.add('\uBBC3');
		charS.add('\uBBC4');
		charS.add('\uBBC5');
		charS.add('\uBBC6');
		charS.add('\uBBC7');
		charS.add('\uBBC8');
		charS.add('\uBBC9');
		charS.add('\uBBCA');
		charS.add('\uBBCB');
		charS.add('\uBBCC');
		charS.add('\uBBCD');
		charS.add('\uBBCE');
		charS.add('\uBBCF');
		charS.add('\uBBD0');
		charS.add('\uBBD1');
		charS.add('\uBBD2');
		charS.add('\uBBD3');
		charS.add('\uBBD4');
		charS.add('\uBBD5');
		charS.add('\uBBD6');
		charS.add('\uBBD7');
		charS.add('\uBBD8');
		charS.add('\uBBD9');
		charS.add('\uBBDA');
		charS.add('\uBBDB');
		charS.add('\uBBDC');
		charS.add('\uBBDD');
		charS.add('\uBBDE');
		charS.add('\uBBDF');
		charS.add('\uBBE0');
		charS.add('\uBBE1');
		charS.add('\uBBE2');
		charS.add('\uBBE3');
		charS.add('\uBBE4');
		charS.add('\uBBE5');
		charS.add('\uBBE6');
		charS.add('\uBBE7');
		charS.add('\uBBE8');
		charS.add('\uBBE9');
		charS.add('\uBBEA');
		charS.add('\uBBEB');
		charS.add('\uBBEC');
		charS.add('\uBBED');
		charS.add('\uBBEE');
		charS.add('\uBBEF');
		charS.add('\uBBF0');
		charS.add('\uBBF1');
		charS.add('\uBBF2');
		charS.add('\uBBF3');
		charS.add('\uBBF4');
		charS.add('\uBBF5');
		charS.add('\uBBF6');
		charS.add('\uBBF7');
		charS.add('\uBBF8');
		charS.add('\uBBF9');
		charS.add('\uBBFA');
		charS.add('\uBBFB');
		charS.add('\uBBFC');
		charS.add('\uBBFD');
		charS.add('\uBBFE');
		charS.add('\uBBFF');
		charS.add('\uBD00');
		charS.add('\uBD01');
		charS.add('\uBD02');
		charS.add('\uBD03');
		charS.add('\uBD04');
		charS.add('\uBD05');
		charS.add('\uBD06');
		charS.add('\uBD07');
		charS.add('\uBD08');
		charS.add('\uBD09');
		charS.add('\uBD0A');
		charS.add('\uBD0B');
		charS.add('\uBD0C');
		charS.add('\uBD0D');
		charS.add('\uBD0E');
		charS.add('\uBD0F');
		charS.add('\uBD10');
		charS.add('\uBD11');
		charS.add('\uBD12');
		charS.add('\uBD13');
		charS.add('\uBD14');
		charS.add('\uBD15');
		charS.add('\uBD16');
		charS.add('\uBD17');
		charS.add('\uBD18');
		charS.add('\uBD19');
		charS.add('\uBD1A');
		charS.add('\uBD1B');
		charS.add('\uBD1C');
		charS.add('\uBD1D');
		charS.add('\uBD1E');
		charS.add('\uBD1F');
		charS.add('\uBD20');
		charS.add('\uBD21');
		charS.add('\uBD22');
		charS.add('\uBD23');
		charS.add('\uBD24');
		charS.add('\uBD25');
		charS.add('\uBD26');
		charS.add('\uBD27');
		charS.add('\uBD28');
		charS.add('\uBD29');
		charS.add('\uBD2A');
		charS.add('\uBD2B');
		charS.add('\uBD2C');
		charS.add('\uBD2D');
		charS.add('\uBD2E');
		charS.add('\uBD2F');
		charS.add('\uBD30');
		charS.add('\uBD31');
		charS.add('\uBD32');
		charS.add('\uBD33');
		charS.add('\uBD34');
		charS.add('\uBD35');
		charS.add('\uBD36');
		charS.add('\uBD37');
		charS.add('\uBD38');
		charS.add('\uBD39');
		charS.add('\uBD3A');
		charS.add('\uBD3B');
		charS.add('\uBD3C');
		charS.add('\uBD3D');
		charS.add('\uBD3E');
		charS.add('\uBD3F');
		charS.add('\uBD40');
		charS.add('\uBD41');
		charS.add('\uBD42');
		charS.add('\uBD43');
		charS.add('\uBD44');
		charS.add('\uBD45');
		charS.add('\uBD46');
		charS.add('\uBD47');
		charS.add('\uBD48');
		charS.add('\uBD49');
		charS.add('\uBD4A');
		charS.add('\uBD4B');
		charS.add('\uBD4C');
		charS.add('\uBD4D');
		charS.add('\uBD4E');
		charS.add('\uBD4F');
		charS.add('\uBD50');
		charS.add('\uBD51');
		charS.add('\uBD52');
		charS.add('\uBD53');
		charS.add('\uBD54');
		charS.add('\uBD55');
		charS.add('\uBD56');
		charS.add('\uBD57');
		charS.add('\uBD58');
		charS.add('\uBD59');
		charS.add('\uBD5A');
		charS.add('\uBD5B');
		charS.add('\uBD5C');
		charS.add('\uBD5D');
		charS.add('\uBD5E');
		charS.add('\uBD5F');
		charS.add('\uBD60');
		charS.add('\uBD61');
		charS.add('\uBD62');
		charS.add('\uBD63');
		charS.add('\uBD64');
		charS.add('\uBD65');
		charS.add('\uBD66');
		charS.add('\uBD67');
		charS.add('\uBD68');
		charS.add('\uBD69');
		charS.add('\uBD6A');
		charS.add('\uBD6B');
		charS.add('\uBD6C');
		charS.add('\uBD6D');
		charS.add('\uBD6E');
		charS.add('\uBD6F');
		charS.add('\uBD70');
		charS.add('\uBD71');
		charS.add('\uBD72');
		charS.add('\uBD73');
		charS.add('\uBD74');
		charS.add('\uBD75');
		charS.add('\uBD76');
		charS.add('\uBD77');
		charS.add('\uBD78');
		charS.add('\uBD79');
		charS.add('\uBD7A');
		charS.add('\uBD7B');
		charS.add('\uBD7C');
		charS.add('\uBD7D');
		charS.add('\uBD7E');
		charS.add('\uBD7F');
		charS.add('\uBD80');
		charS.add('\uBD81');
		charS.add('\uBD82');
		charS.add('\uBD83');
		charS.add('\uBD84');
		charS.add('\uBD85');
		charS.add('\uBD86');
		charS.add('\uBD87');
		charS.add('\uBD88');
		charS.add('\uBD89');
		charS.add('\uBD8A');
		charS.add('\uBD8B');
		charS.add('\uBD8C');
		charS.add('\uBD8D');
		charS.add('\uBD8E');
		charS.add('\uBD8F');
		charS.add('\uBD90');
		charS.add('\uBD91');
		charS.add('\uBD92');
		charS.add('\uBD93');
		charS.add('\uBD94');
		charS.add('\uBD95');
		charS.add('\uBD96');
		charS.add('\uBD97');
		charS.add('\uBD98');
		charS.add('\uBD99');
		charS.add('\uBD9A');
		charS.add('\uBD9B');
		charS.add('\uBD9C');
		charS.add('\uBD9D');
		charS.add('\uBD9E');
		charS.add('\uBD9F');
		charS.add('\uBDA0');
		charS.add('\uBDA1');
		charS.add('\uBDA2');
		charS.add('\uBDA3');
		charS.add('\uBDA4');
		charS.add('\uBDA5');
		charS.add('\uBDA6');
		charS.add('\uBDA7');
		charS.add('\uBDA8');
		charS.add('\uBDA9');
		charS.add('\uBDAA');
		charS.add('\uBDAB');
		charS.add('\uBDAC');
		charS.add('\uBDAD');
		charS.add('\uBDAE');
		charS.add('\uBDAF');
		charS.add('\uBDB0');
		charS.add('\uBDB1');
		charS.add('\uBDB2');
		charS.add('\uBDB3');
		charS.add('\uBDB4');
		charS.add('\uBDB5');
		charS.add('\uBDB6');
		charS.add('\uBDB7');
		charS.add('\uBDB8');
		charS.add('\uBDB9');
		charS.add('\uBDBA');
		charS.add('\uBDBB');
		charS.add('\uBDBC');
		charS.add('\uBD3D');
		charS.add('\uBDBE');
		charS.add('\uBDBF');
		charS.add('\uBDC0');
		charS.add('\uBDC1');
		charS.add('\uBDC2');
		charS.add('\uBDC3');
		charS.add('\uBDC4');
		charS.add('\uBDC5');
		charS.add('\uBDC6');
		charS.add('\uBDC7');
		charS.add('\uBDC8');
		charS.add('\uBDC9');
		charS.add('\uBDCA');
		charS.add('\uBDCB');
		charS.add('\uBDCC');
		charS.add('\uBDCD');
		charS.add('\uBDCE');
		charS.add('\uBDCF');
		charS.add('\uBDD0');
		charS.add('\uBDD1');
		charS.add('\uBDD2');
		charS.add('\uBDD3');
		charS.add('\uBDD4');
		charS.add('\uBDD5');
		charS.add('\uBDD6');
		charS.add('\uBDD7');
		charS.add('\uBDD8');
		charS.add('\uBDD9');
		charS.add('\uBDDA');
		charS.add('\uBDDB');
		charS.add('\uBDDC');
		charS.add('\uBDDD');
		charS.add('\uBDDE');
		charS.add('\uBDDF');
		charS.add('\uBDE0');
		charS.add('\uBDE1');
		charS.add('\uBDE2');
		charS.add('\uBDE3');
		charS.add('\uBDE4');
		charS.add('\uBDE5');
		charS.add('\uBDE6');
		charS.add('\uBDE7');
		charS.add('\uBDE8');
		charS.add('\uBDE9');
		charS.add('\uBDEA');
		charS.add('\uBDEB');
		charS.add('\uBDEC');
		charS.add('\uBDED');
		charS.add('\uBDEE');
		charS.add('\uBDEF');
		charS.add('\uBDF0');
		charS.add('\uBDF1');
		charS.add('\uBDF2');
		charS.add('\uBDF3');
		charS.add('\uBDF4');
		charS.add('\uBDF5');
		charS.add('\uBDF6');
		charS.add('\uBDF7');
		charS.add('\uBDF8');
		charS.add('\uBDF9');
		charS.add('\uBDFA');
		charS.add('\uBDFB');
		charS.add('\uBDFC');
		charS.add('\uBDFD');
		charS.add('\uBDFE');
		charS.add('\uBDFF');
		charS.add('\uBE00');
		charS.add('\uBE01');
		charS.add('\uBE02');
		charS.add('\uBE03');
		charS.add('\uBE04');
		charS.add('\uBE05');
		charS.add('\uBE06');
		charS.add('\uBE07');
		charS.add('\uBE08');
		charS.add('\uBE09');
		charS.add('\uBE0A');
		charS.add('\uBE0B');
		charS.add('\uBE0C');
		charS.add('\uBE0D');
		charS.add('\uBE0E');
		charS.add('\uBE0F');
		charS.add('\uBE10');
		charS.add('\uBE11');
		charS.add('\uBE12');
		charS.add('\uBE13');
		charS.add('\uBE14');
		charS.add('\uBE15');
		charS.add('\uBE16');
		charS.add('\uBE17');
		charS.add('\uBE18');
		charS.add('\uBE19');
		charS.add('\uBE1A');
		charS.add('\uBE1B');
		charS.add('\uBE1C');
		charS.add('\uBE1D');
		charS.add('\uBE1E');
		charS.add('\uBE1F');
		charS.add('\uBE20');
		charS.add('\uBE21');
		charS.add('\uBE22');
		charS.add('\uBE23');
		charS.add('\uBE24');
		charS.add('\uBE25');
		charS.add('\uBE26');
		charS.add('\uBE27');
		charS.add('\uBE28');
		charS.add('\uBE29');
		charS.add('\uBE2A');
		charS.add('\uBE2B');
		charS.add('\uBE2C');
		charS.add('\uBE2D');
		charS.add('\uBE2E');
		charS.add('\uBE2F');
		charS.add('\uBE30');
		charS.add('\uBE31');
		charS.add('\uBE32');
		charS.add('\uBE33');
		charS.add('\uBE34');
		charS.add('\uBE35');
		charS.add('\uBE36');
		charS.add('\uBE37');
		charS.add('\uBE38');
		charS.add('\uBE39');
		charS.add('\uBE3A');
		charS.add('\uBE3B');
		charS.add('\uBE3C');
		charS.add('\uBE3D');
		charS.add('\uBE3E');
		charS.add('\uBE3F');
		charS.add('\uBE40');
		charS.add('\uBE41');
		charS.add('\uBE42');
		charS.add('\uBE43');
		charS.add('\uBE44');
		charS.add('\uBE45');
		charS.add('\uBE46');
		charS.add('\uBE47');
		charS.add('\uBE48');
		charS.add('\uBE49');
		charS.add('\uBE4A');
		charS.add('\uBE4B');
		charS.add('\uBE4C');
		charS.add('\uBE4D');
		charS.add('\uBE4E');
		charS.add('\uBE4F');
		charS.add('\uBE50');
		charS.add('\uBE51');
		charS.add('\uBE52');
		charS.add('\uBE53');
		charS.add('\uBE54');
		charS.add('\uBE55');
		charS.add('\uBE56');
		charS.add('\uBE57');
		charS.add('\uBE58');
		charS.add('\uBE59');
		charS.add('\uBE5A');
		charS.add('\uBE5B');
		charS.add('\uBE5C');
		charS.add('\uBE5D');
		charS.add('\uBE5E');
		charS.add('\uBE5F');
		charS.add('\uBE60');
		charS.add('\uBE61');
		charS.add('\uBE62');
		charS.add('\uBE63');
		charS.add('\uBE64');
		charS.add('\uBE65');
		charS.add('\uBE66');
		charS.add('\uBE67');
		charS.add('\uBE68');
		charS.add('\uBE69');
		charS.add('\uBE6A');
		charS.add('\uBE6B');
		charS.add('\uBE6C');
		charS.add('\uBE6D');
		charS.add('\uBE6E');
		charS.add('\uBE6F');
		charS.add('\uBE70');
		charS.add('\uBE71');
		charS.add('\uBE72');
		charS.add('\uBE73');
		charS.add('\uBE74');
		charS.add('\uBE75');
		charS.add('\uBE76');
		charS.add('\uBE77');
		charS.add('\uBE78');
		charS.add('\uBE79');
		charS.add('\uBE7A');
		charS.add('\uBE7B');
		charS.add('\uBE7C');
		charS.add('\uBE7D');
		charS.add('\uBE7E');
		charS.add('\uBE7F');
		charS.add('\uBE80');
		charS.add('\uBE81');
		charS.add('\uBE82');
		charS.add('\uBE83');
		charS.add('\uBE84');
		charS.add('\uBE85');
		charS.add('\uBE86');
		charS.add('\uBE87');
		charS.add('\uBE88');
		charS.add('\uBE89');
		charS.add('\uBE8A');
		charS.add('\uBE8B');
		charS.add('\uBE8C');
		charS.add('\uBE8D');
		charS.add('\uBE8E');
		charS.add('\uBE8F');
		charS.add('\uBE90');
		charS.add('\uBE91');
		charS.add('\uBE92');
		charS.add('\uBE93');
		charS.add('\uBE94');
		charS.add('\uBE95');
		charS.add('\uBE96');
		charS.add('\uBE97');
		charS.add('\uBE98');
		charS.add('\uBE99');
		charS.add('\uBE9A');
		charS.add('\uBE9B');
		charS.add('\uBE9C');
		charS.add('\uBE9D');
		charS.add('\uBF00');
		charS.add('\uBF01');
		charS.add('\uBF02');
		charS.add('\uBF03');
		charS.add('\uBF04');
		charS.add('\uBF05');
		charS.add('\uBF06');
		charS.add('\uBF07');
		charS.add('\uBF08');
		charS.add('\uBF09');
		charS.add('\uBF0A');
		charS.add('\uBF0B');
		charS.add('\uBF0C');
		charS.add('\uBF0D');
		charS.add('\uBF0E');
		charS.add('\uBF0F');
		charS.add('\uBF10');
		charS.add('\uBF11');
		charS.add('\uBF12');
		charS.add('\uBF13');
		charS.add('\uBF14');
		charS.add('\uBF15');
		charS.add('\uBF16');
		charS.add('\uBF17');
		charS.add('\uBF18');
		charS.add('\uBF19');
		charS.add('\uBF1A');
		charS.add('\uBF1B');
		charS.add('\uBF1C');
		charS.add('\uBF1D');
		charS.add('\uBF1E');
		charS.add('\uBF1F');
		charS.add('\uBF20');
		charS.add('\uBF21');
		charS.add('\uBF22');
		charS.add('\uBF23');
		charS.add('\uBF24');
		charS.add('\uBF25');
		charS.add('\uBF26');
		charS.add('\uBF27');
		charS.add('\uBF28');
		charS.add('\uBF29');
		charS.add('\uBF2A');
		charS.add('\uBF2B');
		charS.add('\uBF2C');
		charS.add('\uBF2D');
		charS.add('\uBF2E');
		charS.add('\uBF2F');
		charS.add('\uBF30');
		charS.add('\uBF31');
		charS.add('\uBF32');
		charS.add('\uBF33');
		charS.add('\uBF34');
		charS.add('\uBF35');
		charS.add('\uBF36');
		charS.add('\uBF37');
		charS.add('\uBF38');
		charS.add('\uBF39');
		charS.add('\uBF3A');
		charS.add('\uBF3B');
		charS.add('\uBF3C');
		charS.add('\uBF3D');
		charS.add('\uBF3E');
		charS.add('\uBF3F');
		charS.add('\uBF40');
		charS.add('\uBF41');
		charS.add('\uBF42');
		charS.add('\uBF43');
		charS.add('\uBF44');
		charS.add('\uBF45');
		charS.add('\uBF46');
		charS.add('\uBF47');
		charS.add('\uBF48');
		charS.add('\uBF49');
		charS.add('\uBF4A');
		charS.add('\uBF4B');
		charS.add('\uBF4C');
		charS.add('\uBF4D');
		charS.add('\uBF4E');
		charS.add('\uBF4F');
		charS.add('\uBF50');
		charS.add('\uBF51');
		charS.add('\uBF52');
		charS.add('\uBF53');
		charS.add('\uBF54');
		charS.add('\uBF55');
		charS.add('\uBF56');
		charS.add('\uBF57');
		charS.add('\uBF58');
		charS.add('\uBF59');
		charS.add('\uBF5A');
		charS.add('\uBF5B');
		charS.add('\uBF5C');
		charS.add('\uBF5D');
		charS.add('\uBF5E');
		charS.add('\uBF5F');
		charS.add('\uBF60');
		charS.add('\uBF61');
		charS.add('\uBF62');
		charS.add('\uBF63');
		charS.add('\uBF64');
		charS.add('\uBF65');
		charS.add('\uBF66');
		charS.add('\uBF67');
		charS.add('\uBF68');
		charS.add('\uBF69');
		charS.add('\uBF6A');
		charS.add('\uBF6B');
		charS.add('\uBF6C');
		charS.add('\uBF6D');
		charS.add('\uBF6E');
		charS.add('\uBF6F');
		charS.add('\uBF70');
		charS.add('\uBF71');
		charS.add('\uBF72');
		charS.add('\uBF73');
		charS.add('\uBF74');
		charS.add('\uBF75');
		charS.add('\uBF76');
		charS.add('\uBF77');
		charS.add('\uBF78');
		charS.add('\uBF79');
		charS.add('\uBF7A');
		charS.add('\uBF7B');
		charS.add('\uBF7C');
		charS.add('\uBF7D');
		charS.add('\uBF7E');
		charS.add('\uBF7F');
		charS.add('\uBF80');
		charS.add('\uBF81');
		charS.add('\uBF82');
		charS.add('\uBF83');
		charS.add('\uBF84');
		charS.add('\uBF85');
		charS.add('\uBF86');
		charS.add('\uBF87');
		charS.add('\uBF88');
		charS.add('\uBF89');
		charS.add('\uBF8A');
		charS.add('\uBF8B');
		charS.add('\uBF8C');
		charS.add('\uBF8D');
		charS.add('\uBF8E');
		charS.add('\uBF8F');
		charS.add('\uBF90');
		charS.add('\uBF91');
		charS.add('\uBF92');
		charS.add('\uBF93');
		charS.add('\uBF94');
		charS.add('\uBF95');
		charS.add('\uBF96');
		charS.add('\uBF97');
		charS.add('\uBF98');
		charS.add('\uBF99');
		charS.add('\uBF9A');
		charS.add('\uBF9B');
		charS.add('\uBF9C');
		charS.add('\uBF9D');
		charS.add('\uBF9E');
		charS.add('\uBF9F');
		charS.add('\uBFA0');
		charS.add('\uBFA1');
		charS.add('\uBFA2');
		charS.add('\uBFA3');
		charS.add('\uBFA4');
		charS.add('\uBFA5');
		charS.add('\uBFA6');
		charS.add('\uBFA7');
		charS.add('\uBFA8');
		charS.add('\uBFA9');
		charS.add('\uBFAA');
		charS.add('\uBFAB');
		charS.add('\uBFAC');
		charS.add('\uBFAD');
		charS.add('\uBFAE');
		charS.add('\uBFAF');
		charS.add('\uBFB0');
		charS.add('\uBFB1');
		charS.add('\uBFB2');
		charS.add('\uBFB3');
		charS.add('\uBFB4');
		charS.add('\uBFB5');
		charS.add('\uBFB6');
		charS.add('\uBFB7');
		charS.add('\uBFB8');
		charS.add('\uBFB9');
		charS.add('\uBFBA');
		charS.add('\uBFBB');
		charS.add('\uBFBC');
		charS.add('\uBF3D');
		charS.add('\uBFBE');
		charS.add('\uBFBF');
		charS.add('\uBFC0');
		charS.add('\uBFC1');
		charS.add('\uBFC2');
		charS.add('\uBFC3');
		charS.add('\uBFC4');
		charS.add('\uBFC5');
		charS.add('\uBFC6');
		charS.add('\uBFC7');
		charS.add('\uBFC8');
		charS.add('\uBFC9');
		charS.add('\uBFCA');
		charS.add('\uBFCB');
		charS.add('\uBFCC');
		charS.add('\uBFCD');
		charS.add('\uBFCE');
		charS.add('\uBFCF');
		charS.add('\uBFD0');
		charS.add('\uBFD1');
		charS.add('\uBFD2');
		charS.add('\uBFD3');
		charS.add('\uBFD4');
		charS.add('\uBFD5');
		charS.add('\uBFD6');
		charS.add('\uBFD7');
		charS.add('\uBFD8');
		charS.add('\uBFD9');
		charS.add('\uBFDA');
		charS.add('\uBFDB');
		charS.add('\uBFDC');
		charS.add('\uBFDD');
		charS.add('\uBFDE');
		charS.add('\uBFDF');
		charS.add('\uBFE0');
		charS.add('\uBFE1');
		charS.add('\uBFE2');
		charS.add('\uBFE3');
		charS.add('\uBFE4');
		charS.add('\uBFE5');
		charS.add('\uBFE6');
		charS.add('\uBFE7');
		charS.add('\uBFE8');
		charS.add('\uBFE9');
		charS.add('\uBFEA');
		charS.add('\uBFEB');
		charS.add('\uBFEC');
		charS.add('\uBFED');
		charS.add('\uBFEE');
		charS.add('\uBFEF');
		charS.add('\uBFF0');
		charS.add('\uBFF1');
		charS.add('\uBFF2');
		charS.add('\uBFF3');
		charS.add('\uBFF4');
		charS.add('\uBFF5');
		charS.add('\uBFF6');
		charS.add('\uBFF7');
		charS.add('\uBFF8');
		charS.add('\uBFF9');
		charS.add('\uBFFA');
		charS.add('\uBFFB');
		charS.add('\uBFFC');
		charS.add('\uBFFD');
		charS.add('\uBFFE');
		charS.add('\uBFFF');
		
		return charS;
	}
	
	public static List<Character> addCharEntryC()
	{
		ArrayList<Character> charS = new ArrayList<Character>();
		
		charS.add('\uC000');
		charS.add('\uC001');
		charS.add('\uC002');
		charS.add('\uC003');
		charS.add('\uC004');
		charS.add('\uC005');
		charS.add('\uC006');
		charS.add('\uC007');
		charS.add('\uC008');
		charS.add('\uC009');
		charS.add('\uC00A');
		charS.add('\uC00B');
		charS.add('\uC00C');
		charS.add('\uC00D');
		charS.add('\uC00E');
		charS.add('\uC00F');
		charS.add('\uC010');
		charS.add('\uC011');
		charS.add('\uC012');
		charS.add('\uC013');
		charS.add('\uC014');
		charS.add('\uC015');
		charS.add('\uC016');
		charS.add('\uC017');
		charS.add('\uC018');
		charS.add('\uC019');
		charS.add('\uC01A');
		charS.add('\uC01B');
		charS.add('\uC01C');
		charS.add('\uC01D');
		charS.add('\uC01E');
		charS.add('\uC01F');
		charS.add('\uC020');
		charS.add('\uC021');
		charS.add('\uC022');
		charS.add('\uC023');
		charS.add('\uC024');
		charS.add('\uC025');
		charS.add('\uC026');
		charS.add('\uC027');
		charS.add('\uC028');
		charS.add('\uC029');
		charS.add('\uC02A');
		charS.add('\uC02B');
		charS.add('\uC02C');
		charS.add('\uC02D');
		charS.add('\uC02E');
		charS.add('\uC02F');
		charS.add('\uC030');
		charS.add('\uC031');
		charS.add('\uC032');
		charS.add('\uC033');
		charS.add('\uC034');
		charS.add('\uC035');
		charS.add('\uC036');
		charS.add('\uC037');
		charS.add('\uC038');
		charS.add('\uC039');
		charS.add('\uC03A');
		charS.add('\uC03B');
		charS.add('\uC03C');
		charS.add('\uC03D');
		charS.add('\uC03E');
		charS.add('\uC03F');
		charS.add('\uC040');
		charS.add('\uC041');
		charS.add('\uC042');
		charS.add('\uC043');
		charS.add('\uC044');
		charS.add('\uC045');
		charS.add('\uC046');
		charS.add('\uC047');
		charS.add('\uC048');
		charS.add('\uC049');
		charS.add('\uC04A');
		charS.add('\uC04B');
		charS.add('\uC04C');
		charS.add('\uC04D');
		charS.add('\uC04E');
		charS.add('\uC04F');
		charS.add('\uC050');
		charS.add('\uC051');
		charS.add('\uC052');
		charS.add('\uC053');
		charS.add('\uC054');
		charS.add('\uC055');
		charS.add('\uC056');
		charS.add('\uC057');
		charS.add('\uC058');
		charS.add('\uC059');
		charS.add('\uC05A');
		charS.add('\uC05B');
		charS.add('\uC05C');
		charS.add('\uC05D');
		charS.add('\uC05E');
		charS.add('\uC05F');
		charS.add('\uC060');
		charS.add('\uC061');
		charS.add('\uC062');
		charS.add('\uC063');
		charS.add('\uC064');
		charS.add('\uC065');
		charS.add('\uC066');
		charS.add('\uC067');
		charS.add('\uC068');
		charS.add('\uC069');
		charS.add('\uC06A');
		charS.add('\uC06B');
		charS.add('\uC06C');
		charS.add('\uC06D');
		charS.add('\uC06E');
		charS.add('\uC06F');
		charS.add('\uC070');
		charS.add('\uC071');
		charS.add('\uC072');
		charS.add('\uC073');
		charS.add('\uC074');
		charS.add('\uC075');
		charS.add('\uC076');
		charS.add('\uC077');
		charS.add('\uC078');
		charS.add('\uC079');
		charS.add('\uC07A');
		charS.add('\uC07B');
		charS.add('\uC07C');
		charS.add('\uC07D');
		charS.add('\uC07E');
		charS.add('\uC07F');
		charS.add('\uC080');
		charS.add('\uC081');
		charS.add('\uC082');
		charS.add('\uC083');
		charS.add('\uC084');
		charS.add('\uC085');
		charS.add('\uC086');
		charS.add('\uC087');
		charS.add('\uC088');
		charS.add('\uC089');
		charS.add('\uC08A');
		charS.add('\uC08B');
		charS.add('\uC08C');
		charS.add('\uC08D');
		charS.add('\uC08E');
		charS.add('\uC08F');
		charS.add('\uC090');
		charS.add('\uC091');
		charS.add('\uC092');
		charS.add('\uC093');
		charS.add('\uC094');
		charS.add('\uC095');
		charS.add('\uC096');
		charS.add('\uC097');
		charS.add('\uC098');
		charS.add('\uC099');
		charS.add('\uC09A');
		charS.add('\uC09B');
		charS.add('\uC09C');
		charS.add('\uC09D');
		charS.add('\uC09E');
		charS.add('\uC09F');
		charS.add('\uC0A0');
		charS.add('\uC0A1');
		charS.add('\uC0A2');
		charS.add('\uC0A3');
		charS.add('\uC0A4');
		charS.add('\uC0A5');
		charS.add('\uC0A6');
		charS.add('\uC0A7');
		charS.add('\uC0A8');
		charS.add('\uC0A9');
		charS.add('\uC0AA');
		charS.add('\uC0AB');
		charS.add('\uC0AC');
		charS.add('\uC0AD');
		charS.add('\uC0AE');
		charS.add('\uC0AF');
		charS.add('\uC0B0');
		charS.add('\uC0B1');
		charS.add('\uC0B2');
		charS.add('\uC0B3');
		charS.add('\uC0B4');
		charS.add('\uC0B5');
		charS.add('\uC0B6');
		charS.add('\uC0B7');
		charS.add('\uC0B8');
		charS.add('\uC0B9');
		charS.add('\uC0BA');
		charS.add('\uC0BB');
		charS.add('\uC0BC');
		charS.add('\uC03D');
		charS.add('\uC0BE');
		charS.add('\uC0BF');
		charS.add('\uC0C0');
		charS.add('\uC0C1');
		charS.add('\uC0C2');
		charS.add('\uC0C3');
		charS.add('\uC0C4');
		charS.add('\uC0C5');
		charS.add('\uC0C6');
		charS.add('\uC0C7');
		charS.add('\uC0C8');
		charS.add('\uC0C9');
		charS.add('\uC0CA');
		charS.add('\uC0CB');
		charS.add('\uC0CC');
		charS.add('\uC0CD');
		charS.add('\uC0CE');
		charS.add('\uC0CF');
		charS.add('\uC0D0');
		charS.add('\uC0D1');
		charS.add('\uC0D2');
		charS.add('\uC0D3');
		charS.add('\uC0D4');
		charS.add('\uC0D5');
		charS.add('\uC0D6');
		charS.add('\uC0D7');
		charS.add('\uC0D8');
		charS.add('\uC0D9');
		charS.add('\uC0DA');
		charS.add('\uC0DB');
		charS.add('\uC0DC');
		charS.add('\uC0DD');
		charS.add('\uC0DE');
		charS.add('\uC0DF');
		charS.add('\uC0E0');
		charS.add('\uC0E1');
		charS.add('\uC0E2');
		charS.add('\uC0E3');
		charS.add('\uC0E4');
		charS.add('\uC0E5');
		charS.add('\uC0E6');
		charS.add('\uC0E7');
		charS.add('\uC0E8');
		charS.add('\uC0E9');
		charS.add('\uC0EA');
		charS.add('\uC0EB');
		charS.add('\uC0EC');
		charS.add('\uC0ED');
		charS.add('\uC0EE');
		charS.add('\uC0EF');
		charS.add('\uC0F0');
		charS.add('\uC0F1');
		charS.add('\uC0F2');
		charS.add('\uC0F3');
		charS.add('\uC0F4');
		charS.add('\uC0F5');
		charS.add('\uC0F6');
		charS.add('\uC0F7');
		charS.add('\uC0F8');
		charS.add('\uC0F9');
		charS.add('\uC0FA');
		charS.add('\uC0FB');
		charS.add('\uC0FC');
		charS.add('\uC0FD');
		charS.add('\uC0FE');
		charS.add('\uC0FF');
		charS.add('\uC0E8');
		charS.add('\uC0E9');
		charS.add('\uC0EA');
		charS.add('\uC0EB');
		charS.add('\uC0EC');
		charS.add('\uC0ED');
		charS.add('\uC0EE');
		charS.add('\uC0EF');
		charS.add('\uC0F0');
		charS.add('\uC0F1');
		charS.add('\uC0F2');
		charS.add('\uC0F3');
		charS.add('\uC0F4');
		charS.add('\uC0F5');
		charS.add('\uC0F6');
		charS.add('\uC0F7');
		charS.add('\uC0F8');
		charS.add('\uC0F9');
		charS.add('\uC0FA');
		charS.add('\uC0FB');
		charS.add('\uC0FC');
		charS.add('\uC0FD');
		charS.add('\uC0FE');
		charS.add('\uC0FF');
		charS.add('\uC100');
		charS.add('\uC101');
		charS.add('\uC102');
		charS.add('\uC103');
		charS.add('\uC104');
		charS.add('\uC105');
		charS.add('\uC106');
		charS.add('\uC107');
		charS.add('\uC108');
		charS.add('\uC109');
		charS.add('\uC10A');
		charS.add('\uC10B');
		charS.add('\uC10C');
		charS.add('\uC10D');
		charS.add('\uC10E');
		charS.add('\uC10F');
		charS.add('\uC110');
		charS.add('\uC111');
		charS.add('\uC112');
		charS.add('\uC113');
		charS.add('\uC114');
		charS.add('\uC115');
		charS.add('\uC116');
		charS.add('\uC117');
		charS.add('\uC118');
		charS.add('\uC119');
		charS.add('\uC11A');
		charS.add('\uC11B');
		charS.add('\uC11C');
		charS.add('\uC11D');
		charS.add('\uC11E');
		charS.add('\uC11F');
		charS.add('\uC120');
		charS.add('\uC121');
		charS.add('\uC122');
		charS.add('\uC123');
		charS.add('\uC124');
		charS.add('\uC125');
		charS.add('\uC126');
		charS.add('\uC127');
		charS.add('\uC128');
		charS.add('\uC129');
		charS.add('\uC12A');
		charS.add('\uC12B');
		charS.add('\uC12C');
		charS.add('\uC12D');
		charS.add('\uC12E');
		charS.add('\uC12F');
		charS.add('\uC130');
		charS.add('\uC131');
		charS.add('\uC132');
		charS.add('\uC133');
		charS.add('\uC134');
		charS.add('\uC135');
		charS.add('\uC136');
		charS.add('\uC137');
		charS.add('\uC138');
		charS.add('\uC139');
		charS.add('\uC13A');
		charS.add('\uC13B');
		charS.add('\uC13C');
		charS.add('\uC13D');
		charS.add('\uC13E');
		charS.add('\uC13F');
		charS.add('\uC140');
		charS.add('\uC141');
		charS.add('\uC142');
		charS.add('\uC143');
		charS.add('\uC144');
		charS.add('\uC145');
		charS.add('\uC146');
		charS.add('\uC147');
		charS.add('\uC148');
		charS.add('\uC149');
		charS.add('\uC14A');
		charS.add('\uC14B');
		charS.add('\uC14C');
		charS.add('\uC14D');
		charS.add('\uC14E');
		charS.add('\uC14F');
		charS.add('\uC150');
		charS.add('\uC151');
		charS.add('\uC152');
		charS.add('\uC153');
		charS.add('\uC154');
		charS.add('\uC155');
		charS.add('\uC156');
		charS.add('\uC157');
		charS.add('\uC158');
		charS.add('\uC159');
		charS.add('\uC15A');
		charS.add('\uC15B');
		charS.add('\uC15C');
		charS.add('\uC15D');
		charS.add('\uC15E');
		charS.add('\uC15F');
		charS.add('\uC160');
		charS.add('\uC161');
		charS.add('\uC162');
		charS.add('\uC163');
		charS.add('\uC164');
		charS.add('\uC165');
		charS.add('\uC166');
		charS.add('\uC167');
		charS.add('\uC168');
		charS.add('\uC169');
		charS.add('\uC16A');
		charS.add('\uC16B');
		charS.add('\uC16C');
		charS.add('\uC16D');
		charS.add('\uC16E');
		charS.add('\uC16F');
		charS.add('\uC170');
		charS.add('\uC171');
		charS.add('\uC172');
		charS.add('\uC173');
		charS.add('\uC174');
		charS.add('\uC175');
		charS.add('\uC176');
		charS.add('\uC177');
		charS.add('\uC178');
		charS.add('\uC179');
		charS.add('\uC17A');
		charS.add('\uC17B');
		charS.add('\uC17C');
		charS.add('\uC17D');
		charS.add('\uC17E');
		charS.add('\uC17F');
		charS.add('\uC180');
		charS.add('\uC181');
		charS.add('\uC182');
		charS.add('\uC183');
		charS.add('\uC184');
		charS.add('\uC185');
		charS.add('\uC186');
		charS.add('\uC187');
		charS.add('\uC188');
		charS.add('\uC189');
		charS.add('\uC18A');
		charS.add('\uC18B');
		charS.add('\uC18C');
		charS.add('\uC18D');
		charS.add('\uC18E');
		charS.add('\uC18F');
		charS.add('\uC190');
		charS.add('\uC191');
		charS.add('\uC192');
		charS.add('\uC193');
		charS.add('\uC194');
		charS.add('\uC195');
		charS.add('\uC196');
		charS.add('\uC197');
		charS.add('\uC198');
		charS.add('\uC199');
		charS.add('\uC19A');
		charS.add('\uC19B');
		charS.add('\uC19C');
		charS.add('\uC19D');
		charS.add('\uC19E');
		charS.add('\uC19F');
		charS.add('\uC1A0');
		charS.add('\uC1A1');
		charS.add('\uC1A2');
		charS.add('\uC1A3');
		charS.add('\uC1A4');
		charS.add('\uC1A5');
		charS.add('\uC1A6');
		charS.add('\uC1A7');
		charS.add('\uC1A8');
		charS.add('\uC1A9');
		charS.add('\uC1AA');
		charS.add('\uC1AB');
		charS.add('\uC1AC');
		charS.add('\uC1AD');
		charS.add('\uC1AE');
		charS.add('\uC1AF');
		charS.add('\uC1B0');
		charS.add('\uC1B1');
		charS.add('\uC1B2');
		charS.add('\uC1B3');
		charS.add('\uC1B4');
		charS.add('\uC1B5');
		charS.add('\uC1B6');
		charS.add('\uC1B7');
		charS.add('\uC1B8');
		charS.add('\uC1B9');
		charS.add('\uC1BA');
		charS.add('\uC1BB');
		charS.add('\uC1BC');
		charS.add('\uC13D');
		charS.add('\uC1BE');
		charS.add('\uC1BF');
		charS.add('\uC1C0');
		charS.add('\uC1C1');
		charS.add('\uC1C2');
		charS.add('\uC1C3');
		charS.add('\uC1C4');
		charS.add('\uC1C5');
		charS.add('\uC1C6');
		charS.add('\uC1C7');
		charS.add('\uC1C8');
		charS.add('\uC1C9');
		charS.add('\uC1CA');
		charS.add('\uC1CB');
		charS.add('\uC1CC');
		charS.add('\uC1CD');
		charS.add('\uC1CE');
		charS.add('\uC1CF');
		charS.add('\uC1D0');
		charS.add('\uC1D1');
		charS.add('\uC1D2');
		charS.add('\uC1D3');
		charS.add('\uC1D4');
		charS.add('\uC1D5');
		charS.add('\uC1D6');
		charS.add('\uC1D7');
		charS.add('\uC1D8');
		charS.add('\uC1D9');
		charS.add('\uC1DA');
		charS.add('\uC1DB');
		charS.add('\uC1DC');
		charS.add('\uC1DD');
		charS.add('\uC1DE');
		charS.add('\uC1DF');
		charS.add('\uC1E0');
		charS.add('\uC1E1');
		charS.add('\uC1E2');
		charS.add('\uC1E3');
		charS.add('\uC1E4');
		charS.add('\uC1E5');
		charS.add('\uC1E6');
		charS.add('\uC1E7');
		charS.add('\uC1E8');
		charS.add('\uC1E9');
		charS.add('\uC1EA');
		charS.add('\uC1EB');
		charS.add('\uC1EC');
		charS.add('\uC1ED');
		charS.add('\uC1EE');
		charS.add('\uC1EF');
		charS.add('\uC1F0');
		charS.add('\uC1F1');
		charS.add('\uC1F2');
		charS.add('\uC1F3');
		charS.add('\uC1F4');
		charS.add('\uC1F5');
		charS.add('\uC1F6');
		charS.add('\uC1F7');
		charS.add('\uC1F8');
		charS.add('\uC1F9');
		charS.add('\uC1FA');
		charS.add('\uC1FB');
		charS.add('\uC1FC');
		charS.add('\uC1FD');
		charS.add('\uC1FE');
		charS.add('\uC1FF');
		charS.add('\uC200');
		charS.add('\uC201');
		charS.add('\uC202');
		charS.add('\uC203');
		charS.add('\uC204');
		charS.add('\uC205');
		charS.add('\uC206');
		charS.add('\uC207');
		charS.add('\uC208');
		charS.add('\uC209');
		charS.add('\uC20A');
		charS.add('\uC20B');
		charS.add('\uC20C');
		charS.add('\uC20D');
		charS.add('\uC20E');
		charS.add('\uC20F');
		charS.add('\uC210');
		charS.add('\uC211');
		charS.add('\uC212');
		charS.add('\uC213');
		charS.add('\uC214');
		charS.add('\uC215');
		charS.add('\uC216');
		charS.add('\uC217');
		charS.add('\uC218');
		charS.add('\uC219');
		charS.add('\uC21A');
		charS.add('\uC21B');
		charS.add('\uC21C');
		charS.add('\uC21D');
		charS.add('\uC21E');
		charS.add('\uC21F');
		charS.add('\uC220');
		charS.add('\uC221');
		charS.add('\uC222');
		charS.add('\uC223');
		charS.add('\uC224');
		charS.add('\uC225');
		charS.add('\uC226');
		charS.add('\uC227');
		charS.add('\uC228');
		charS.add('\uC229');
		charS.add('\uC22A');
		charS.add('\uC22B');
		charS.add('\uC22C');
		charS.add('\uC22D');
		charS.add('\uC22E');
		charS.add('\uC22F');
		charS.add('\uC230');
		charS.add('\uC231');
		charS.add('\uC232');
		charS.add('\uC233');
		charS.add('\uC234');
		charS.add('\uC235');
		charS.add('\uC236');
		charS.add('\uC237');
		charS.add('\uC238');
		charS.add('\uC239');
		charS.add('\uC23A');
		charS.add('\uC23B');
		charS.add('\uC23C');
		charS.add('\uC23D');
		charS.add('\uC23E');
		charS.add('\uC23F');
		charS.add('\uC240');
		charS.add('\uC241');
		charS.add('\uC242');
		charS.add('\uC243');
		charS.add('\uC244');
		charS.add('\uC245');
		charS.add('\uC246');
		charS.add('\uC247');
		charS.add('\uC248');
		charS.add('\uC249');
		charS.add('\uC24A');
		charS.add('\uC24B');
		charS.add('\uC24C');
		charS.add('\uC24D');
		charS.add('\uC24E');
		charS.add('\uC24F');
		charS.add('\uC250');
		charS.add('\uC251');
		charS.add('\uC252');
		charS.add('\uC253');
		charS.add('\uC254');
		charS.add('\uC255');
		charS.add('\uC256');
		charS.add('\uC257');
		charS.add('\uC258');
		charS.add('\uC259');
		charS.add('\uC25A');
		charS.add('\uC25B');
		charS.add('\uC25C');
		charS.add('\uC25D');
		charS.add('\uC25E');
		charS.add('\uC25F');
		charS.add('\uC260');
		charS.add('\uC261');
		charS.add('\uC262');
		charS.add('\uC263');
		charS.add('\uC264');
		charS.add('\uC265');
		charS.add('\uC266');
		charS.add('\uC267');
		charS.add('\uC268');
		charS.add('\uC269');
		charS.add('\uC26A');
		charS.add('\uC26B');
		charS.add('\uC26C');
		charS.add('\uC26D');
		charS.add('\uC26E');
		charS.add('\uC26F');
		charS.add('\uC270');
		charS.add('\uC271');
		charS.add('\uC272');
		charS.add('\uC273');
		charS.add('\uC274');
		charS.add('\uC275');
		charS.add('\uC276');
		charS.add('\uC277');
		charS.add('\uC278');
		charS.add('\uC279');
		charS.add('\uC27A');
		charS.add('\uC27B');
		charS.add('\uC27C');
		charS.add('\uC27D');
		charS.add('\uC27E');
		charS.add('\uC27F');
		charS.add('\uC280');
		charS.add('\uC281');
		charS.add('\uC282');
		charS.add('\uC283');
		charS.add('\uC284');
		charS.add('\uC285');
		charS.add('\uC286');
		charS.add('\uC287');
		charS.add('\uC288');
		charS.add('\uC289');
		charS.add('\uC28A');
		charS.add('\uC28B');
		charS.add('\uC28C');
		charS.add('\uC28D');
		charS.add('\uC28E');
		charS.add('\uC28F');
		charS.add('\uC290');
		charS.add('\uC291');
		charS.add('\uC292');
		charS.add('\uC293');
		charS.add('\uC294');
		charS.add('\uC295');
		charS.add('\uC296');
		charS.add('\uC297');
		charS.add('\uC298');
		charS.add('\uC299');
		charS.add('\uC29A');
		charS.add('\uC29B');
		charS.add('\uC29C');
		charS.add('\uC29D');
		charS.add('\uC29E');
		charS.add('\uC29F');
		charS.add('\uC2A0');
		charS.add('\uC2A1');
		charS.add('\uC2A2');
		charS.add('\uC2A3');
		charS.add('\uC2A4');
		charS.add('\uC2A5');
		charS.add('\uC2A6');
		charS.add('\uC2A7');
		charS.add('\uC2A8');
		charS.add('\uC2A9');
		charS.add('\uC2AA');
		charS.add('\uC2AB');
		charS.add('\uC2AC');
		charS.add('\uC2AD');
		charS.add('\uC2AE');
		charS.add('\uC2AF');
		charS.add('\uC2B0');
		charS.add('\uC2B1');
		charS.add('\uC2B2');
		charS.add('\uC2B3');
		charS.add('\uC2B4');
		charS.add('\uC2B5');
		charS.add('\uC2B6');
		charS.add('\uC2B7');
		charS.add('\uC2B8');
		charS.add('\uC2B9');
		charS.add('\uC2BA');
		charS.add('\uC2BB');
		charS.add('\uC2BC');
		charS.add('\uC23D');
		charS.add('\uC2BE');
		charS.add('\uC2BF');
		charS.add('\uC2C0');
		charS.add('\uC2C1');
		charS.add('\uC2C2');
		charS.add('\uC2C3');
		charS.add('\uC2C4');
		charS.add('\uC2C5');
		charS.add('\uC2C6');
		charS.add('\uC2C7');
		charS.add('\uC2C8');
		charS.add('\uC2C9');
		charS.add('\uC2CA');
		charS.add('\uC2CB');
		charS.add('\uC2CC');
		charS.add('\uC2CD');
		charS.add('\uC2CE');
		charS.add('\uC2CF');
		charS.add('\uC2D0');
		charS.add('\uC2D1');
		charS.add('\uC2D2');
		charS.add('\uC2D3');
		charS.add('\uC2D4');
		charS.add('\uC2D5');
		charS.add('\uC2D6');
		charS.add('\uC2D7');
		charS.add('\uC2D8');
		charS.add('\uC2D9');
		charS.add('\uC2DA');
		charS.add('\uC2DB');
		charS.add('\uC2DC');
		charS.add('\uC2DD');
		charS.add('\uC2DE');
		charS.add('\uC2DF');
		charS.add('\uC2E0');
		charS.add('\uC2E1');
		charS.add('\uC2E2');
		charS.add('\uC2E3');
		charS.add('\uC2E4');
		charS.add('\uC2E5');
		charS.add('\uC2E6');
		charS.add('\uC2E7');
		charS.add('\uC2E8');
		charS.add('\uC2E9');
		charS.add('\uC2EA');
		charS.add('\uC2EB');
		charS.add('\uC2EC');
		charS.add('\uC2ED');
		charS.add('\uC2EE');
		charS.add('\uC2EF');
		charS.add('\uC2F0');
		charS.add('\uC2F1');
		charS.add('\uC2F2');
		charS.add('\uC2F3');
		charS.add('\uC2F4');
		charS.add('\uC2F5');
		charS.add('\uC2F6');
		charS.add('\uC2F7');
		charS.add('\uC2F8');
		charS.add('\uC2F9');
		charS.add('\uC2FA');
		charS.add('\uC2FB');
		charS.add('\uC2FC');
		charS.add('\uC2FD');
		charS.add('\uC2FE');
		charS.add('\uC2FF');
		charS.add('\uC300');
		charS.add('\uC301');
		charS.add('\uC302');
		charS.add('\uC303');
		charS.add('\uC304');
		charS.add('\uC305');
		charS.add('\uC306');
		charS.add('\uC307');
		charS.add('\uC308');
		charS.add('\uC309');
		charS.add('\uC30A');
		charS.add('\uC30B');
		charS.add('\uC30C');
		charS.add('\uC30D');
		charS.add('\uC30E');
		charS.add('\uC30F');
		charS.add('\uC310');
		charS.add('\uC311');
		charS.add('\uC312');
		charS.add('\uC313');
		charS.add('\uC314');
		charS.add('\uC315');
		charS.add('\uC316');
		charS.add('\uC317');
		charS.add('\uC318');
		charS.add('\uC319');
		charS.add('\uC31A');
		charS.add('\uC31B');
		charS.add('\uC31C');
		charS.add('\uC31D');
		charS.add('\uC31E');
		charS.add('\uC31F');
		charS.add('\uC320');
		charS.add('\uC321');
		charS.add('\uC322');
		charS.add('\uC323');
		charS.add('\uC324');
		charS.add('\uC325');
		charS.add('\uC326');
		charS.add('\uC327');
		charS.add('\uC328');
		charS.add('\uC329');
		charS.add('\uC32A');
		charS.add('\uC32B');
		charS.add('\uC32C');
		charS.add('\uC32D');
		charS.add('\uC32E');
		charS.add('\uC32F');
		charS.add('\uC330');
		charS.add('\uC331');
		charS.add('\uC332');
		charS.add('\uC333');
		charS.add('\uC334');
		charS.add('\uC335');
		charS.add('\uC336');
		charS.add('\uC337');
		charS.add('\uC338');
		charS.add('\uC339');
		charS.add('\uC33A');
		charS.add('\uC33B');
		charS.add('\uC33C');
		charS.add('\uC33D');
		charS.add('\uC33E');
		charS.add('\uC33F');
		charS.add('\uC340');
		charS.add('\uC341');
		charS.add('\uC342');
		charS.add('\uC343');
		charS.add('\uC344');
		charS.add('\uC345');
		charS.add('\uC346');
		charS.add('\uC347');
		charS.add('\uC348');
		charS.add('\uC349');
		charS.add('\uC34A');
		charS.add('\uC34B');
		charS.add('\uC34C');
		charS.add('\uC34D');
		charS.add('\uC34E');
		charS.add('\uC34F');
		charS.add('\uC350');
		charS.add('\uC351');
		charS.add('\uC352');
		charS.add('\uC353');
		charS.add('\uC354');
		charS.add('\uC355');
		charS.add('\uC356');
		charS.add('\uC357');
		charS.add('\uC358');
		charS.add('\uC359');
		charS.add('\uC35A');
		charS.add('\uC35B');
		charS.add('\uC35C');
		charS.add('\uC35D');
		charS.add('\uC35E');
		charS.add('\uC35F');
		charS.add('\uC360');
		charS.add('\uC361');
		charS.add('\uC362');
		charS.add('\uC363');
		charS.add('\uC364');
		charS.add('\uC365');
		charS.add('\uC366');
		charS.add('\uC367');
		charS.add('\uC368');
		charS.add('\uC369');
		charS.add('\uC36A');
		charS.add('\uC36B');
		charS.add('\uC36C');
		charS.add('\uC36D');
		charS.add('\uC36E');
		charS.add('\uC36F');
		charS.add('\uC370');
		charS.add('\uC371');
		charS.add('\uC372');
		charS.add('\uC373');
		charS.add('\uC374');
		charS.add('\uC375');
		charS.add('\uC376');
		charS.add('\uC377');
		charS.add('\uC378');
		charS.add('\uC379');
		charS.add('\uC37A');
		charS.add('\uC37B');
		charS.add('\uC37C');
		charS.add('\uC37D');
		charS.add('\uC37E');
		charS.add('\uC37F');
		charS.add('\uC380');
		charS.add('\uC381');
		charS.add('\uC382');
		charS.add('\uC383');
		charS.add('\uC384');
		charS.add('\uC385');
		charS.add('\uC386');
		charS.add('\uC387');
		charS.add('\uC388');
		charS.add('\uC389');
		charS.add('\uC38A');
		charS.add('\uC38B');
		charS.add('\uC38C');
		charS.add('\uC38D');
		charS.add('\uC38E');
		charS.add('\uC38F');
		charS.add('\uC390');
		charS.add('\uC391');
		charS.add('\uC392');
		charS.add('\uC393');
		charS.add('\uC394');
		charS.add('\uC395');
		charS.add('\uC396');
		charS.add('\uC397');
		charS.add('\uC398');
		charS.add('\uC399');
		charS.add('\uC39A');
		charS.add('\uC39B');
		charS.add('\uC39C');
		charS.add('\uC39D');
		charS.add('\uC39E');
		charS.add('\uC39F');
		charS.add('\uC3A0');
		charS.add('\uC3A1');
		charS.add('\uC3A2');
		charS.add('\uC3A3');
		charS.add('\uC3A4');
		charS.add('\uC3A5');
		charS.add('\uC3A6');
		charS.add('\uC3A7');
		charS.add('\uC3A8');
		charS.add('\uC3A9');
		charS.add('\uC3AA');
		charS.add('\uC3AB');
		charS.add('\uC3AC');
		charS.add('\uC3AD');
		charS.add('\uC3AE');
		charS.add('\uC3AF');
		charS.add('\uC3B0');
		charS.add('\uC3B1');
		charS.add('\uC3B2');
		charS.add('\uC3B3');
		charS.add('\uC3B4');
		charS.add('\uC3B5');
		charS.add('\uC3B6');
		charS.add('\uC3B7');
		charS.add('\uC3B8');
		charS.add('\uC3B9');
		charS.add('\uC3BA');
		charS.add('\uC3BB');
		charS.add('\uC3BC');
		charS.add('\uC33D');
		charS.add('\uC3BE');
		charS.add('\uC3BF');
		charS.add('\uC3C0');
		charS.add('\uC3C1');
		charS.add('\uC3C2');
		charS.add('\uC3C3');
		charS.add('\uC3C4');
		charS.add('\uC3C5');
		charS.add('\uC3C6');
		charS.add('\uC3C7');
		charS.add('\uC3C8');
		charS.add('\uC3C9');
		charS.add('\uC3CA');
		charS.add('\uC3CB');
		charS.add('\uC3CC');
		charS.add('\uC3CD');
		charS.add('\uC3CE');
		charS.add('\uC3CF');
		charS.add('\uC3D0');
		charS.add('\uC3D1');
		charS.add('\uC3D2');
		charS.add('\uC3D3');
		charS.add('\uC3D4');
		charS.add('\uC3D5');
		charS.add('\uC3D6');
		charS.add('\uC3D7');
		charS.add('\uC3D8');
		charS.add('\uC3D9');
		charS.add('\uC3DA');
		charS.add('\uC3DB');
		charS.add('\uC3DC');
		charS.add('\uC3DD');
		charS.add('\uC3DE');
		charS.add('\uC3DF');
		charS.add('\uC3E0');
		charS.add('\uC3E1');
		charS.add('\uC3E2');
		charS.add('\uC3E3');
		charS.add('\uC3E4');
		charS.add('\uC3E5');
		charS.add('\uC3E6');
		charS.add('\uC3E7');
		charS.add('\uC3E8');
		charS.add('\uC3E9');
		charS.add('\uC3EA');
		charS.add('\uC3EB');
		charS.add('\uC3EC');
		charS.add('\uC3ED');
		charS.add('\uC3EE');
		charS.add('\uC3EF');
		charS.add('\uC3F0');
		charS.add('\uC3F1');
		charS.add('\uC3F2');
		charS.add('\uC3F3');
		charS.add('\uC3F4');
		charS.add('\uC3F5');
		charS.add('\uC3F6');
		charS.add('\uC3F7');
		charS.add('\uC3F8');
		charS.add('\uC3F9');
		charS.add('\uC3FA');
		charS.add('\uC3FB');
		charS.add('\uC3FC');
		charS.add('\uC3FD');
		charS.add('\uC3FE');
		charS.add('\uC3FF');
		charS.add('\uC400');
		charS.add('\uC401');
		charS.add('\uC402');
		charS.add('\uC403');
		charS.add('\uC404');
		charS.add('\uC405');
		charS.add('\uC406');
		charS.add('\uC407');
		charS.add('\uC408');
		charS.add('\uC409');
		charS.add('\uC40A');
		charS.add('\uC40B');
		charS.add('\uC40C');
		charS.add('\uC40D');
		charS.add('\uC40E');
		charS.add('\uC40F');
		charS.add('\uC410');
		charS.add('\uC411');
		charS.add('\uC412');
		charS.add('\uC413');
		charS.add('\uC414');
		charS.add('\uC415');
		charS.add('\uC416');
		charS.add('\uC417');
		charS.add('\uC418');
		charS.add('\uC419');
		charS.add('\uC41A');
		charS.add('\uC41B');
		charS.add('\uC41C');
		charS.add('\uC41D');
		charS.add('\uC41E');
		charS.add('\uC41F');
		charS.add('\uC420');
		charS.add('\uC421');
		charS.add('\uC422');
		charS.add('\uC423');
		charS.add('\uC424');
		charS.add('\uC425');
		charS.add('\uC426');
		charS.add('\uC427');
		charS.add('\uC428');
		charS.add('\uC429');
		charS.add('\uC42A');
		charS.add('\uC42B');
		charS.add('\uC42C');
		charS.add('\uC42D');
		charS.add('\uC42E');
		charS.add('\uC42F');
		charS.add('\uC430');
		charS.add('\uC431');
		charS.add('\uC432');
		charS.add('\uC433');
		charS.add('\uC434');
		charS.add('\uC435');
		charS.add('\uC436');
		charS.add('\uC437');
		charS.add('\uC438');
		charS.add('\uC439');
		charS.add('\uC43A');
		charS.add('\uC43B');
		charS.add('\uC43C');
		charS.add('\uC43D');
		charS.add('\uC43E');
		charS.add('\uC43F');
		charS.add('\uC440');
		charS.add('\uC441');
		charS.add('\uC442');
		charS.add('\uC443');
		charS.add('\uC444');
		charS.add('\uC445');
		charS.add('\uC446');
		charS.add('\uC447');
		charS.add('\uC448');
		charS.add('\uC449');
		charS.add('\uC44A');
		charS.add('\uC44B');
		charS.add('\uC44C');
		charS.add('\uC44D');
		charS.add('\uC44E');
		charS.add('\uC44F');
		charS.add('\uC450');
		charS.add('\uC451');
		charS.add('\uC452');
		charS.add('\uC453');
		charS.add('\uC454');
		charS.add('\uC455');
		charS.add('\uC456');
		charS.add('\uC457');
		charS.add('\uC458');
		charS.add('\uC459');
		charS.add('\uC45A');
		charS.add('\uC45B');
		charS.add('\uC45C');
		charS.add('\uC45D');
		charS.add('\uC45E');
		charS.add('\uC45F');
		charS.add('\uC460');
		charS.add('\uC461');
		charS.add('\uC462');
		charS.add('\uC463');
		charS.add('\uC464');
		charS.add('\uC465');
		charS.add('\uC466');
		charS.add('\uC467');
		charS.add('\uC468');
		charS.add('\uC469');
		charS.add('\uC46A');
		charS.add('\uC46B');
		charS.add('\uC46C');
		charS.add('\uC46D');
		charS.add('\uC46E');
		charS.add('\uC46F');
		charS.add('\uC470');
		charS.add('\uC471');
		charS.add('\uC472');
		charS.add('\uC473');
		charS.add('\uC474');
		charS.add('\uC475');
		charS.add('\uC476');
		charS.add('\uC477');
		charS.add('\uC478');
		charS.add('\uC479');
		charS.add('\uC47A');
		charS.add('\uC47B');
		charS.add('\uC47C');
		charS.add('\uC47D');
		charS.add('\uC47E');
		charS.add('\uC47F');
		charS.add('\uC480');
		charS.add('\uC481');
		charS.add('\uC482');
		charS.add('\uC483');
		charS.add('\uC484');
		charS.add('\uC485');
		charS.add('\uC486');
		charS.add('\uC487');
		charS.add('\uC488');
		charS.add('\uC489');
		charS.add('\uC48A');
		charS.add('\uC48B');
		charS.add('\uC48C');
		charS.add('\uC48D');
		charS.add('\uC48E');
		charS.add('\uC48F');
		charS.add('\uC490');
		charS.add('\uC491');
		charS.add('\uC492');
		charS.add('\uC493');
		charS.add('\uC494');
		charS.add('\uC495');
		charS.add('\uC496');
		charS.add('\uC497');
		charS.add('\uC498');
		charS.add('\uC499');
		charS.add('\uC49A');
		charS.add('\uC49B');
		charS.add('\uC49C');
		charS.add('\uC49D');
		charS.add('\uC49E');
		charS.add('\uC49F');
		charS.add('\uC4A0');
		charS.add('\uC4A1');
		charS.add('\uC4A2');
		charS.add('\uC4A3');
		charS.add('\uC4A4');
		charS.add('\uC4A5');
		charS.add('\uC4A6');
		charS.add('\uC4A7');
		charS.add('\uC4A8');
		charS.add('\uC4A9');
		charS.add('\uC4AA');
		charS.add('\uC4AB');
		charS.add('\uC4AC');
		charS.add('\uC4AD');
		charS.add('\uC4AE');
		charS.add('\uC4AF');
		charS.add('\uC4B0');
		charS.add('\uC4B1');
		charS.add('\uC4B2');
		charS.add('\uC4B3');
		charS.add('\uC4B4');
		charS.add('\uC4B5');
		charS.add('\uC4B6');
		charS.add('\uC4B7');
		charS.add('\uC4B8');
		charS.add('\uC4B9');
		charS.add('\uC4BA');
		charS.add('\uC4BB');
		charS.add('\uC4BC');
		charS.add('\uC43D');
		charS.add('\uC4BE');
		charS.add('\uC4BF');
		charS.add('\uC4C0');
		charS.add('\uC4C1');
		charS.add('\uC4C2');
		charS.add('\uC4C3');
		charS.add('\uC4C4');
		charS.add('\uC4C5');
		charS.add('\uC4C6');
		charS.add('\uC4C7');
		charS.add('\uC4C8');
		charS.add('\uC4C9');
		charS.add('\uC4CA');
		charS.add('\uC4CB');
		charS.add('\uC4CC');
		charS.add('\uC4CD');
		charS.add('\uC4CE');
		charS.add('\uC4CF');
		charS.add('\uC4D0');
		charS.add('\uC4D1');
		charS.add('\uC4D2');
		charS.add('\uC4D3');
		charS.add('\uC4D4');
		charS.add('\uC4D5');
		charS.add('\uC4D6');
		charS.add('\uC4D7');
		charS.add('\uC4D8');
		charS.add('\uC4D9');
		charS.add('\uC4DA');
		charS.add('\uC4DB');
		charS.add('\uC4DC');
		charS.add('\uC4DD');
		charS.add('\uC4DE');
		charS.add('\uC4DF');
		charS.add('\uC4E0');
		charS.add('\uC4E1');
		charS.add('\uC4E2');
		charS.add('\uC4E3');
		charS.add('\uC4E4');
		charS.add('\uC4E5');
		charS.add('\uC4E6');
		charS.add('\uC4E7');
		charS.add('\uC4E8');
		charS.add('\uC4E9');
		charS.add('\uC4EA');
		charS.add('\uC4EB');
		charS.add('\uC4EC');
		charS.add('\uC4ED');
		charS.add('\uC4EE');
		charS.add('\uC4EF');
		charS.add('\uC4F0');
		charS.add('\uC4F1');
		charS.add('\uC4F2');
		charS.add('\uC4F3');
		charS.add('\uC4F4');
		charS.add('\uC4F5');
		charS.add('\uC4F6');
		charS.add('\uC4F7');
		charS.add('\uC4F8');
		charS.add('\uC4F9');
		charS.add('\uC4FA');
		charS.add('\uC4FB');
		charS.add('\uC4FC');
		charS.add('\uC4FD');
		charS.add('\uC4FE');
		charS.add('\uC4FF');
		charS.add('\uC500');
		charS.add('\uC501');
		charS.add('\uC502');
		charS.add('\uC503');
		charS.add('\uC504');
		charS.add('\uC505');
		charS.add('\uC506');
		charS.add('\uC507');
		charS.add('\uC508');
		charS.add('\uC509');
		charS.add('\uC50A');
		charS.add('\uC50B');
		charS.add('\uC50C');
		charS.add('\uC50D');
		charS.add('\uC50E');
		charS.add('\uC50F');
		charS.add('\uC510');
		charS.add('\uC511');
		charS.add('\uC512');
		charS.add('\uC513');
		charS.add('\uC514');
		charS.add('\uC515');
		charS.add('\uC516');
		charS.add('\uC517');
		charS.add('\uC518');
		charS.add('\uC519');
		charS.add('\uC51A');
		charS.add('\uC51B');
		charS.add('\uC51C');
		charS.add('\uC51D');
		charS.add('\uC51E');
		charS.add('\uC51F');
		charS.add('\uC520');
		charS.add('\uC521');
		charS.add('\uC522');
		charS.add('\uC523');
		charS.add('\uC524');
		charS.add('\uC525');
		charS.add('\uC526');
		charS.add('\uC527');
		charS.add('\uC528');
		charS.add('\uC529');
		charS.add('\uC52A');
		charS.add('\uC52B');
		charS.add('\uC52C');
		charS.add('\uC52D');
		charS.add('\uC52E');
		charS.add('\uC52F');
		charS.add('\uC530');
		charS.add('\uC531');
		charS.add('\uC532');
		charS.add('\uC533');
		charS.add('\uC534');
		charS.add('\uC535');
		charS.add('\uC536');
		charS.add('\uC537');
		charS.add('\uC538');
		charS.add('\uC539');
		charS.add('\uC53A');
		charS.add('\uC53B');
		charS.add('\uC53C');
		charS.add('\uC53D');
		charS.add('\uC53E');
		charS.add('\uC53F');
		charS.add('\uC540');
		charS.add('\uC541');
		charS.add('\uC542');
		charS.add('\uC543');
		charS.add('\uC544');
		charS.add('\uC545');
		charS.add('\uC546');
		charS.add('\uC547');
		charS.add('\uC548');
		charS.add('\uC549');
		charS.add('\uC54A');
		charS.add('\uC54B');
		charS.add('\uC54C');
		charS.add('\uC54D');
		charS.add('\uC54E');
		charS.add('\uC54F');
		charS.add('\uC550');
		charS.add('\uC551');
		charS.add('\uC552');
		charS.add('\uC553');
		charS.add('\uC554');
		charS.add('\uC555');
		charS.add('\uC556');
		charS.add('\uC557');
		charS.add('\uC558');
		charS.add('\uC559');
		charS.add('\uC55A');
		charS.add('\uC55B');
		charS.add('\uC55C');
		charS.add('\uC55D');
		charS.add('\uC55E');
		charS.add('\uC55F');
		charS.add('\uC560');
		charS.add('\uC561');
		charS.add('\uC562');
		charS.add('\uC563');
		charS.add('\uC564');
		charS.add('\uC565');
		charS.add('\uC566');
		charS.add('\uC567');
		charS.add('\uC568');
		charS.add('\uC569');
		charS.add('\uC56A');
		charS.add('\uC56B');
		charS.add('\uC56C');
		charS.add('\uC56D');
		charS.add('\uC56E');
		charS.add('\uC56F');
		charS.add('\uC570');
		charS.add('\uC571');
		charS.add('\uC572');
		charS.add('\uC573');
		charS.add('\uC574');
		charS.add('\uC575');
		charS.add('\uC576');
		charS.add('\uC577');
		charS.add('\uC578');
		charS.add('\uC579');
		charS.add('\uC57A');
		charS.add('\uC57B');
		charS.add('\uC57C');
		charS.add('\uC57D');
		charS.add('\uC57E');
		charS.add('\uC57F');
		charS.add('\uC580');
		charS.add('\uC581');
		charS.add('\uC582');
		charS.add('\uC583');
		charS.add('\uC584');
		charS.add('\uC585');
		charS.add('\uC586');
		charS.add('\uC587');
		charS.add('\uC588');
		charS.add('\uC589');
		charS.add('\uC58A');
		charS.add('\uC58B');
		charS.add('\uC58C');
		charS.add('\uC58D');
		charS.add('\uC58E');
		charS.add('\uC58F');
		charS.add('\uC590');
		charS.add('\uC591');
		charS.add('\uC592');
		charS.add('\uC593');
		charS.add('\uC594');
		charS.add('\uC595');
		charS.add('\uC596');
		charS.add('\uC597');
		charS.add('\uC598');
		charS.add('\uC599');
		charS.add('\uC59A');
		charS.add('\uC59B');
		charS.add('\uC59C');
		charS.add('\uC59D');
		charS.add('\uC59E');
		charS.add('\uC59F');
		charS.add('\uC5A0');
		charS.add('\uC5A1');
		charS.add('\uC5A2');
		charS.add('\uC5A3');
		charS.add('\uC5A4');
		charS.add('\uC5A5');
		charS.add('\uC5A6');
		charS.add('\uC5A7');
		charS.add('\uC5A8');
		charS.add('\uC5A9');
		charS.add('\uC5AA');
		charS.add('\uC5AB');
		charS.add('\uC5AC');
		charS.add('\uC5AD');
		charS.add('\uC5AE');
		charS.add('\uC5AF');
		charS.add('\uC5B0');
		charS.add('\uC5B1');
		charS.add('\uC5B2');
		charS.add('\uC5B3');
		charS.add('\uC5B4');
		charS.add('\uC5B5');
		charS.add('\uC5B6');
		charS.add('\uC5B7');
		charS.add('\uC5B8');
		charS.add('\uC5B9');
		charS.add('\uC5BA');
		charS.add('\uC5BB');
		charS.add('\uC5BC');
		charS.add('\uC53D');
		charS.add('\uC5BE');
		charS.add('\uC5BF');
		charS.add('\uC5C0');
		charS.add('\uC5C1');
		charS.add('\uC5C2');
		charS.add('\uC5C3');
		charS.add('\uC5C4');
		charS.add('\uC5C5');
		charS.add('\uC5C6');
		charS.add('\uC5C7');
		charS.add('\uC5C8');
		charS.add('\uC5C9');
		charS.add('\uC5CA');
		charS.add('\uC5CB');
		charS.add('\uC5CC');
		charS.add('\uC5CD');
		charS.add('\uC5CE');
		charS.add('\uC5CF');
		charS.add('\uC5D0');
		charS.add('\uC5D1');
		charS.add('\uC5D2');
		charS.add('\uC5D3');
		charS.add('\uC5D4');
		charS.add('\uC5D5');
		charS.add('\uC5D6');
		charS.add('\uC5D7');
		charS.add('\uC5D8');
		charS.add('\uC5D9');
		charS.add('\uC5DA');
		charS.add('\uC5DB');
		charS.add('\uC5DC');
		charS.add('\uC5DD');
		charS.add('\uC5DE');
		charS.add('\uC5DF');
		charS.add('\uC5E0');
		charS.add('\uC5E1');
		charS.add('\uC5E2');
		charS.add('\uC5E3');
		charS.add('\uC5E4');
		charS.add('\uC5E5');
		charS.add('\uC5E6');
		charS.add('\uC5E7');
		charS.add('\uC5E8');
		charS.add('\uC5E9');
		charS.add('\uC5EA');
		charS.add('\uC5EB');
		charS.add('\uC5EC');
		charS.add('\uC5ED');
		charS.add('\uC5EE');
		charS.add('\uC5EF');
		charS.add('\uC5F0');
		charS.add('\uC5F1');
		charS.add('\uC5F2');
		charS.add('\uC5F3');
		charS.add('\uC5F4');
		charS.add('\uC5F5');
		charS.add('\uC5F6');
		charS.add('\uC5F7');
		charS.add('\uC5F8');
		charS.add('\uC5F9');
		charS.add('\uC5FA');
		charS.add('\uC5FB');
		charS.add('\uC5FC');
		charS.add('\uC5FD');
		charS.add('\uC5FE');
		charS.add('\uC5FF');
		charS.add('\uC600');
		charS.add('\uC601');
		charS.add('\uC602');
		charS.add('\uC603');
		charS.add('\uC604');
		charS.add('\uC605');
		charS.add('\uC606');
		charS.add('\uC607');
		charS.add('\uC608');
		charS.add('\uC609');
		charS.add('\uC60A');
		charS.add('\uC60B');
		charS.add('\uC60C');
		charS.add('\uC60D');
		charS.add('\uC60E');
		charS.add('\uC60F');
		charS.add('\uC610');
		charS.add('\uC611');
		charS.add('\uC612');
		charS.add('\uC613');
		charS.add('\uC614');
		charS.add('\uC615');
		charS.add('\uC616');
		charS.add('\uC617');
		charS.add('\uC618');
		charS.add('\uC619');
		charS.add('\uC61A');
		charS.add('\uC61B');
		charS.add('\uC61C');
		charS.add('\uC61D');
		charS.add('\uC61E');
		charS.add('\uC61F');
		charS.add('\uC620');
		charS.add('\uC621');
		charS.add('\uC622');
		charS.add('\uC623');
		charS.add('\uC624');
		charS.add('\uC625');
		charS.add('\uC626');
		charS.add('\uC627');
		charS.add('\uC628');
		charS.add('\uC629');
		charS.add('\uC62A');
		charS.add('\uC62B');
		charS.add('\uC62C');
		charS.add('\uC62D');
		charS.add('\uC62E');
		charS.add('\uC62F');
		charS.add('\uC630');
		charS.add('\uC631');
		charS.add('\uC632');
		charS.add('\uC633');
		charS.add('\uC634');
		charS.add('\uC635');
		charS.add('\uC636');
		charS.add('\uC637');
		charS.add('\uC638');
		charS.add('\uC639');
		charS.add('\uC63A');
		charS.add('\uC63B');
		charS.add('\uC63C');
		charS.add('\uC63D');
		charS.add('\uC63E');
		charS.add('\uC63F');
		charS.add('\uC640');
		charS.add('\uC641');
		charS.add('\uC642');
		charS.add('\uC643');
		charS.add('\uC644');
		charS.add('\uC645');
		charS.add('\uC646');
		charS.add('\uC647');
		charS.add('\uC648');
		charS.add('\uC649');
		charS.add('\uC64A');
		charS.add('\uC64B');
		charS.add('\uC64C');
		charS.add('\uC64D');
		charS.add('\uC64E');
		charS.add('\uC64F');
		charS.add('\uC650');
		charS.add('\uC651');
		charS.add('\uC652');
		charS.add('\uC653');
		charS.add('\uC654');
		charS.add('\uC655');
		charS.add('\uC656');
		charS.add('\uC657');
		charS.add('\uC658');
		charS.add('\uC659');
		charS.add('\uC65A');
		charS.add('\uC65B');
		charS.add('\uC65C');
		charS.add('\uC65D');
		charS.add('\uC65E');
		charS.add('\uC65F');
		charS.add('\uC660');
		charS.add('\uC661');
		charS.add('\uC662');
		charS.add('\uC663');
		charS.add('\uC664');
		charS.add('\uC665');
		charS.add('\uC666');
		charS.add('\uC667');
		charS.add('\uC668');
		charS.add('\uC669');
		charS.add('\uC66A');
		charS.add('\uC66B');
		charS.add('\uC66C');
		charS.add('\uC66D');
		charS.add('\uC66E');
		charS.add('\uC66F');
		charS.add('\uC670');
		charS.add('\uC671');
		charS.add('\uC672');
		charS.add('\uC673');
		charS.add('\uC674');
		charS.add('\uC675');
		charS.add('\uC676');
		charS.add('\uC677');
		charS.add('\uC678');
		charS.add('\uC679');
		charS.add('\uC67A');
		charS.add('\uC67B');
		charS.add('\uC67C');
		charS.add('\uC67D');
		charS.add('\uC67E');
		charS.add('\uC67F');
		charS.add('\uC680');
		charS.add('\uC681');
		charS.add('\uC682');
		charS.add('\uC683');
		charS.add('\uC684');
		charS.add('\uC685');
		charS.add('\uC686');
		charS.add('\uC687');
		charS.add('\uC688');
		charS.add('\uC689');
		charS.add('\uC68A');
		charS.add('\uC68B');
		charS.add('\uC68C');
		charS.add('\uC68D');
		charS.add('\uC68E');
		charS.add('\uC68F');
		charS.add('\uC690');
		charS.add('\uC691');
		charS.add('\uC692');
		charS.add('\uC693');
		charS.add('\uC694');
		charS.add('\uC695');
		charS.add('\uC696');
		charS.add('\uC697');
		charS.add('\uC698');
		charS.add('\uC699');
		charS.add('\uC69A');
		charS.add('\uC69B');
		charS.add('\uC69C');
		charS.add('\uC69D');
		charS.add('\uC69E');
		charS.add('\uC69F');
		charS.add('\uC6A0');
		charS.add('\uC6A1');
		charS.add('\uC6A2');
		charS.add('\uC6A3');
		charS.add('\uC6A4');
		charS.add('\uC6A5');
		charS.add('\uC6A6');
		charS.add('\uC6A7');
		charS.add('\uC6A8');
		charS.add('\uC6A9');
		charS.add('\uC6AA');
		charS.add('\uC6AB');
		charS.add('\uC6AC');
		charS.add('\uC6AD');
		charS.add('\uC6AE');
		charS.add('\uC6AF');
		charS.add('\uC6B0');
		charS.add('\uC6B1');
		charS.add('\uC6B2');
		charS.add('\uC6B3');
		charS.add('\uC6B4');
		charS.add('\uC6B5');
		charS.add('\uC6B6');
		charS.add('\uC6B7');
		charS.add('\uC6B8');
		charS.add('\uC6B9');
		charS.add('\uC6BA');
		charS.add('\uC6BB');
		charS.add('\uC6BC');
		charS.add('\uC63D');
		charS.add('\uC6BE');
		charS.add('\uC6BF');
		charS.add('\uC6C0');
		charS.add('\uC6C1');
		charS.add('\uC6C2');
		charS.add('\uC6C3');
		charS.add('\uC6C4');
		charS.add('\uC6C5');
		charS.add('\uC6C6');
		charS.add('\uC6C7');
		charS.add('\uC6C8');
		charS.add('\uC6C9');
		charS.add('\uC6CA');
		charS.add('\uC6CB');
		charS.add('\uC6CC');
		charS.add('\uC6CD');
		charS.add('\uC6CE');
		charS.add('\uC6CF');
		charS.add('\uC6D0');
		charS.add('\uC6D1');
		charS.add('\uC6D2');
		charS.add('\uC6D3');
		charS.add('\uC6D4');
		charS.add('\uC6D5');
		charS.add('\uC6D6');
		charS.add('\uC6D7');
		charS.add('\uC6D8');
		charS.add('\uC6D9');
		charS.add('\uC6DA');
		charS.add('\uC6DB');
		charS.add('\uC6DC');
		charS.add('\uC6DD');
		charS.add('\uC6DE');
		charS.add('\uC6DF');
		charS.add('\uC6E0');
		charS.add('\uC6E1');
		charS.add('\uC6E2');
		charS.add('\uC6E3');
		charS.add('\uC6E4');
		charS.add('\uC6E5');
		charS.add('\uC6E6');
		charS.add('\uC6E7');
		charS.add('\uC6E8');
		charS.add('\uC6E9');
		charS.add('\uC6EA');
		charS.add('\uC6EB');
		charS.add('\uC6EC');
		charS.add('\uC6ED');
		charS.add('\uC6EE');
		charS.add('\uC6EF');
		charS.add('\uC6F0');
		charS.add('\uC6F1');
		charS.add('\uC6F2');
		charS.add('\uC6F3');
		charS.add('\uC6F4');
		charS.add('\uC6F5');
		charS.add('\uC6F6');
		charS.add('\uC6F7');
		charS.add('\uC6F8');
		charS.add('\uC6F9');
		charS.add('\uC6FA');
		charS.add('\uC6FB');
		charS.add('\uC6FC');
		charS.add('\uC6FD');
		charS.add('\uC6FE');
		charS.add('\uC6FF');
		charS.add('\uC700');
		charS.add('\uC701');
		charS.add('\uC702');
		charS.add('\uC703');
		charS.add('\uC704');
		charS.add('\uC705');
		charS.add('\uC706');
		charS.add('\uC707');
		charS.add('\uC708');
		charS.add('\uC709');
		charS.add('\uC70A');
		charS.add('\uC70B');
		charS.add('\uC70C');
		charS.add('\uC70D');
		charS.add('\uC70E');
		charS.add('\uC70F');
		charS.add('\uC710');
		charS.add('\uC711');
		charS.add('\uC712');
		charS.add('\uC713');
		charS.add('\uC714');
		charS.add('\uC715');
		charS.add('\uC716');
		charS.add('\uC717');
		charS.add('\uC718');
		charS.add('\uC719');
		charS.add('\uC71A');
		charS.add('\uC71B');
		charS.add('\uC71C');
		charS.add('\uC71D');
		charS.add('\uC71E');
		charS.add('\uC71F');
		charS.add('\uC720');
		charS.add('\uC721');
		charS.add('\uC722');
		charS.add('\uC723');
		charS.add('\uC724');
		charS.add('\uC725');
		charS.add('\uC726');
		charS.add('\uC727');
		charS.add('\uC728');
		charS.add('\uC729');
		charS.add('\uC72A');
		charS.add('\uC72B');
		charS.add('\uC72C');
		charS.add('\uC72D');
		charS.add('\uC72E');
		charS.add('\uC72F');
		charS.add('\uC730');
		charS.add('\uC731');
		charS.add('\uC732');
		charS.add('\uC733');
		charS.add('\uC734');
		charS.add('\uC735');
		charS.add('\uC736');
		charS.add('\uC737');
		charS.add('\uC738');
		charS.add('\uC739');
		charS.add('\uC73A');
		charS.add('\uC73B');
		charS.add('\uC73C');
		charS.add('\uC73D');
		charS.add('\uC73E');
		charS.add('\uC73F');
		charS.add('\uC740');
		charS.add('\uC741');
		charS.add('\uC742');
		charS.add('\uC743');
		charS.add('\uC744');
		charS.add('\uC745');
		charS.add('\uC746');
		charS.add('\uC747');
		charS.add('\uC748');
		charS.add('\uC749');
		charS.add('\uC74A');
		charS.add('\uC74B');
		charS.add('\uC74C');
		charS.add('\uC74D');
		charS.add('\uC74E');
		charS.add('\uC74F');
		charS.add('\uC750');
		charS.add('\uC751');
		charS.add('\uC752');
		charS.add('\uC753');
		charS.add('\uC754');
		charS.add('\uC755');
		charS.add('\uC756');
		charS.add('\uC757');
		charS.add('\uC758');
		charS.add('\uC759');
		charS.add('\uC75A');
		charS.add('\uC75B');
		charS.add('\uC75C');
		charS.add('\uC75D');
		charS.add('\uC75E');
		charS.add('\uC75F');
		charS.add('\uC760');
		charS.add('\uC761');
		charS.add('\uC762');
		charS.add('\uC763');
		charS.add('\uC764');
		charS.add('\uC765');
		charS.add('\uC766');
		charS.add('\uC767');
		charS.add('\uC768');
		charS.add('\uC769');
		charS.add('\uC76A');
		charS.add('\uC76B');
		charS.add('\uC76C');
		charS.add('\uC76D');
		charS.add('\uC76E');
		charS.add('\uC76F');
		charS.add('\uC770');
		charS.add('\uC771');
		charS.add('\uC772');
		charS.add('\uC773');
		charS.add('\uC774');
		charS.add('\uC775');
		charS.add('\uC776');
		charS.add('\uC777');
		charS.add('\uC778');
		charS.add('\uC779');
		charS.add('\uC77A');
		charS.add('\uC77B');
		charS.add('\uC77C');
		charS.add('\uC77D');
		charS.add('\uC77E');
		charS.add('\uC77F');
		charS.add('\uC780');
		charS.add('\uC781');
		charS.add('\uC782');
		charS.add('\uC783');
		charS.add('\uC784');
		charS.add('\uC785');
		charS.add('\uC786');
		charS.add('\uC787');
		charS.add('\uC788');
		charS.add('\uC789');
		charS.add('\uC78A');
		charS.add('\uC78B');
		charS.add('\uC78C');
		charS.add('\uC78D');
		charS.add('\uC78E');
		charS.add('\uC78F');
		charS.add('\uC790');
		charS.add('\uC791');
		charS.add('\uC792');
		charS.add('\uC793');
		charS.add('\uC794');
		charS.add('\uC795');
		charS.add('\uC796');
		charS.add('\uC797');
		charS.add('\uC798');
		charS.add('\uC799');
		charS.add('\uC79A');
		charS.add('\uC79B');
		charS.add('\uC79C');
		charS.add('\uC79D');
		charS.add('\uC79E');
		charS.add('\uC79F');
		charS.add('\uC7A0');
		charS.add('\uC7A1');
		charS.add('\uC7A2');
		charS.add('\uC7A3');
		charS.add('\uC7A4');
		charS.add('\uC7A5');
		charS.add('\uC7A6');
		charS.add('\uC7A7');
		charS.add('\uC7A8');
		charS.add('\uC7A9');
		charS.add('\uC7AA');
		charS.add('\uC7AB');
		charS.add('\uC7AC');
		charS.add('\uC7AD');
		charS.add('\uC7AE');
		charS.add('\uC7AF');
		charS.add('\uC7B0');
		charS.add('\uC7B1');
		charS.add('\uC7B2');
		charS.add('\uC7B3');
		charS.add('\uC7B4');
		charS.add('\uC7B5');
		charS.add('\uC7B6');
		charS.add('\uC7B7');
		charS.add('\uC7B8');
		charS.add('\uC7B9');
		charS.add('\uC7BA');
		charS.add('\uC7BB');
		charS.add('\uC7BC');
		charS.add('\uC73D');
		charS.add('\uC7BE');
		charS.add('\uC7BF');
		charS.add('\uC7C0');
		charS.add('\uC7C1');
		charS.add('\uC7C2');
		charS.add('\uC7C3');
		charS.add('\uC7C4');
		charS.add('\uC7C5');
		charS.add('\uC7C6');
		charS.add('\uC7C7');
		charS.add('\uC7C8');
		charS.add('\uC7C9');
		charS.add('\uC7CA');
		charS.add('\uC7CB');
		charS.add('\uC7CC');
		charS.add('\uC7CD');
		charS.add('\uC7CE');
		charS.add('\uC7CF');
		charS.add('\uC7D0');
		charS.add('\uC7D1');
		charS.add('\uC7D2');
		charS.add('\uC7D3');
		charS.add('\uC7D4');
		charS.add('\uC7D5');
		charS.add('\uC7D6');
		charS.add('\uC7D7');
		charS.add('\uC7D8');
		charS.add('\uC7D9');
		charS.add('\uC7DA');
		charS.add('\uC7DB');
		charS.add('\uC7DC');
		charS.add('\uC7DD');
		charS.add('\uC7DE');
		charS.add('\uC7DF');
		charS.add('\uC7E0');
		charS.add('\uC7E1');
		charS.add('\uC7E2');
		charS.add('\uC7E3');
		charS.add('\uC7E4');
		charS.add('\uC7E5');
		charS.add('\uC7E6');
		charS.add('\uC7E7');
		charS.add('\uC7E8');
		charS.add('\uC7E9');
		charS.add('\uC7EA');
		charS.add('\uC7EB');
		charS.add('\uC7EC');
		charS.add('\uC7ED');
		charS.add('\uC7EE');
		charS.add('\uC7EF');
		charS.add('\uC7F0');
		charS.add('\uC7F1');
		charS.add('\uC7F2');
		charS.add('\uC7F3');
		charS.add('\uC7F4');
		charS.add('\uC7F5');
		charS.add('\uC7F6');
		charS.add('\uC7F7');
		charS.add('\uC7F8');
		charS.add('\uC7F9');
		charS.add('\uC7FA');
		charS.add('\uC7FB');
		charS.add('\uC7FC');
		charS.add('\uC7FD');
		charS.add('\uC7FE');
		charS.add('\uC7FF');
		charS.add('\uC800');
		charS.add('\uC801');
		charS.add('\uC802');
		charS.add('\uC803');
		charS.add('\uC804');
		charS.add('\uC805');
		charS.add('\uC806');
		charS.add('\uC807');
		charS.add('\uC808');
		charS.add('\uC809');
		charS.add('\uC80A');
		charS.add('\uC80B');
		charS.add('\uC80C');
		charS.add('\uC80D');
		charS.add('\uC80E');
		charS.add('\uC80F');
		charS.add('\uC810');
		charS.add('\uC811');
		charS.add('\uC812');
		charS.add('\uC813');
		charS.add('\uC814');
		charS.add('\uC815');
		charS.add('\uC816');
		charS.add('\uC817');
		charS.add('\uC818');
		charS.add('\uC819');
		charS.add('\uC81A');
		charS.add('\uC81B');
		charS.add('\uC81C');
		charS.add('\uC81D');
		charS.add('\uC81E');
		charS.add('\uC81F');
		charS.add('\uC820');
		charS.add('\uC821');
		charS.add('\uC822');
		charS.add('\uC823');
		charS.add('\uC824');
		charS.add('\uC825');
		charS.add('\uC826');
		charS.add('\uC827');
		charS.add('\uC828');
		charS.add('\uC829');
		charS.add('\uC82A');
		charS.add('\uC82B');
		charS.add('\uC82C');
		charS.add('\uC82D');
		charS.add('\uC82E');
		charS.add('\uC82F');
		charS.add('\uC830');
		charS.add('\uC831');
		charS.add('\uC832');
		charS.add('\uC833');
		charS.add('\uC834');
		charS.add('\uC835');
		charS.add('\uC836');
		charS.add('\uC837');
		charS.add('\uC838');
		charS.add('\uC839');
		charS.add('\uC83A');
		charS.add('\uC83B');
		charS.add('\uC83C');
		charS.add('\uC83D');
		charS.add('\uC83E');
		charS.add('\uC83F');
		charS.add('\uC840');
		charS.add('\uC841');
		charS.add('\uC842');
		charS.add('\uC843');
		charS.add('\uC844');
		charS.add('\uC845');
		charS.add('\uC846');
		charS.add('\uC847');
		charS.add('\uC848');
		charS.add('\uC849');
		charS.add('\uC84A');
		charS.add('\uC84B');
		charS.add('\uC84C');
		charS.add('\uC84D');
		charS.add('\uC84E');
		charS.add('\uC84F');
		charS.add('\uC850');
		charS.add('\uC851');
		charS.add('\uC852');
		charS.add('\uC853');
		charS.add('\uC854');
		charS.add('\uC855');
		charS.add('\uC856');
		charS.add('\uC857');
		charS.add('\uC858');
		charS.add('\uC859');
		charS.add('\uC85A');
		charS.add('\uC85B');
		charS.add('\uC85C');
		charS.add('\uC85D');
		charS.add('\uC85E');
		charS.add('\uC85F');
		charS.add('\uC860');
		charS.add('\uC861');
		charS.add('\uC862');
		charS.add('\uC863');
		charS.add('\uC864');
		charS.add('\uC865');
		charS.add('\uC866');
		charS.add('\uC867');
		charS.add('\uC868');
		charS.add('\uC869');
		charS.add('\uC86A');
		charS.add('\uC86B');
		charS.add('\uC86C');
		charS.add('\uC86D');
		charS.add('\uC86E');
		charS.add('\uC86F');
		charS.add('\uC870');
		charS.add('\uC871');
		charS.add('\uC872');
		charS.add('\uC873');
		charS.add('\uC874');
		charS.add('\uC875');
		charS.add('\uC876');
		charS.add('\uC877');
		charS.add('\uC878');
		charS.add('\uC879');
		charS.add('\uC87A');
		charS.add('\uC87B');
		charS.add('\uC87C');
		charS.add('\uC87D');
		charS.add('\uC87E');
		charS.add('\uC87F');
		charS.add('\uC880');
		charS.add('\uC881');
		charS.add('\uC882');
		charS.add('\uC883');
		charS.add('\uC884');
		charS.add('\uC885');
		charS.add('\uC886');
		charS.add('\uC887');
		charS.add('\uC888');
		charS.add('\uC889');
		charS.add('\uC88A');
		charS.add('\uC88B');
		charS.add('\uC88C');
		charS.add('\uC88D');
		charS.add('\uC88E');
		charS.add('\uC88F');
		charS.add('\uC890');
		charS.add('\uC891');
		charS.add('\uC892');
		charS.add('\uC893');
		charS.add('\uC894');
		charS.add('\uC895');
		charS.add('\uC896');
		charS.add('\uC897');
		charS.add('\uC898');
		charS.add('\uC899');
		charS.add('\uC89A');
		charS.add('\uC89B');
		charS.add('\uC89C');
		charS.add('\uC89D');
		charS.add('\uC89E');
		charS.add('\uC89F');
		charS.add('\uC8A0');
		charS.add('\uC8A1');
		charS.add('\uC8A2');
		charS.add('\uC8A3');
		charS.add('\uC8A4');
		charS.add('\uC8A5');
		charS.add('\uC8A6');
		charS.add('\uC8A7');
		charS.add('\uC8A8');
		charS.add('\uC8A9');
		charS.add('\uC8AA');
		charS.add('\uC8AB');
		charS.add('\uC8AC');
		charS.add('\uC8AD');
		charS.add('\uC8AE');
		charS.add('\uC8AF');
		charS.add('\uC8B0');
		charS.add('\uC8B1');
		charS.add('\uC8B2');
		charS.add('\uC8B3');
		charS.add('\uC8B4');
		charS.add('\uC8B5');
		charS.add('\uC8B6');
		charS.add('\uC8B7');
		charS.add('\uC8B8');
		charS.add('\uC8B9');
		charS.add('\uC8BA');
		charS.add('\uC8BB');
		charS.add('\uC8BC');
		charS.add('\uC83D');
		charS.add('\uC8BE');
		charS.add('\uC8BF');
		charS.add('\uC8C0');
		charS.add('\uC8C1');
		charS.add('\uC8C2');
		charS.add('\uC8C3');
		charS.add('\uC8C4');
		charS.add('\uC8C5');
		charS.add('\uC8C6');
		charS.add('\uC8C7');
		charS.add('\uC8C8');
		charS.add('\uC8C9');
		charS.add('\uC8CA');
		charS.add('\uC8CB');
		charS.add('\uC8CC');
		charS.add('\uC8CD');
		charS.add('\uC8CE');
		charS.add('\uC8CF');
		charS.add('\uC8D0');
		charS.add('\uC8D1');
		charS.add('\uC8D2');
		charS.add('\uC8D3');
		charS.add('\uC8D4');
		charS.add('\uC8D5');
		charS.add('\uC8D6');
		charS.add('\uC8D7');
		charS.add('\uC8D8');
		charS.add('\uC8D9');
		charS.add('\uC8DA');
		charS.add('\uC8DB');
		charS.add('\uC8DC');
		charS.add('\uC8DD');
		charS.add('\uC8DE');
		charS.add('\uC8DF');
		charS.add('\uC8E0');
		charS.add('\uC8E1');
		charS.add('\uC8E2');
		charS.add('\uC8E3');
		charS.add('\uC8E4');
		charS.add('\uC8E5');
		charS.add('\uC8E6');
		charS.add('\uC8E7');
		charS.add('\uC8E8');
		charS.add('\uC8E9');
		charS.add('\uC8EA');
		charS.add('\uC8EB');
		charS.add('\uC8EC');
		charS.add('\uC8ED');
		charS.add('\uC8EE');
		charS.add('\uC8EF');
		charS.add('\uC8F0');
		charS.add('\uC8F1');
		charS.add('\uC8F2');
		charS.add('\uC8F3');
		charS.add('\uC8F4');
		charS.add('\uC8F5');
		charS.add('\uC8F6');
		charS.add('\uC8F7');
		charS.add('\uC8F8');
		charS.add('\uC8F9');
		charS.add('\uC8FA');
		charS.add('\uC8FB');
		charS.add('\uC8FC');
		charS.add('\uC8FD');
		charS.add('\uC8FE');
		charS.add('\uC8FF');
		charS.add('\uC900');
		charS.add('\uC901');
		charS.add('\uC902');
		charS.add('\uC903');
		charS.add('\uC904');
		charS.add('\uC905');
		charS.add('\uC906');
		charS.add('\uC907');
		charS.add('\uC908');
		charS.add('\uC909');
		charS.add('\uC90A');
		charS.add('\uC90B');
		charS.add('\uC90C');
		charS.add('\uC90D');
		charS.add('\uC90E');
		charS.add('\uC90F');
		charS.add('\uC910');
		charS.add('\uC911');
		charS.add('\uC912');
		charS.add('\uC913');
		charS.add('\uC914');
		charS.add('\uC915');
		charS.add('\uC916');
		charS.add('\uC917');
		charS.add('\uC918');
		charS.add('\uC919');
		charS.add('\uC91A');
		charS.add('\uC91B');
		charS.add('\uC91C');
		charS.add('\uC91D');
		charS.add('\uC91E');
		charS.add('\uC91F');
		charS.add('\uC920');
		charS.add('\uC921');
		charS.add('\uC922');
		charS.add('\uC923');
		charS.add('\uC924');
		charS.add('\uC925');
		charS.add('\uC926');
		charS.add('\uC927');
		charS.add('\uC928');
		charS.add('\uC929');
		charS.add('\uC92A');
		charS.add('\uC92B');
		charS.add('\uC92C');
		charS.add('\uC92D');
		charS.add('\uC92E');
		charS.add('\uC92F');
		charS.add('\uC930');
		charS.add('\uC931');
		charS.add('\uC932');
		charS.add('\uC933');
		charS.add('\uC934');
		charS.add('\uC935');
		charS.add('\uC936');
		charS.add('\uC937');
		charS.add('\uC938');
		charS.add('\uC939');
		charS.add('\uC93A');
		charS.add('\uC93B');
		charS.add('\uC93C');
		charS.add('\uC93D');
		charS.add('\uC93E');
		charS.add('\uC93F');
		charS.add('\uC940');
		charS.add('\uC941');
		charS.add('\uC942');
		charS.add('\uC943');
		charS.add('\uC944');
		charS.add('\uC945');
		charS.add('\uC946');
		charS.add('\uC947');
		charS.add('\uC948');
		charS.add('\uC949');
		charS.add('\uC94A');
		charS.add('\uC94B');
		charS.add('\uC94C');
		charS.add('\uC94D');
		charS.add('\uC94E');
		charS.add('\uC94F');
		charS.add('\uC950');
		charS.add('\uC951');
		charS.add('\uC952');
		charS.add('\uC953');
		charS.add('\uC954');
		charS.add('\uC955');
		charS.add('\uC956');
		charS.add('\uC957');
		charS.add('\uC958');
		charS.add('\uC959');
		charS.add('\uC95A');
		charS.add('\uC95B');
		charS.add('\uC95C');
		charS.add('\uC95D');
		charS.add('\uC95E');
		charS.add('\uC95F');
		charS.add('\uC960');
		charS.add('\uC961');
		charS.add('\uC962');
		charS.add('\uC963');
		charS.add('\uC964');
		charS.add('\uC965');
		charS.add('\uC966');
		charS.add('\uC967');
		charS.add('\uC968');
		charS.add('\uC969');
		charS.add('\uC96A');
		charS.add('\uC96B');
		charS.add('\uC96C');
		charS.add('\uC96D');
		charS.add('\uC96E');
		charS.add('\uC96F');
		charS.add('\uC970');
		charS.add('\uC971');
		charS.add('\uC972');
		charS.add('\uC973');
		charS.add('\uC974');
		charS.add('\uC975');
		charS.add('\uC976');
		charS.add('\uC977');
		charS.add('\uC978');
		charS.add('\uC979');
		charS.add('\uC97A');
		charS.add('\uC97B');
		charS.add('\uC97C');
		charS.add('\uC97D');
		charS.add('\uC97E');
		charS.add('\uC97F');
		charS.add('\uC980');
		charS.add('\uC981');
		charS.add('\uC982');
		charS.add('\uC983');
		charS.add('\uC984');
		charS.add('\uC985');
		charS.add('\uC986');
		charS.add('\uC987');
		charS.add('\uC988');
		charS.add('\uC989');
		charS.add('\uC98A');
		charS.add('\uC98B');
		charS.add('\uC98C');
		charS.add('\uC98D');
		charS.add('\uC98E');
		charS.add('\uC98F');
		charS.add('\uC990');
		charS.add('\uC991');
		charS.add('\uC992');
		charS.add('\uC993');
		charS.add('\uC994');
		charS.add('\uC995');
		charS.add('\uC996');
		charS.add('\uC997');
		charS.add('\uC998');
		charS.add('\uC999');
		charS.add('\uC99A');
		charS.add('\uC99B');
		charS.add('\uC99C');
		charS.add('\uC99D');
		charS.add('\uC99E');
		charS.add('\uC99F');
		charS.add('\uC9A0');
		charS.add('\uC9A1');
		charS.add('\uC9A2');
		charS.add('\uC9A3');
		charS.add('\uC9A4');
		charS.add('\uC9A5');
		charS.add('\uC9A6');
		charS.add('\uC9A7');
		charS.add('\uC9A8');
		charS.add('\uC9A9');
		charS.add('\uC9AA');
		charS.add('\uC9AB');
		charS.add('\uC9AC');
		charS.add('\uC9AD');
		charS.add('\uC9AE');
		charS.add('\uC9AF');
		charS.add('\uC9B0');
		charS.add('\uC9B1');
		charS.add('\uC9B2');
		charS.add('\uC9B3');
		charS.add('\uC9B4');
		charS.add('\uC9B5');
		charS.add('\uC9B6');
		charS.add('\uC9B7');
		charS.add('\uC9B8');
		charS.add('\uC9B9');
		charS.add('\uC9BA');
		charS.add('\uC9BB');
		charS.add('\uC9BC');
		charS.add('\uC93D');
		charS.add('\uC9BE');
		charS.add('\uC9BF');
		charS.add('\uC9C0');
		charS.add('\uC9C1');
		charS.add('\uC9C2');
		charS.add('\uC9C3');
		charS.add('\uC9C4');
		charS.add('\uC9C5');
		charS.add('\uC9C6');
		charS.add('\uC9C7');
		charS.add('\uC9C8');
		charS.add('\uC9C9');
		charS.add('\uC9CA');
		charS.add('\uC9CB');
		charS.add('\uC9CC');
		charS.add('\uC9CD');
		charS.add('\uC9CE');
		charS.add('\uC9CF');
		charS.add('\uC9D0');
		charS.add('\uC9D1');
		charS.add('\uC9D2');
		charS.add('\uC9D3');
		charS.add('\uC9D4');
		charS.add('\uC9D5');
		charS.add('\uC9D6');
		charS.add('\uC9D7');
		charS.add('\uC9D8');
		charS.add('\uC9D9');
		charS.add('\uC9DA');
		charS.add('\uC9DB');
		charS.add('\uC9DC');
		charS.add('\uC9DD');
		charS.add('\uC9DE');
		charS.add('\uC9DF');
		charS.add('\uC9E0');
		charS.add('\uC9E1');
		charS.add('\uC9E2');
		charS.add('\uC9E3');
		charS.add('\uC9E4');
		charS.add('\uC9E5');
		charS.add('\uC9E6');
		charS.add('\uC9E7');
		charS.add('\uC9E8');
		charS.add('\uC9E9');
		charS.add('\uC9EA');
		charS.add('\uC9EB');
		charS.add('\uC9EC');
		charS.add('\uC9ED');
		charS.add('\uC9EE');
		charS.add('\uC9EF');
		charS.add('\uC9F0');
		charS.add('\uC9F1');
		charS.add('\uC9F2');
		charS.add('\uC9F3');
		charS.add('\uC9F4');
		charS.add('\uC9F5');
		charS.add('\uC9F6');
		charS.add('\uC9F7');
		charS.add('\uC9F8');
		charS.add('\uC9F9');
		charS.add('\uC9FA');
		charS.add('\uC9FB');
		charS.add('\uC9FC');
		charS.add('\uC9FD');
		charS.add('\uC9FE');
		charS.add('\uC9FF');
		charS.add('\uCA00');
		charS.add('\uCA01');
		charS.add('\uCA02');
		charS.add('\uCA03');
		charS.add('\uCA04');
		charS.add('\uCA05');
		charS.add('\uCA06');
		charS.add('\uCA07');
		charS.add('\uCA08');
		charS.add('\uCA09');
		charS.add('\uCA0A');
		charS.add('\uCA0B');
		charS.add('\uCA0C');
		charS.add('\uCA0D');
		charS.add('\uCA0E');
		charS.add('\uCA0F');
		charS.add('\uCA10');
		charS.add('\uCA11');
		charS.add('\uCA12');
		charS.add('\uCA13');
		charS.add('\uCA14');
		charS.add('\uCA15');
		charS.add('\uCA16');
		charS.add('\uCA17');
		charS.add('\uCA18');
		charS.add('\uCA19');
		charS.add('\uCA1A');
		charS.add('\uCA1B');
		charS.add('\uCA1C');
		charS.add('\uCA1D');
		charS.add('\uCA1E');
		charS.add('\uCA1F');
		charS.add('\uCA20');
		charS.add('\uCA21');
		charS.add('\uCA22');
		charS.add('\uCA23');
		charS.add('\uCA24');
		charS.add('\uCA25');
		charS.add('\uCA26');
		charS.add('\uCA27');
		charS.add('\uCA28');
		charS.add('\uCA29');
		charS.add('\uCA2A');
		charS.add('\uCA2B');
		charS.add('\uCA2C');
		charS.add('\uCA2D');
		charS.add('\uCA2E');
		charS.add('\uCA2F');
		charS.add('\uCA30');
		charS.add('\uCA31');
		charS.add('\uCA32');
		charS.add('\uCA33');
		charS.add('\uCA34');
		charS.add('\uCA35');
		charS.add('\uCA36');
		charS.add('\uCA37');
		charS.add('\uCA38');
		charS.add('\uCA39');
		charS.add('\uCA3A');
		charS.add('\uCA3B');
		charS.add('\uCA3C');
		charS.add('\uCA3D');
		charS.add('\uCA3E');
		charS.add('\uCA3F');
		charS.add('\uCA40');
		charS.add('\uCA41');
		charS.add('\uCA42');
		charS.add('\uCA43');
		charS.add('\uCA44');
		charS.add('\uCA45');
		charS.add('\uCA46');
		charS.add('\uCA47');
		charS.add('\uCA48');
		charS.add('\uCA49');
		charS.add('\uCA4A');
		charS.add('\uCA4B');
		charS.add('\uCA4C');
		charS.add('\uCA4D');
		charS.add('\uCA4E');
		charS.add('\uCA4F');
		charS.add('\uCA50');
		charS.add('\uCA51');
		charS.add('\uCA52');
		charS.add('\uCA53');
		charS.add('\uCA54');
		charS.add('\uCA55');
		charS.add('\uCA56');
		charS.add('\uCA57');
		charS.add('\uCA58');
		charS.add('\uCA59');
		charS.add('\uCA5A');
		charS.add('\uCA5B');
		charS.add('\uCA5C');
		charS.add('\uCA5D');
		charS.add('\uCA5E');
		charS.add('\uCA5F');
		charS.add('\uCA60');
		charS.add('\uCA61');
		charS.add('\uCA62');
		charS.add('\uCA63');
		charS.add('\uCA64');
		charS.add('\uCA65');
		charS.add('\uCA66');
		charS.add('\uCA67');
		charS.add('\uCA68');
		charS.add('\uCA69');
		charS.add('\uCA6A');
		charS.add('\uCA6B');
		charS.add('\uCA6C');
		charS.add('\uCA6D');
		charS.add('\uCA6E');
		charS.add('\uCA6F');
		charS.add('\uCA70');
		charS.add('\uCA71');
		charS.add('\uCA72');
		charS.add('\uCA73');
		charS.add('\uCA74');
		charS.add('\uCA75');
		charS.add('\uCA76');
		charS.add('\uCA77');
		charS.add('\uCA78');
		charS.add('\uCA79');
		charS.add('\uCA7A');
		charS.add('\uCA7B');
		charS.add('\uCA7C');
		charS.add('\uCA7D');
		charS.add('\uCA7E');
		charS.add('\uCA7F');
		charS.add('\uCA80');
		charS.add('\uCA81');
		charS.add('\uCA82');
		charS.add('\uCA83');
		charS.add('\uCA84');
		charS.add('\uCA85');
		charS.add('\uCA86');
		charS.add('\uCA87');
		charS.add('\uCA88');
		charS.add('\uCA89');
		charS.add('\uCA8A');
		charS.add('\uCA8B');
		charS.add('\uCA8C');
		charS.add('\uCA8D');
		charS.add('\uCA8E');
		charS.add('\uCA8F');
		charS.add('\uCA90');
		charS.add('\uCA91');
		charS.add('\uCA92');
		charS.add('\uCA93');
		charS.add('\uCA94');
		charS.add('\uCA95');
		charS.add('\uCA96');
		charS.add('\uCA97');
		charS.add('\uCA98');
		charS.add('\uCA99');
		charS.add('\uCA9A');
		charS.add('\uCA9B');
		charS.add('\uCA9C');
		charS.add('\uCA9D');
		charS.add('\uCA9E');
		charS.add('\uCA9F');
		charS.add('\uCAA0');
		charS.add('\uCAA1');
		charS.add('\uCAA2');
		charS.add('\uCAA3');
		charS.add('\uCAA4');
		charS.add('\uCAA5');
		charS.add('\uCAA6');
		charS.add('\uCAA7');
		charS.add('\uCAA8');
		charS.add('\uCAA9');
		charS.add('\uCAAA');
		charS.add('\uCAAB');
		charS.add('\uCAAC');
		charS.add('\uCAAD');
		charS.add('\uCAAE');
		charS.add('\uCAAF');
		charS.add('\uCAB0');
		charS.add('\uCAB1');
		charS.add('\uCAB2');
		charS.add('\uCAB3');
		charS.add('\uCAB4');
		charS.add('\uCAB5');
		charS.add('\uCAB6');
		charS.add('\uCAB7');
		charS.add('\uCAB8');
		charS.add('\uCAB9');
		charS.add('\uCABA');
		charS.add('\uCABB');
		charS.add('\uCABC');
		charS.add('\uCA3D');
		charS.add('\uCABE');
		charS.add('\uCABF');
		charS.add('\uCAC0');
		charS.add('\uCAC1');
		charS.add('\uCAC2');
		charS.add('\uCAC3');
		charS.add('\uCAC4');
		charS.add('\uCAC5');
		charS.add('\uCAC6');
		charS.add('\uCAC7');
		charS.add('\uCAC8');
		charS.add('\uCAC9');
		charS.add('\uCACA');
		charS.add('\uCACB');
		charS.add('\uCACC');
		charS.add('\uCACD');
		charS.add('\uCACE');
		charS.add('\uCACF');
		charS.add('\uCAD0');
		charS.add('\uCAD1');
		charS.add('\uCAD2');
		charS.add('\uCAD3');
		charS.add('\uCAD4');
		charS.add('\uCAD5');
		charS.add('\uCAD6');
		charS.add('\uCAD7');
		charS.add('\uCAD8');
		charS.add('\uCAD9');
		charS.add('\uCADA');
		charS.add('\uCADB');
		charS.add('\uCADC');
		charS.add('\uCADD');
		charS.add('\uCADE');
		charS.add('\uCADF');
		charS.add('\uCAE0');
		charS.add('\uCAE1');
		charS.add('\uCAE2');
		charS.add('\uCAE3');
		charS.add('\uCAE4');
		charS.add('\uCAE5');
		charS.add('\uCAE6');
		charS.add('\uCAE7');
		charS.add('\uCAE8');
		charS.add('\uCAE9');
		charS.add('\uCAEA');
		charS.add('\uCAEB');
		charS.add('\uCAEC');
		charS.add('\uCAED');
		charS.add('\uCAEE');
		charS.add('\uCAEF');
		charS.add('\uCAF0');
		charS.add('\uCAF1');
		charS.add('\uCAF2');
		charS.add('\uCAF3');
		charS.add('\uCAF4');
		charS.add('\uCAF5');
		charS.add('\uCAF6');
		charS.add('\uCAF7');
		charS.add('\uCAF8');
		charS.add('\uCAF9');
		charS.add('\uCAFA');
		charS.add('\uCAFB');
		charS.add('\uCAFC');
		charS.add('\uCAFD');
		charS.add('\uCAFE');
		charS.add('\uCAFF');
		charS.add('\uCB00');
		charS.add('\uCB01');
		charS.add('\uCB02');
		charS.add('\uCB03');
		charS.add('\uCB04');
		charS.add('\uCB05');
		charS.add('\uCB06');
		charS.add('\uCB07');
		charS.add('\uCB08');
		charS.add('\uCB09');
		charS.add('\uCB0A');
		charS.add('\uCB0B');
		charS.add('\uCB0C');
		charS.add('\uCB0D');
		charS.add('\uCB0E');
		charS.add('\uCB0F');
		charS.add('\uCB10');
		charS.add('\uCB11');
		charS.add('\uCB12');
		charS.add('\uCB13');
		charS.add('\uCB14');
		charS.add('\uCB15');
		charS.add('\uCB16');
		charS.add('\uCB17');
		charS.add('\uCB18');
		charS.add('\uCB19');
		charS.add('\uCB1A');
		charS.add('\uCB1B');
		charS.add('\uCB1C');
		charS.add('\uCB1D');
		charS.add('\uCB1E');
		charS.add('\uCB1F');
		charS.add('\uCB20');
		charS.add('\uCB21');
		charS.add('\uCB22');
		charS.add('\uCB23');
		charS.add('\uCB24');
		charS.add('\uCB25');
		charS.add('\uCB26');
		charS.add('\uCB27');
		charS.add('\uCB28');
		charS.add('\uCB29');
		charS.add('\uCB2A');
		charS.add('\uCB2B');
		charS.add('\uCB2C');
		charS.add('\uCB2D');
		charS.add('\uCB2E');
		charS.add('\uCB2F');
		charS.add('\uCB30');
		charS.add('\uCB31');
		charS.add('\uCB32');
		charS.add('\uCB33');
		charS.add('\uCB34');
		charS.add('\uCB35');
		charS.add('\uCB36');
		charS.add('\uCB37');
		charS.add('\uCB38');
		charS.add('\uCB39');
		charS.add('\uCB3A');
		charS.add('\uCB3B');
		charS.add('\uCB3C');
		charS.add('\uCB3D');
		charS.add('\uCB3E');
		charS.add('\uCB3F');
		charS.add('\uCB40');
		charS.add('\uCB41');
		charS.add('\uCB42');
		charS.add('\uCB43');
		charS.add('\uCB44');
		charS.add('\uCB45');
		charS.add('\uCB46');
		charS.add('\uCB47');
		charS.add('\uCB48');
		charS.add('\uCB49');
		charS.add('\uCB4A');
		charS.add('\uCB4B');
		charS.add('\uCB4C');
		charS.add('\uCB4D');
		charS.add('\uCB4E');
		charS.add('\uCB4F');
		charS.add('\uCB50');
		charS.add('\uCB51');
		charS.add('\uCB52');
		charS.add('\uCB53');
		charS.add('\uCB54');
		charS.add('\uCB55');
		charS.add('\uCB56');
		charS.add('\uCB57');
		charS.add('\uCB58');
		charS.add('\uCB59');
		charS.add('\uCB5A');
		charS.add('\uCB5B');
		charS.add('\uCB5C');
		charS.add('\uCB5D');
		charS.add('\uCB5E');
		charS.add('\uCB5F');
		charS.add('\uCB60');
		charS.add('\uCB61');
		charS.add('\uCB62');
		charS.add('\uCB63');
		charS.add('\uCB64');
		charS.add('\uCB65');
		charS.add('\uCB66');
		charS.add('\uCB67');
		charS.add('\uCB68');
		charS.add('\uCB69');
		charS.add('\uCB6A');
		charS.add('\uCB6B');
		charS.add('\uCB6C');
		charS.add('\uCB6D');
		charS.add('\uCB6E');
		charS.add('\uCB6F');
		charS.add('\uCB70');
		charS.add('\uCB71');
		charS.add('\uCB72');
		charS.add('\uCB73');
		charS.add('\uCB74');
		charS.add('\uCB75');
		charS.add('\uCB76');
		charS.add('\uCB77');
		charS.add('\uCB78');
		charS.add('\uCB79');
		charS.add('\uCB7A');
		charS.add('\uCB7B');
		charS.add('\uCB7C');
		charS.add('\uCB7D');
		charS.add('\uCB7E');
		charS.add('\uCB7F');
		charS.add('\uCB80');
		charS.add('\uCB81');
		charS.add('\uCB82');
		charS.add('\uCB83');
		charS.add('\uCB84');
		charS.add('\uCB85');
		charS.add('\uCB86');
		charS.add('\uCB87');
		charS.add('\uCB88');
		charS.add('\uCB89');
		charS.add('\uCB8A');
		charS.add('\uCB8B');
		charS.add('\uCB8C');
		charS.add('\uCB8D');
		charS.add('\uCB8E');
		charS.add('\uCB8F');
		charS.add('\uCB90');
		charS.add('\uCB91');
		charS.add('\uCB92');
		charS.add('\uCB93');
		charS.add('\uCB94');
		charS.add('\uCB95');
		charS.add('\uCB96');
		charS.add('\uCB97');
		charS.add('\uCB98');
		charS.add('\uCB99');
		charS.add('\uCB9A');
		charS.add('\uCB9B');
		charS.add('\uCB9C');
		charS.add('\uCB9D');
		charS.add('\uCB9E');
		charS.add('\uCB9F');
		charS.add('\uCBA0');
		charS.add('\uCBA1');
		charS.add('\uCBA2');
		charS.add('\uCBA3');
		charS.add('\uCBA4');
		charS.add('\uCBA5');
		charS.add('\uCBA6');
		charS.add('\uCBA7');
		charS.add('\uCBA8');
		charS.add('\uCBA9');
		charS.add('\uCBAA');
		charS.add('\uCBAB');
		charS.add('\uCBAC');
		charS.add('\uCBAD');
		charS.add('\uCBAE');
		charS.add('\uCBAF');
		charS.add('\uCBB0');
		charS.add('\uCBB1');
		charS.add('\uCBB2');
		charS.add('\uCBB3');
		charS.add('\uCBB4');
		charS.add('\uCBB5');
		charS.add('\uCBB6');
		charS.add('\uCBB7');
		charS.add('\uCBB8');
		charS.add('\uCBB9');
		charS.add('\uCBBA');
		charS.add('\uCBBB');
		charS.add('\uCBBC');
		charS.add('\uCB3D');
		charS.add('\uCBBE');
		charS.add('\uCBBF');
		charS.add('\uCBC0');
		charS.add('\uCBC1');
		charS.add('\uCBC2');
		charS.add('\uCBC3');
		charS.add('\uCBC4');
		charS.add('\uCBC5');
		charS.add('\uCBC6');
		charS.add('\uCBC7');
		charS.add('\uCBC8');
		charS.add('\uCBC9');
		charS.add('\uCBCA');
		charS.add('\uCBCB');
		charS.add('\uCBCC');
		charS.add('\uCBCD');
		charS.add('\uCBCE');
		charS.add('\uCBCF');
		charS.add('\uCBD0');
		charS.add('\uCBD1');
		charS.add('\uCBD2');
		charS.add('\uCBD3');
		charS.add('\uCBD4');
		charS.add('\uCBD5');
		charS.add('\uCBD6');
		charS.add('\uCBD7');
		charS.add('\uCBD8');
		charS.add('\uCBD9');
		charS.add('\uCBDA');
		charS.add('\uCBDB');
		charS.add('\uCBDC');
		charS.add('\uCBDD');
		charS.add('\uCBDE');
		charS.add('\uCBDF');
		charS.add('\uCBE0');
		charS.add('\uCBE1');
		charS.add('\uCBE2');
		charS.add('\uCBE3');
		charS.add('\uCBE4');
		charS.add('\uCBE5');
		charS.add('\uCBE6');
		charS.add('\uCBE7');
		charS.add('\uCBE8');
		charS.add('\uCBE9');
		charS.add('\uCBEA');
		charS.add('\uCBEB');
		charS.add('\uCBEC');
		charS.add('\uCBED');
		charS.add('\uCBEE');
		charS.add('\uCBEF');
		charS.add('\uCBF0');
		charS.add('\uCBF1');
		charS.add('\uCBF2');
		charS.add('\uCBF3');
		charS.add('\uCBF4');
		charS.add('\uCBF5');
		charS.add('\uCBF6');
		charS.add('\uCBF7');
		charS.add('\uCBF8');
		charS.add('\uCBF9');
		charS.add('\uCBFA');
		charS.add('\uCBFB');
		charS.add('\uCBFC');
		charS.add('\uCBFD');
		charS.add('\uCBFE');
		charS.add('\uCBFF');
		charS.add('\uCD00');
		charS.add('\uCD01');
		charS.add('\uCD02');
		charS.add('\uCD03');
		charS.add('\uCD04');
		charS.add('\uCD05');
		charS.add('\uCD06');
		charS.add('\uCD07');
		charS.add('\uCD08');
		charS.add('\uCD09');
		charS.add('\uCD0A');
		charS.add('\uCD0B');
		charS.add('\uCD0C');
		charS.add('\uCD0D');
		charS.add('\uCD0E');
		charS.add('\uCD0F');
		charS.add('\uCD10');
		charS.add('\uCD11');
		charS.add('\uCD12');
		charS.add('\uCD13');
		charS.add('\uCD14');
		charS.add('\uCD15');
		charS.add('\uCD16');
		charS.add('\uCD17');
		charS.add('\uCD18');
		charS.add('\uCD19');
		charS.add('\uCD1A');
		charS.add('\uCD1B');
		charS.add('\uCD1C');
		charS.add('\uCD1D');
		charS.add('\uCD1E');
		charS.add('\uCD1F');
		charS.add('\uCD20');
		charS.add('\uCD21');
		charS.add('\uCD22');
		charS.add('\uCD23');
		charS.add('\uCD24');
		charS.add('\uCD25');
		charS.add('\uCD26');
		charS.add('\uCD27');
		charS.add('\uCD28');
		charS.add('\uCD29');
		charS.add('\uCD2A');
		charS.add('\uCD2B');
		charS.add('\uCD2C');
		charS.add('\uCD2D');
		charS.add('\uCD2E');
		charS.add('\uCD2F');
		charS.add('\uCD30');
		charS.add('\uCD31');
		charS.add('\uCD32');
		charS.add('\uCD33');
		charS.add('\uCD34');
		charS.add('\uCD35');
		charS.add('\uCD36');
		charS.add('\uCD37');
		charS.add('\uCD38');
		charS.add('\uCD39');
		charS.add('\uCD3A');
		charS.add('\uCD3B');
		charS.add('\uCD3C');
		charS.add('\uCD3D');
		charS.add('\uCD3E');
		charS.add('\uCD3F');
		charS.add('\uCD40');
		charS.add('\uCD41');
		charS.add('\uCD42');
		charS.add('\uCD43');
		charS.add('\uCD44');
		charS.add('\uCD45');
		charS.add('\uCD46');
		charS.add('\uCD47');
		charS.add('\uCD48');
		charS.add('\uCD49');
		charS.add('\uCD4A');
		charS.add('\uCD4B');
		charS.add('\uCD4C');
		charS.add('\uCD4D');
		charS.add('\uCD4E');
		charS.add('\uCD4F');
		charS.add('\uCD50');
		charS.add('\uCD51');
		charS.add('\uCD52');
		charS.add('\uCD53');
		charS.add('\uCD54');
		charS.add('\uCD55');
		charS.add('\uCD56');
		charS.add('\uCD57');
		charS.add('\uCD58');
		charS.add('\uCD59');
		charS.add('\uCD5A');
		charS.add('\uCD5B');
		charS.add('\uCD5C');
		charS.add('\uCD5D');
		charS.add('\uCD5E');
		charS.add('\uCD5F');
		charS.add('\uCD60');
		charS.add('\uCD61');
		charS.add('\uCD62');
		charS.add('\uCD63');
		charS.add('\uCD64');
		charS.add('\uCD65');
		charS.add('\uCD66');
		charS.add('\uCD67');
		charS.add('\uCD68');
		charS.add('\uCD69');
		charS.add('\uCD6A');
		charS.add('\uCD6B');
		charS.add('\uCD6C');
		charS.add('\uCD6D');
		charS.add('\uCD6E');
		charS.add('\uCD6F');
		charS.add('\uCD70');
		charS.add('\uCD71');
		charS.add('\uCD72');
		charS.add('\uCD73');
		charS.add('\uCD74');
		charS.add('\uCD75');
		charS.add('\uCD76');
		charS.add('\uCD77');
		charS.add('\uCD78');
		charS.add('\uCD79');
		charS.add('\uCD7A');
		charS.add('\uCD7B');
		charS.add('\uCD7C');
		charS.add('\uCD7D');
		charS.add('\uCD7E');
		charS.add('\uCD7F');
		charS.add('\uCD80');
		charS.add('\uCD81');
		charS.add('\uCD82');
		charS.add('\uCD83');
		charS.add('\uCD84');
		charS.add('\uCD85');
		charS.add('\uCD86');
		charS.add('\uCD87');
		charS.add('\uCD88');
		charS.add('\uCD89');
		charS.add('\uCD8A');
		charS.add('\uCD8B');
		charS.add('\uCD8C');
		charS.add('\uCD8D');
		charS.add('\uCD8E');
		charS.add('\uCD8F');
		charS.add('\uCD90');
		charS.add('\uCD91');
		charS.add('\uCD92');
		charS.add('\uCD93');
		charS.add('\uCD94');
		charS.add('\uCD95');
		charS.add('\uCD96');
		charS.add('\uCD97');
		charS.add('\uCD98');
		charS.add('\uCD99');
		charS.add('\uCD9A');
		charS.add('\uCD9B');
		charS.add('\uCD9C');
		charS.add('\uCD9D');
		charS.add('\uCD9E');
		charS.add('\uCD9F');
		charS.add('\uCDA0');
		charS.add('\uCDA1');
		charS.add('\uCDA2');
		charS.add('\uCDA3');
		charS.add('\uCDA4');
		charS.add('\uCDA5');
		charS.add('\uCDA6');
		charS.add('\uCDA7');
		charS.add('\uCDA8');
		charS.add('\uCDA9');
		charS.add('\uCDAA');
		charS.add('\uCDAB');
		charS.add('\uCDAC');
		charS.add('\uCDAD');
		charS.add('\uCDAE');
		charS.add('\uCDAF');
		charS.add('\uCDB0');
		charS.add('\uCDB1');
		charS.add('\uCDB2');
		charS.add('\uCDB3');
		charS.add('\uCDB4');
		charS.add('\uCDB5');
		charS.add('\uCDB6');
		charS.add('\uCDB7');
		charS.add('\uCDB8');
		charS.add('\uCDB9');
		charS.add('\uCDBA');
		charS.add('\uCDBB');
		charS.add('\uCDBC');
		charS.add('\uCD3D');
		charS.add('\uCDBE');
		charS.add('\uCDBF');
		charS.add('\uCDC0');
		charS.add('\uCDC1');
		charS.add('\uCDC2');
		charS.add('\uCDC3');
		charS.add('\uCDC4');
		charS.add('\uCDC5');
		charS.add('\uCDC6');
		charS.add('\uCDC7');
		charS.add('\uCDC8');
		charS.add('\uCDC9');
		charS.add('\uCDCA');
		charS.add('\uCDCB');
		charS.add('\uCDCC');
		charS.add('\uCDCD');
		charS.add('\uCDCE');
		charS.add('\uCDCF');
		charS.add('\uCDD0');
		charS.add('\uCDD1');
		charS.add('\uCDD2');
		charS.add('\uCDD3');
		charS.add('\uCDD4');
		charS.add('\uCDD5');
		charS.add('\uCDD6');
		charS.add('\uCDD7');
		charS.add('\uCDD8');
		charS.add('\uCDD9');
		charS.add('\uCDDA');
		charS.add('\uCDDB');
		charS.add('\uCDDC');
		charS.add('\uCDDD');
		charS.add('\uCDDE');
		charS.add('\uCDDF');
		charS.add('\uCDE0');
		charS.add('\uCDE1');
		charS.add('\uCDE2');
		charS.add('\uCDE3');
		charS.add('\uCDE4');
		charS.add('\uCDE5');
		charS.add('\uCDE6');
		charS.add('\uCDE7');
		charS.add('\uCDE8');
		charS.add('\uCDE9');
		charS.add('\uCDEA');
		charS.add('\uCDEB');
		charS.add('\uCDEC');
		charS.add('\uCDED');
		charS.add('\uCDEE');
		charS.add('\uCDEF');
		charS.add('\uCDF0');
		charS.add('\uCDF1');
		charS.add('\uCDF2');
		charS.add('\uCDF3');
		charS.add('\uCDF4');
		charS.add('\uCDF5');
		charS.add('\uCDF6');
		charS.add('\uCDF7');
		charS.add('\uCDF8');
		charS.add('\uCDF9');
		charS.add('\uCDFA');
		charS.add('\uCDFB');
		charS.add('\uCDFC');
		charS.add('\uCDFD');
		charS.add('\uCDFE');
		charS.add('\uCDFF');
		charS.add('\uCE00');
		charS.add('\uCE01');
		charS.add('\uCE02');
		charS.add('\uCE03');
		charS.add('\uCE04');
		charS.add('\uCE05');
		charS.add('\uCE06');
		charS.add('\uCE07');
		charS.add('\uCE08');
		charS.add('\uCE09');
		charS.add('\uCE0A');
		charS.add('\uCE0B');
		charS.add('\uCE0C');
		charS.add('\uCE0D');
		charS.add('\uCE0E');
		charS.add('\uCE0F');
		charS.add('\uCE10');
		charS.add('\uCE11');
		charS.add('\uCE12');
		charS.add('\uCE13');
		charS.add('\uCE14');
		charS.add('\uCE15');
		charS.add('\uCE16');
		charS.add('\uCE17');
		charS.add('\uCE18');
		charS.add('\uCE19');
		charS.add('\uCE1A');
		charS.add('\uCE1B');
		charS.add('\uCE1C');
		charS.add('\uCE1D');
		charS.add('\uCE1E');
		charS.add('\uCE1F');
		charS.add('\uCE20');
		charS.add('\uCE21');
		charS.add('\uCE22');
		charS.add('\uCE23');
		charS.add('\uCE24');
		charS.add('\uCE25');
		charS.add('\uCE26');
		charS.add('\uCE27');
		charS.add('\uCE28');
		charS.add('\uCE29');
		charS.add('\uCE2A');
		charS.add('\uCE2B');
		charS.add('\uCE2C');
		charS.add('\uCE2D');
		charS.add('\uCE2E');
		charS.add('\uCE2F');
		charS.add('\uCE30');
		charS.add('\uCE31');
		charS.add('\uCE32');
		charS.add('\uCE33');
		charS.add('\uCE34');
		charS.add('\uCE35');
		charS.add('\uCE36');
		charS.add('\uCE37');
		charS.add('\uCE38');
		charS.add('\uCE39');
		charS.add('\uCE3A');
		charS.add('\uCE3B');
		charS.add('\uCE3C');
		charS.add('\uCE3D');
		charS.add('\uCE3E');
		charS.add('\uCE3F');
		charS.add('\uCE40');
		charS.add('\uCE41');
		charS.add('\uCE42');
		charS.add('\uCE43');
		charS.add('\uCE44');
		charS.add('\uCE45');
		charS.add('\uCE46');
		charS.add('\uCE47');
		charS.add('\uCE48');
		charS.add('\uCE49');
		charS.add('\uCE4A');
		charS.add('\uCE4B');
		charS.add('\uCE4C');
		charS.add('\uCE4D');
		charS.add('\uCE4E');
		charS.add('\uCE4F');
		charS.add('\uCE50');
		charS.add('\uCE51');
		charS.add('\uCE52');
		charS.add('\uCE53');
		charS.add('\uCE54');
		charS.add('\uCE55');
		charS.add('\uCE56');
		charS.add('\uCE57');
		charS.add('\uCE58');
		charS.add('\uCE59');
		charS.add('\uCE5A');
		charS.add('\uCE5B');
		charS.add('\uCE5C');
		charS.add('\uCE5D');
		charS.add('\uCE5E');
		charS.add('\uCE5F');
		charS.add('\uCE60');
		charS.add('\uCE61');
		charS.add('\uCE62');
		charS.add('\uCE63');
		charS.add('\uCE64');
		charS.add('\uCE65');
		charS.add('\uCE66');
		charS.add('\uCE67');
		charS.add('\uCE68');
		charS.add('\uCE69');
		charS.add('\uCE6A');
		charS.add('\uCE6B');
		charS.add('\uCE6C');
		charS.add('\uCE6D');
		charS.add('\uCE6E');
		charS.add('\uCE6F');
		charS.add('\uCE70');
		charS.add('\uCE71');
		charS.add('\uCE72');
		charS.add('\uCE73');
		charS.add('\uCE74');
		charS.add('\uCE75');
		charS.add('\uCE76');
		charS.add('\uCE77');
		charS.add('\uCE78');
		charS.add('\uCE79');
		charS.add('\uCE7A');
		charS.add('\uCE7B');
		charS.add('\uCE7C');
		charS.add('\uCE7D');
		charS.add('\uCE7E');
		charS.add('\uCE7F');
		charS.add('\uCE80');
		charS.add('\uCE81');
		charS.add('\uCE82');
		charS.add('\uCE83');
		charS.add('\uCE84');
		charS.add('\uCE85');
		charS.add('\uCE86');
		charS.add('\uCE87');
		charS.add('\uCE88');
		charS.add('\uCE89');
		charS.add('\uCE8A');
		charS.add('\uCE8B');
		charS.add('\uCE8C');
		charS.add('\uCE8D');
		charS.add('\uCE8E');
		charS.add('\uCE8F');
		charS.add('\uCE90');
		charS.add('\uCE91');
		charS.add('\uCE92');
		charS.add('\uCE93');
		charS.add('\uCE94');
		charS.add('\uCE95');
		charS.add('\uCE96');
		charS.add('\uCE97');
		charS.add('\uCE98');
		charS.add('\uCE99');
		charS.add('\uCE9A');
		charS.add('\uCE9B');
		charS.add('\uCE9C');
		charS.add('\uCE9D');
		charS.add('\uCF00');
		charS.add('\uCF01');
		charS.add('\uCF02');
		charS.add('\uCF03');
		charS.add('\uCF04');
		charS.add('\uCF05');
		charS.add('\uCF06');
		charS.add('\uCF07');
		charS.add('\uCF08');
		charS.add('\uCF09');
		charS.add('\uCF0A');
		charS.add('\uCF0B');
		charS.add('\uCF0C');
		charS.add('\uCF0D');
		charS.add('\uCF0E');
		charS.add('\uCF0F');
		charS.add('\uCF10');
		charS.add('\uCF11');
		charS.add('\uCF12');
		charS.add('\uCF13');
		charS.add('\uCF14');
		charS.add('\uCF15');
		charS.add('\uCF16');
		charS.add('\uCF17');
		charS.add('\uCF18');
		charS.add('\uCF19');
		charS.add('\uCF1A');
		charS.add('\uCF1B');
		charS.add('\uCF1C');
		charS.add('\uCF1D');
		charS.add('\uCF1E');
		charS.add('\uCF1F');
		charS.add('\uCF20');
		charS.add('\uCF21');
		charS.add('\uCF22');
		charS.add('\uCF23');
		charS.add('\uCF24');
		charS.add('\uCF25');
		charS.add('\uCF26');
		charS.add('\uCF27');
		charS.add('\uCF28');
		charS.add('\uCF29');
		charS.add('\uCF2A');
		charS.add('\uCF2B');
		charS.add('\uCF2C');
		charS.add('\uCF2D');
		charS.add('\uCF2E');
		charS.add('\uCF2F');
		charS.add('\uCF30');
		charS.add('\uCF31');
		charS.add('\uCF32');
		charS.add('\uCF33');
		charS.add('\uCF34');
		charS.add('\uCF35');
		charS.add('\uCF36');
		charS.add('\uCF37');
		charS.add('\uCF38');
		charS.add('\uCF39');
		charS.add('\uCF3A');
		charS.add('\uCF3B');
		charS.add('\uCF3C');
		charS.add('\uCF3D');
		charS.add('\uCF3E');
		charS.add('\uCF3F');
		charS.add('\uCF40');
		charS.add('\uCF41');
		charS.add('\uCF42');
		charS.add('\uCF43');
		charS.add('\uCF44');
		charS.add('\uCF45');
		charS.add('\uCF46');
		charS.add('\uCF47');
		charS.add('\uCF48');
		charS.add('\uCF49');
		charS.add('\uCF4A');
		charS.add('\uCF4B');
		charS.add('\uCF4C');
		charS.add('\uCF4D');
		charS.add('\uCF4E');
		charS.add('\uCF4F');
		charS.add('\uCF50');
		charS.add('\uCF51');
		charS.add('\uCF52');
		charS.add('\uCF53');
		charS.add('\uCF54');
		charS.add('\uCF55');
		charS.add('\uCF56');
		charS.add('\uCF57');
		charS.add('\uCF58');
		charS.add('\uCF59');
		charS.add('\uCF5A');
		charS.add('\uCF5B');
		charS.add('\uCF5C');
		charS.add('\uCF5D');
		charS.add('\uCF5E');
		charS.add('\uCF5F');
		charS.add('\uCF60');
		charS.add('\uCF61');
		charS.add('\uCF62');
		charS.add('\uCF63');
		charS.add('\uCF64');
		charS.add('\uCF65');
		charS.add('\uCF66');
		charS.add('\uCF67');
		charS.add('\uCF68');
		charS.add('\uCF69');
		charS.add('\uCF6A');
		charS.add('\uCF6B');
		charS.add('\uCF6C');
		charS.add('\uCF6D');
		charS.add('\uCF6E');
		charS.add('\uCF6F');
		charS.add('\uCF70');
		charS.add('\uCF71');
		charS.add('\uCF72');
		charS.add('\uCF73');
		charS.add('\uCF74');
		charS.add('\uCF75');
		charS.add('\uCF76');
		charS.add('\uCF77');
		charS.add('\uCF78');
		charS.add('\uCF79');
		charS.add('\uCF7A');
		charS.add('\uCF7B');
		charS.add('\uCF7C');
		charS.add('\uCF7D');
		charS.add('\uCF7E');
		charS.add('\uCF7F');
		charS.add('\uCF80');
		charS.add('\uCF81');
		charS.add('\uCF82');
		charS.add('\uCF83');
		charS.add('\uCF84');
		charS.add('\uCF85');
		charS.add('\uCF86');
		charS.add('\uCF87');
		charS.add('\uCF88');
		charS.add('\uCF89');
		charS.add('\uCF8A');
		charS.add('\uCF8B');
		charS.add('\uCF8C');
		charS.add('\uCF8D');
		charS.add('\uCF8E');
		charS.add('\uCF8F');
		charS.add('\uCF90');
		charS.add('\uCF91');
		charS.add('\uCF92');
		charS.add('\uCF93');
		charS.add('\uCF94');
		charS.add('\uCF95');
		charS.add('\uCF96');
		charS.add('\uCF97');
		charS.add('\uCF98');
		charS.add('\uCF99');
		charS.add('\uCF9A');
		charS.add('\uCF9B');
		charS.add('\uCF9C');
		charS.add('\uCF9D');
		charS.add('\uCF9E');
		charS.add('\uCF9F');
		charS.add('\uCFA0');
		charS.add('\uCFA1');
		charS.add('\uCFA2');
		charS.add('\uCFA3');
		charS.add('\uCFA4');
		charS.add('\uCFA5');
		charS.add('\uCFA6');
		charS.add('\uCFA7');
		charS.add('\uCFA8');
		charS.add('\uCFA9');
		charS.add('\uCFAA');
		charS.add('\uCFAB');
		charS.add('\uCFAC');
		charS.add('\uCFAD');
		charS.add('\uCFAE');
		charS.add('\uCFAF');
		charS.add('\uCFB0');
		charS.add('\uCFB1');
		charS.add('\uCFB2');
		charS.add('\uCFB3');
		charS.add('\uCFB4');
		charS.add('\uCFB5');
		charS.add('\uCFB6');
		charS.add('\uCFB7');
		charS.add('\uCFB8');
		charS.add('\uCFB9');
		charS.add('\uCFBA');
		charS.add('\uCFBB');
		charS.add('\uCFBC');
		charS.add('\uCF3D');
		charS.add('\uCFBE');
		charS.add('\uCFBF');
		charS.add('\uCFC0');
		charS.add('\uCFC1');
		charS.add('\uCFC2');
		charS.add('\uCFC3');
		charS.add('\uCFC4');
		charS.add('\uCFC5');
		charS.add('\uCFC6');
		charS.add('\uCFC7');
		charS.add('\uCFC8');
		charS.add('\uCFC9');
		charS.add('\uCFCA');
		charS.add('\uCFCB');
		charS.add('\uCFCC');
		charS.add('\uCFCD');
		charS.add('\uCFCE');
		charS.add('\uCFCF');
		charS.add('\uCFD0');
		charS.add('\uCFD1');
		charS.add('\uCFD2');
		charS.add('\uCFD3');
		charS.add('\uCFD4');
		charS.add('\uCFD5');
		charS.add('\uCFD6');
		charS.add('\uCFD7');
		charS.add('\uCFD8');
		charS.add('\uCFD9');
		charS.add('\uCFDA');
		charS.add('\uCFDB');
		charS.add('\uCFDC');
		charS.add('\uCFDD');
		charS.add('\uCFDE');
		charS.add('\uCFDF');
		charS.add('\uCFE0');
		charS.add('\uCFE1');
		charS.add('\uCFE2');
		charS.add('\uCFE3');
		charS.add('\uCFE4');
		charS.add('\uCFE5');
		charS.add('\uCFE6');
		charS.add('\uCFE7');
		charS.add('\uCFE8');
		charS.add('\uCFE9');
		charS.add('\uCFEA');
		charS.add('\uCFEB');
		charS.add('\uCFEC');
		charS.add('\uCFED');
		charS.add('\uCFEE');
		charS.add('\uCFEF');
		charS.add('\uCFF0');
		charS.add('\uCFF1');
		charS.add('\uCFF2');
		charS.add('\uCFF3');
		charS.add('\uCFF4');
		charS.add('\uCFF5');
		charS.add('\uCFF6');
		charS.add('\uCFF7');
		charS.add('\uCFF8');
		charS.add('\uCFF9');
		charS.add('\uCFFA');
		charS.add('\uCFFB');
		charS.add('\uCFFC');
		charS.add('\uCFFD');
		charS.add('\uCFFE');
		charS.add('\uCFFF');
		
		return charS;
	}
	
	public static List<Character> addCharEntryD()
	{
		ArrayList<Character> charS = new ArrayList<Character>();
		
		charS.add('\uD000');
		charS.add('\uD001');
		charS.add('\uD002');
		charS.add('\uD003');
		charS.add('\uD004');
		charS.add('\uD005');
		charS.add('\uD006');
		charS.add('\uD007');
		charS.add('\uD008');
		charS.add('\uD009');
		charS.add('\uD00A');
		charS.add('\uD00B');
		charS.add('\uD00C');
		charS.add('\uD00D');
		charS.add('\uD00E');
		charS.add('\uD00F');
		charS.add('\uD010');
		charS.add('\uD011');
		charS.add('\uD012');
		charS.add('\uD013');
		charS.add('\uD014');
		charS.add('\uD015');
		charS.add('\uD016');
		charS.add('\uD017');
		charS.add('\uD018');
		charS.add('\uD019');
		charS.add('\uD01A');
		charS.add('\uD01B');
		charS.add('\uD01C');
		charS.add('\uD01D');
		charS.add('\uD01E');
		charS.add('\uD01F');
		charS.add('\uD020');
		charS.add('\uD021');
		charS.add('\uD022');
		charS.add('\uD023');
		charS.add('\uD024');
		charS.add('\uD025');
		charS.add('\uD026');
		charS.add('\uD027');
		charS.add('\uD028');
		charS.add('\uD029');
		charS.add('\uD02A');
		charS.add('\uD02B');
		charS.add('\uD02C');
		charS.add('\uD02D');
		charS.add('\uD02E');
		charS.add('\uD02F');
		charS.add('\uD030');
		charS.add('\uD031');
		charS.add('\uD032');
		charS.add('\uD033');
		charS.add('\uD034');
		charS.add('\uD035');
		charS.add('\uD036');
		charS.add('\uD037');
		charS.add('\uD038');
		charS.add('\uD039');
		charS.add('\uD03A');
		charS.add('\uD03B');
		charS.add('\uD03C');
		charS.add('\uD03D');
		charS.add('\uD03E');
		charS.add('\uD03F');
		charS.add('\uD040');
		charS.add('\uD041');
		charS.add('\uD042');
		charS.add('\uD043');
		charS.add('\uD044');
		charS.add('\uD045');
		charS.add('\uD046');
		charS.add('\uD047');
		charS.add('\uD048');
		charS.add('\uD049');
		charS.add('\uD04A');
		charS.add('\uD04B');
		charS.add('\uD04C');
		charS.add('\uD04D');
		charS.add('\uD04E');
		charS.add('\uD04F');
		charS.add('\uD050');
		charS.add('\uD051');
		charS.add('\uD052');
		charS.add('\uD053');
		charS.add('\uD054');
		charS.add('\uD055');
		charS.add('\uD056');
		charS.add('\uD057');
		charS.add('\uD058');
		charS.add('\uD059');
		charS.add('\uD05A');
		charS.add('\uD05B');
		charS.add('\uD05C');
		charS.add('\uD05D');
		charS.add('\uD05E');
		charS.add('\uD05F');
		charS.add('\uD060');
		charS.add('\uD061');
		charS.add('\uD062');
		charS.add('\uD063');
		charS.add('\uD064');
		charS.add('\uD065');
		charS.add('\uD066');
		charS.add('\uD067');
		charS.add('\uD068');
		charS.add('\uD069');
		charS.add('\uD06A');
		charS.add('\uD06B');
		charS.add('\uD06C');
		charS.add('\uD06D');
		charS.add('\uD06E');
		charS.add('\uD06F');
		charS.add('\uD070');
		charS.add('\uD071');
		charS.add('\uD072');
		charS.add('\uD073');
		charS.add('\uD074');
		charS.add('\uD075');
		charS.add('\uD076');
		charS.add('\uD077');
		charS.add('\uD078');
		charS.add('\uD079');
		charS.add('\uD07A');
		charS.add('\uD07B');
		charS.add('\uD07C');
		charS.add('\uD07D');
		charS.add('\uD07E');
		charS.add('\uD07F');
		charS.add('\uD080');
		charS.add('\uD081');
		charS.add('\uD082');
		charS.add('\uD083');
		charS.add('\uD084');
		charS.add('\uD085');
		charS.add('\uD086');
		charS.add('\uD087');
		charS.add('\uD088');
		charS.add('\uD089');
		charS.add('\uD08A');
		charS.add('\uD08B');
		charS.add('\uD08C');
		charS.add('\uD08D');
		charS.add('\uD08E');
		charS.add('\uD08F');
		charS.add('\uD090');
		charS.add('\uD091');
		charS.add('\uD092');
		charS.add('\uD093');
		charS.add('\uD094');
		charS.add('\uD095');
		charS.add('\uD096');
		charS.add('\uD097');
		charS.add('\uD098');
		charS.add('\uD099');
		charS.add('\uD09A');
		charS.add('\uD09B');
		charS.add('\uD09C');
		charS.add('\uD09D');
		charS.add('\uD09E');
		charS.add('\uD09F');
		charS.add('\uD0A0');
		charS.add('\uD0A1');
		charS.add('\uD0A2');
		charS.add('\uD0A3');
		charS.add('\uD0A4');
		charS.add('\uD0A5');
		charS.add('\uD0A6');
		charS.add('\uD0A7');
		charS.add('\uD0A8');
		charS.add('\uD0A9');
		charS.add('\uD0AA');
		charS.add('\uD0AB');
		charS.add('\uD0AC');
		charS.add('\uD0AD');
		charS.add('\uD0AE');
		charS.add('\uD0AF');
		charS.add('\uD0B0');
		charS.add('\uD0B1');
		charS.add('\uD0B2');
		charS.add('\uD0B3');
		charS.add('\uD0B4');
		charS.add('\uD0B5');
		charS.add('\uD0B6');
		charS.add('\uD0B7');
		charS.add('\uD0B8');
		charS.add('\uD0B9');
		charS.add('\uD0BA');
		charS.add('\uD0BB');
		charS.add('\uD0BC');
		charS.add('\uD03D');
		charS.add('\uD0BE');
		charS.add('\uD0BF');
		charS.add('\uD0C0');
		charS.add('\uD0C1');
		charS.add('\uD0C2');
		charS.add('\uD0C3');
		charS.add('\uD0C4');
		charS.add('\uD0C5');
		charS.add('\uD0C6');
		charS.add('\uD0C7');
		charS.add('\uD0C8');
		charS.add('\uD0C9');
		charS.add('\uD0CA');
		charS.add('\uD0CB');
		charS.add('\uD0CC');
		charS.add('\uD0CD');
		charS.add('\uD0CE');
		charS.add('\uD0CF');
		charS.add('\uD0D0');
		charS.add('\uD0D1');
		charS.add('\uD0D2');
		charS.add('\uD0D3');
		charS.add('\uD0D4');
		charS.add('\uD0D5');
		charS.add('\uD0D6');
		charS.add('\uD0D7');
		charS.add('\uD0D8');
		charS.add('\uD0D9');
		charS.add('\uD0DA');
		charS.add('\uD0DB');
		charS.add('\uD0DC');
		charS.add('\uD0DD');
		charS.add('\uD0DE');
		charS.add('\uD0DF');
		charS.add('\uD0E0');
		charS.add('\uD0E1');
		charS.add('\uD0E2');
		charS.add('\uD0E3');
		charS.add('\uD0E4');
		charS.add('\uD0E5');
		charS.add('\uD0E6');
		charS.add('\uD0E7');
		charS.add('\uD0E8');
		charS.add('\uD0E9');
		charS.add('\uD0EA');
		charS.add('\uD0EB');
		charS.add('\uD0EC');
		charS.add('\uD0ED');
		charS.add('\uD0EE');
		charS.add('\uD0EF');
		charS.add('\uD0F0');
		charS.add('\uD0F1');
		charS.add('\uD0F2');
		charS.add('\uD0F3');
		charS.add('\uD0F4');
		charS.add('\uD0F5');
		charS.add('\uD0F6');
		charS.add('\uD0F7');
		charS.add('\uD0F8');
		charS.add('\uD0F9');
		charS.add('\uD0FA');
		charS.add('\uD0FB');
		charS.add('\uD0FC');
		charS.add('\uD0FD');
		charS.add('\uD0FE');
		charS.add('\uD0FF');
		charS.add('\uD0E8');
		charS.add('\uD0E9');
		charS.add('\uD0EA');
		charS.add('\uD0EB');
		charS.add('\uD0EC');
		charS.add('\uD0ED');
		charS.add('\uD0EE');
		charS.add('\uD0EF');
		charS.add('\uD0F0');
		charS.add('\uD0F1');
		charS.add('\uD0F2');
		charS.add('\uD0F3');
		charS.add('\uD0F4');
		charS.add('\uD0F5');
		charS.add('\uD0F6');
		charS.add('\uD0F7');
		charS.add('\uD0F8');
		charS.add('\uD0F9');
		charS.add('\uD0FA');
		charS.add('\uD0FB');
		charS.add('\uD0FC');
		charS.add('\uD0FD');
		charS.add('\uD0FE');
		charS.add('\uD0FF');
		charS.add('\uD100');
		charS.add('\uD101');
		charS.add('\uD102');
		charS.add('\uD103');
		charS.add('\uD104');
		charS.add('\uD105');
		charS.add('\uD106');
		charS.add('\uD107');
		charS.add('\uD108');
		charS.add('\uD109');
		charS.add('\uD10A');
		charS.add('\uD10B');
		charS.add('\uD10C');
		charS.add('\uD10D');
		charS.add('\uD10E');
		charS.add('\uD10F');
		charS.add('\uD110');
		charS.add('\uD111');
		charS.add('\uD112');
		charS.add('\uD113');
		charS.add('\uD114');
		charS.add('\uD115');
		charS.add('\uD116');
		charS.add('\uD117');
		charS.add('\uD118');
		charS.add('\uD119');
		charS.add('\uD11A');
		charS.add('\uD11B');
		charS.add('\uD11C');
		charS.add('\uD11D');
		charS.add('\uD11E');
		charS.add('\uD11F');
		charS.add('\uD120');
		charS.add('\uD121');
		charS.add('\uD122');
		charS.add('\uD123');
		charS.add('\uD124');
		charS.add('\uD125');
		charS.add('\uD126');
		charS.add('\uD127');
		charS.add('\uD128');
		charS.add('\uD129');
		charS.add('\uD12A');
		charS.add('\uD12B');
		charS.add('\uD12C');
		charS.add('\uD12D');
		charS.add('\uD12E');
		charS.add('\uD12F');
		charS.add('\uD130');
		charS.add('\uD131');
		charS.add('\uD132');
		charS.add('\uD133');
		charS.add('\uD134');
		charS.add('\uD135');
		charS.add('\uD136');
		charS.add('\uD137');
		charS.add('\uD138');
		charS.add('\uD139');
		charS.add('\uD13A');
		charS.add('\uD13B');
		charS.add('\uD13C');
		charS.add('\uD13D');
		charS.add('\uD13E');
		charS.add('\uD13F');
		charS.add('\uD140');
		charS.add('\uD141');
		charS.add('\uD142');
		charS.add('\uD143');
		charS.add('\uD144');
		charS.add('\uD145');
		charS.add('\uD146');
		charS.add('\uD147');
		charS.add('\uD148');
		charS.add('\uD149');
		charS.add('\uD14A');
		charS.add('\uD14B');
		charS.add('\uD14C');
		charS.add('\uD14D');
		charS.add('\uD14E');
		charS.add('\uD14F');
		charS.add('\uD150');
		charS.add('\uD151');
		charS.add('\uD152');
		charS.add('\uD153');
		charS.add('\uD154');
		charS.add('\uD155');
		charS.add('\uD156');
		charS.add('\uD157');
		charS.add('\uD158');
		charS.add('\uD159');
		charS.add('\uD15A');
		charS.add('\uD15B');
		charS.add('\uD15C');
		charS.add('\uD15D');
		charS.add('\uD15E');
		charS.add('\uD15F');
		charS.add('\uD160');
		charS.add('\uD161');
		charS.add('\uD162');
		charS.add('\uD163');
		charS.add('\uD164');
		charS.add('\uD165');
		charS.add('\uD166');
		charS.add('\uD167');
		charS.add('\uD168');
		charS.add('\uD169');
		charS.add('\uD16A');
		charS.add('\uD16B');
		charS.add('\uD16C');
		charS.add('\uD16D');
		charS.add('\uD16E');
		charS.add('\uD16F');
		charS.add('\uD170');
		charS.add('\uD171');
		charS.add('\uD172');
		charS.add('\uD173');
		charS.add('\uD174');
		charS.add('\uD175');
		charS.add('\uD176');
		charS.add('\uD177');
		charS.add('\uD178');
		charS.add('\uD179');
		charS.add('\uD17A');
		charS.add('\uD17B');
		charS.add('\uD17C');
		charS.add('\uD17D');
		charS.add('\uD17E');
		charS.add('\uD17F');
		charS.add('\uD180');
		charS.add('\uD181');
		charS.add('\uD182');
		charS.add('\uD183');
		charS.add('\uD184');
		charS.add('\uD185');
		charS.add('\uD186');
		charS.add('\uD187');
		charS.add('\uD188');
		charS.add('\uD189');
		charS.add('\uD18A');
		charS.add('\uD18B');
		charS.add('\uD18C');
		charS.add('\uD18D');
		charS.add('\uD18E');
		charS.add('\uD18F');
		charS.add('\uD190');
		charS.add('\uD191');
		charS.add('\uD192');
		charS.add('\uD193');
		charS.add('\uD194');
		charS.add('\uD195');
		charS.add('\uD196');
		charS.add('\uD197');
		charS.add('\uD198');
		charS.add('\uD199');
		charS.add('\uD19A');
		charS.add('\uD19B');
		charS.add('\uD19C');
		charS.add('\uD19D');
		charS.add('\uD19E');
		charS.add('\uD19F');
		charS.add('\uD1A0');
		charS.add('\uD1A1');
		charS.add('\uD1A2');
		charS.add('\uD1A3');
		charS.add('\uD1A4');
		charS.add('\uD1A5');
		charS.add('\uD1A6');
		charS.add('\uD1A7');
		charS.add('\uD1A8');
		charS.add('\uD1A9');
		charS.add('\uD1AA');
		charS.add('\uD1AB');
		charS.add('\uD1AC');
		charS.add('\uD1AD');
		charS.add('\uD1AE');
		charS.add('\uD1AF');
		charS.add('\uD1B0');
		charS.add('\uD1B1');
		charS.add('\uD1B2');
		charS.add('\uD1B3');
		charS.add('\uD1B4');
		charS.add('\uD1B5');
		charS.add('\uD1B6');
		charS.add('\uD1B7');
		charS.add('\uD1B8');
		charS.add('\uD1B9');
		charS.add('\uD1BA');
		charS.add('\uD1BB');
		charS.add('\uD1BC');
		charS.add('\uD13D');
		charS.add('\uD1BE');
		charS.add('\uD1BF');
		charS.add('\uD1C0');
		charS.add('\uD1C1');
		charS.add('\uD1C2');
		charS.add('\uD1C3');
		charS.add('\uD1C4');
		charS.add('\uD1C5');
		charS.add('\uD1C6');
		charS.add('\uD1C7');
		charS.add('\uD1C8');
		charS.add('\uD1C9');
		charS.add('\uD1CA');
		charS.add('\uD1CB');
		charS.add('\uD1CC');
		charS.add('\uD1CD');
		charS.add('\uD1CE');
		charS.add('\uD1CF');
		charS.add('\uD1D0');
		charS.add('\uD1D1');
		charS.add('\uD1D2');
		charS.add('\uD1D3');
		charS.add('\uD1D4');
		charS.add('\uD1D5');
		charS.add('\uD1D6');
		charS.add('\uD1D7');
		charS.add('\uD1D8');
		charS.add('\uD1D9');
		charS.add('\uD1DA');
		charS.add('\uD1DB');
		charS.add('\uD1DC');
		charS.add('\uD1DD');
		charS.add('\uD1DE');
		charS.add('\uD1DF');
		charS.add('\uD1E0');
		charS.add('\uD1E1');
		charS.add('\uD1E2');
		charS.add('\uD1E3');
		charS.add('\uD1E4');
		charS.add('\uD1E5');
		charS.add('\uD1E6');
		charS.add('\uD1E7');
		charS.add('\uD1E8');
		charS.add('\uD1E9');
		charS.add('\uD1EA');
		charS.add('\uD1EB');
		charS.add('\uD1EC');
		charS.add('\uD1ED');
		charS.add('\uD1EE');
		charS.add('\uD1EF');
		charS.add('\uD1F0');
		charS.add('\uD1F1');
		charS.add('\uD1F2');
		charS.add('\uD1F3');
		charS.add('\uD1F4');
		charS.add('\uD1F5');
		charS.add('\uD1F6');
		charS.add('\uD1F7');
		charS.add('\uD1F8');
		charS.add('\uD1F9');
		charS.add('\uD1FA');
		charS.add('\uD1FB');
		charS.add('\uD1FC');
		charS.add('\uD1FD');
		charS.add('\uD1FE');
		charS.add('\uD1FF');
		charS.add('\uD200');
		charS.add('\uD201');
		charS.add('\uD202');
		charS.add('\uD203');
		charS.add('\uD204');
		charS.add('\uD205');
		charS.add('\uD206');
		charS.add('\uD207');
		charS.add('\uD208');
		charS.add('\uD209');
		charS.add('\uD20A');
		charS.add('\uD20B');
		charS.add('\uD20C');
		charS.add('\uD20D');
		charS.add('\uD20E');
		charS.add('\uD20F');
		charS.add('\uD210');
		charS.add('\uD211');
		charS.add('\uD212');
		charS.add('\uD213');
		charS.add('\uD214');
		charS.add('\uD215');
		charS.add('\uD216');
		charS.add('\uD217');
		charS.add('\uD218');
		charS.add('\uD219');
		charS.add('\uD21A');
		charS.add('\uD21B');
		charS.add('\uD21C');
		charS.add('\uD21D');
		charS.add('\uD21E');
		charS.add('\uD21F');
		charS.add('\uD220');
		charS.add('\uD221');
		charS.add('\uD222');
		charS.add('\uD223');
		charS.add('\uD224');
		charS.add('\uD225');
		charS.add('\uD226');
		charS.add('\uD227');
		charS.add('\uD228');
		charS.add('\uD229');
		charS.add('\uD22A');
		charS.add('\uD22B');
		charS.add('\uD22C');
		charS.add('\uD22D');
		charS.add('\uD22E');
		charS.add('\uD22F');
		charS.add('\uD230');
		charS.add('\uD231');
		charS.add('\uD232');
		charS.add('\uD233');
		charS.add('\uD234');
		charS.add('\uD235');
		charS.add('\uD236');
		charS.add('\uD237');
		charS.add('\uD238');
		charS.add('\uD239');
		charS.add('\uD23A');
		charS.add('\uD23B');
		charS.add('\uD23C');
		charS.add('\uD23D');
		charS.add('\uD23E');
		charS.add('\uD23F');
		charS.add('\uD240');
		charS.add('\uD241');
		charS.add('\uD242');
		charS.add('\uD243');
		charS.add('\uD244');
		charS.add('\uD245');
		charS.add('\uD246');
		charS.add('\uD247');
		charS.add('\uD248');
		charS.add('\uD249');
		charS.add('\uD24A');
		charS.add('\uD24B');
		charS.add('\uD24C');
		charS.add('\uD24D');
		charS.add('\uD24E');
		charS.add('\uD24F');
		charS.add('\uD250');
		charS.add('\uD251');
		charS.add('\uD252');
		charS.add('\uD253');
		charS.add('\uD254');
		charS.add('\uD255');
		charS.add('\uD256');
		charS.add('\uD257');
		charS.add('\uD258');
		charS.add('\uD259');
		charS.add('\uD25A');
		charS.add('\uD25B');
		charS.add('\uD25C');
		charS.add('\uD25D');
		charS.add('\uD25E');
		charS.add('\uD25F');
		charS.add('\uD260');
		charS.add('\uD261');
		charS.add('\uD262');
		charS.add('\uD263');
		charS.add('\uD264');
		charS.add('\uD265');
		charS.add('\uD266');
		charS.add('\uD267');
		charS.add('\uD268');
		charS.add('\uD269');
		charS.add('\uD26A');
		charS.add('\uD26B');
		charS.add('\uD26C');
		charS.add('\uD26D');
		charS.add('\uD26E');
		charS.add('\uD26F');
		charS.add('\uD270');
		charS.add('\uD271');
		charS.add('\uD272');
		charS.add('\uD273');
		charS.add('\uD274');
		charS.add('\uD275');
		charS.add('\uD276');
		charS.add('\uD277');
		charS.add('\uD278');
		charS.add('\uD279');
		charS.add('\uD27A');
		charS.add('\uD27B');
		charS.add('\uD27C');
		charS.add('\uD27D');
		charS.add('\uD27E');
		charS.add('\uD27F');
		charS.add('\uD280');
		charS.add('\uD281');
		charS.add('\uD282');
		charS.add('\uD283');
		charS.add('\uD284');
		charS.add('\uD285');
		charS.add('\uD286');
		charS.add('\uD287');
		charS.add('\uD288');
		charS.add('\uD289');
		charS.add('\uD28A');
		charS.add('\uD28B');
		charS.add('\uD28C');
		charS.add('\uD28D');
		charS.add('\uD28E');
		charS.add('\uD28F');
		charS.add('\uD290');
		charS.add('\uD291');
		charS.add('\uD292');
		charS.add('\uD293');
		charS.add('\uD294');
		charS.add('\uD295');
		charS.add('\uD296');
		charS.add('\uD297');
		charS.add('\uD298');
		charS.add('\uD299');
		charS.add('\uD29A');
		charS.add('\uD29B');
		charS.add('\uD29C');
		charS.add('\uD29D');
		charS.add('\uD29E');
		charS.add('\uD29F');
		charS.add('\uD2A0');
		charS.add('\uD2A1');
		charS.add('\uD2A2');
		charS.add('\uD2A3');
		charS.add('\uD2A4');
		charS.add('\uD2A5');
		charS.add('\uD2A6');
		charS.add('\uD2A7');
		charS.add('\uD2A8');
		charS.add('\uD2A9');
		charS.add('\uD2AA');
		charS.add('\uD2AB');
		charS.add('\uD2AC');
		charS.add('\uD2AD');
		charS.add('\uD2AE');
		charS.add('\uD2AF');
		charS.add('\uD2B0');
		charS.add('\uD2B1');
		charS.add('\uD2B2');
		charS.add('\uD2B3');
		charS.add('\uD2B4');
		charS.add('\uD2B5');
		charS.add('\uD2B6');
		charS.add('\uD2B7');
		charS.add('\uD2B8');
		charS.add('\uD2B9');
		charS.add('\uD2BA');
		charS.add('\uD2BB');
		charS.add('\uD2BC');
		charS.add('\uD23D');
		charS.add('\uD2BE');
		charS.add('\uD2BF');
		charS.add('\uD2C0');
		charS.add('\uD2C1');
		charS.add('\uD2C2');
		charS.add('\uD2C3');
		charS.add('\uD2C4');
		charS.add('\uD2C5');
		charS.add('\uD2C6');
		charS.add('\uD2C7');
		charS.add('\uD2C8');
		charS.add('\uD2C9');
		charS.add('\uD2CA');
		charS.add('\uD2CB');
		charS.add('\uD2CC');
		charS.add('\uD2CD');
		charS.add('\uD2CE');
		charS.add('\uD2CF');
		charS.add('\uD2D0');
		charS.add('\uD2D1');
		charS.add('\uD2D2');
		charS.add('\uD2D3');
		charS.add('\uD2D4');
		charS.add('\uD2D5');
		charS.add('\uD2D6');
		charS.add('\uD2D7');
		charS.add('\uD2D8');
		charS.add('\uD2D9');
		charS.add('\uD2DA');
		charS.add('\uD2DB');
		charS.add('\uD2DC');
		charS.add('\uD2DD');
		charS.add('\uD2DE');
		charS.add('\uD2DF');
		charS.add('\uD2E0');
		charS.add('\uD2E1');
		charS.add('\uD2E2');
		charS.add('\uD2E3');
		charS.add('\uD2E4');
		charS.add('\uD2E5');
		charS.add('\uD2E6');
		charS.add('\uD2E7');
		charS.add('\uD2E8');
		charS.add('\uD2E9');
		charS.add('\uD2EA');
		charS.add('\uD2EB');
		charS.add('\uD2EC');
		charS.add('\uD2ED');
		charS.add('\uD2EE');
		charS.add('\uD2EF');
		charS.add('\uD2F0');
		charS.add('\uD2F1');
		charS.add('\uD2F2');
		charS.add('\uD2F3');
		charS.add('\uD2F4');
		charS.add('\uD2F5');
		charS.add('\uD2F6');
		charS.add('\uD2F7');
		charS.add('\uD2F8');
		charS.add('\uD2F9');
		charS.add('\uD2FA');
		charS.add('\uD2FB');
		charS.add('\uD2FC');
		charS.add('\uD2FD');
		charS.add('\uD2FE');
		charS.add('\uD2FF');
		charS.add('\uD300');
		charS.add('\uD301');
		charS.add('\uD302');
		charS.add('\uD303');
		charS.add('\uD304');
		charS.add('\uD305');
		charS.add('\uD306');
		charS.add('\uD307');
		charS.add('\uD308');
		charS.add('\uD309');
		charS.add('\uD30A');
		charS.add('\uD30B');
		charS.add('\uD30C');
		charS.add('\uD30D');
		charS.add('\uD30E');
		charS.add('\uD30F');
		charS.add('\uD310');
		charS.add('\uD311');
		charS.add('\uD312');
		charS.add('\uD313');
		charS.add('\uD314');
		charS.add('\uD315');
		charS.add('\uD316');
		charS.add('\uD317');
		charS.add('\uD318');
		charS.add('\uD319');
		charS.add('\uD31A');
		charS.add('\uD31B');
		charS.add('\uD31C');
		charS.add('\uD31D');
		charS.add('\uD31E');
		charS.add('\uD31F');
		charS.add('\uD320');
		charS.add('\uD321');
		charS.add('\uD322');
		charS.add('\uD323');
		charS.add('\uD324');
		charS.add('\uD325');
		charS.add('\uD326');
		charS.add('\uD327');
		charS.add('\uD328');
		charS.add('\uD329');
		charS.add('\uD32A');
		charS.add('\uD32B');
		charS.add('\uD32C');
		charS.add('\uD32D');
		charS.add('\uD32E');
		charS.add('\uD32F');
		charS.add('\uD330');
		charS.add('\uD331');
		charS.add('\uD332');
		charS.add('\uD333');
		charS.add('\uD334');
		charS.add('\uD335');
		charS.add('\uD336');
		charS.add('\uD337');
		charS.add('\uD338');
		charS.add('\uD339');
		charS.add('\uD33A');
		charS.add('\uD33B');
		charS.add('\uD33C');
		charS.add('\uD33D');
		charS.add('\uD33E');
		charS.add('\uD33F');
		charS.add('\uD340');
		charS.add('\uD341');
		charS.add('\uD342');
		charS.add('\uD343');
		charS.add('\uD344');
		charS.add('\uD345');
		charS.add('\uD346');
		charS.add('\uD347');
		charS.add('\uD348');
		charS.add('\uD349');
		charS.add('\uD34A');
		charS.add('\uD34B');
		charS.add('\uD34C');
		charS.add('\uD34D');
		charS.add('\uD34E');
		charS.add('\uD34F');
		charS.add('\uD350');
		charS.add('\uD351');
		charS.add('\uD352');
		charS.add('\uD353');
		charS.add('\uD354');
		charS.add('\uD355');
		charS.add('\uD356');
		charS.add('\uD357');
		charS.add('\uD358');
		charS.add('\uD359');
		charS.add('\uD35A');
		charS.add('\uD35B');
		charS.add('\uD35C');
		charS.add('\uD35D');
		charS.add('\uD35E');
		charS.add('\uD35F');
		charS.add('\uD360');
		charS.add('\uD361');
		charS.add('\uD362');
		charS.add('\uD363');
		charS.add('\uD364');
		charS.add('\uD365');
		charS.add('\uD366');
		charS.add('\uD367');
		charS.add('\uD368');
		charS.add('\uD369');
		charS.add('\uD36A');
		charS.add('\uD36B');
		charS.add('\uD36C');
		charS.add('\uD36D');
		charS.add('\uD36E');
		charS.add('\uD36F');
		charS.add('\uD370');
		charS.add('\uD371');
		charS.add('\uD372');
		charS.add('\uD373');
		charS.add('\uD374');
		charS.add('\uD375');
		charS.add('\uD376');
		charS.add('\uD377');
		charS.add('\uD378');
		charS.add('\uD379');
		charS.add('\uD37A');
		charS.add('\uD37B');
		charS.add('\uD37C');
		charS.add('\uD37D');
		charS.add('\uD37E');
		charS.add('\uD37F');
		charS.add('\uD380');
		charS.add('\uD381');
		charS.add('\uD382');
		charS.add('\uD383');
		charS.add('\uD384');
		charS.add('\uD385');
		charS.add('\uD386');
		charS.add('\uD387');
		charS.add('\uD388');
		charS.add('\uD389');
		charS.add('\uD38A');
		charS.add('\uD38B');
		charS.add('\uD38C');
		charS.add('\uD38D');
		charS.add('\uD38E');
		charS.add('\uD38F');
		charS.add('\uD390');
		charS.add('\uD391');
		charS.add('\uD392');
		charS.add('\uD393');
		charS.add('\uD394');
		charS.add('\uD395');
		charS.add('\uD396');
		charS.add('\uD397');
		charS.add('\uD398');
		charS.add('\uD399');
		charS.add('\uD39A');
		charS.add('\uD39B');
		charS.add('\uD39C');
		charS.add('\uD39D');
		charS.add('\uD39E');
		charS.add('\uD39F');
		charS.add('\uD3A0');
		charS.add('\uD3A1');
		charS.add('\uD3A2');
		charS.add('\uD3A3');
		charS.add('\uD3A4');
		charS.add('\uD3A5');
		charS.add('\uD3A6');
		charS.add('\uD3A7');
		charS.add('\uD3A8');
		charS.add('\uD3A9');
		charS.add('\uD3AA');
		charS.add('\uD3AB');
		charS.add('\uD3AC');
		charS.add('\uD3AD');
		charS.add('\uD3AE');
		charS.add('\uD3AF');
		charS.add('\uD3B0');
		charS.add('\uD3B1');
		charS.add('\uD3B2');
		charS.add('\uD3B3');
		charS.add('\uD3B4');
		charS.add('\uD3B5');
		charS.add('\uD3B6');
		charS.add('\uD3B7');
		charS.add('\uD3B8');
		charS.add('\uD3B9');
		charS.add('\uD3BA');
		charS.add('\uD3BB');
		charS.add('\uD3BC');
		charS.add('\uD33D');
		charS.add('\uD3BE');
		charS.add('\uD3BF');
		charS.add('\uD3C0');
		charS.add('\uD3C1');
		charS.add('\uD3C2');
		charS.add('\uD3C3');
		charS.add('\uD3C4');
		charS.add('\uD3C5');
		charS.add('\uD3C6');
		charS.add('\uD3C7');
		charS.add('\uD3C8');
		charS.add('\uD3C9');
		charS.add('\uD3CA');
		charS.add('\uD3CB');
		charS.add('\uD3CC');
		charS.add('\uD3CD');
		charS.add('\uD3CE');
		charS.add('\uD3CF');
		charS.add('\uD3D0');
		charS.add('\uD3D1');
		charS.add('\uD3D2');
		charS.add('\uD3D3');
		charS.add('\uD3D4');
		charS.add('\uD3D5');
		charS.add('\uD3D6');
		charS.add('\uD3D7');
		charS.add('\uD3D8');
		charS.add('\uD3D9');
		charS.add('\uD3DA');
		charS.add('\uD3DB');
		charS.add('\uD3DC');
		charS.add('\uD3DD');
		charS.add('\uD3DE');
		charS.add('\uD3DF');
		charS.add('\uD3E0');
		charS.add('\uD3E1');
		charS.add('\uD3E2');
		charS.add('\uD3E3');
		charS.add('\uD3E4');
		charS.add('\uD3E5');
		charS.add('\uD3E6');
		charS.add('\uD3E7');
		charS.add('\uD3E8');
		charS.add('\uD3E9');
		charS.add('\uD3EA');
		charS.add('\uD3EB');
		charS.add('\uD3EC');
		charS.add('\uD3ED');
		charS.add('\uD3EE');
		charS.add('\uD3EF');
		charS.add('\uD3F0');
		charS.add('\uD3F1');
		charS.add('\uD3F2');
		charS.add('\uD3F3');
		charS.add('\uD3F4');
		charS.add('\uD3F5');
		charS.add('\uD3F6');
		charS.add('\uD3F7');
		charS.add('\uD3F8');
		charS.add('\uD3F9');
		charS.add('\uD3FA');
		charS.add('\uD3FB');
		charS.add('\uD3FC');
		charS.add('\uD3FD');
		charS.add('\uD3FE');
		charS.add('\uD3FF');
		charS.add('\uD400');
		charS.add('\uD401');
		charS.add('\uD402');
		charS.add('\uD403');
		charS.add('\uD404');
		charS.add('\uD405');
		charS.add('\uD406');
		charS.add('\uD407');
		charS.add('\uD408');
		charS.add('\uD409');
		charS.add('\uD40A');
		charS.add('\uD40B');
		charS.add('\uD40C');
		charS.add('\uD40D');
		charS.add('\uD40E');
		charS.add('\uD40F');
		charS.add('\uD410');
		charS.add('\uD411');
		charS.add('\uD412');
		charS.add('\uD413');
		charS.add('\uD414');
		charS.add('\uD415');
		charS.add('\uD416');
		charS.add('\uD417');
		charS.add('\uD418');
		charS.add('\uD419');
		charS.add('\uD41A');
		charS.add('\uD41B');
		charS.add('\uD41C');
		charS.add('\uD41D');
		charS.add('\uD41E');
		charS.add('\uD41F');
		charS.add('\uD420');
		charS.add('\uD421');
		charS.add('\uD422');
		charS.add('\uD423');
		charS.add('\uD424');
		charS.add('\uD425');
		charS.add('\uD426');
		charS.add('\uD427');
		charS.add('\uD428');
		charS.add('\uD429');
		charS.add('\uD42A');
		charS.add('\uD42B');
		charS.add('\uD42C');
		charS.add('\uD42D');
		charS.add('\uD42E');
		charS.add('\uD42F');
		charS.add('\uD430');
		charS.add('\uD431');
		charS.add('\uD432');
		charS.add('\uD433');
		charS.add('\uD434');
		charS.add('\uD435');
		charS.add('\uD436');
		charS.add('\uD437');
		charS.add('\uD438');
		charS.add('\uD439');
		charS.add('\uD43A');
		charS.add('\uD43B');
		charS.add('\uD43C');
		charS.add('\uD43D');
		charS.add('\uD43E');
		charS.add('\uD43F');
		charS.add('\uD440');
		charS.add('\uD441');
		charS.add('\uD442');
		charS.add('\uD443');
		charS.add('\uD444');
		charS.add('\uD445');
		charS.add('\uD446');
		charS.add('\uD447');
		charS.add('\uD448');
		charS.add('\uD449');
		charS.add('\uD44A');
		charS.add('\uD44B');
		charS.add('\uD44C');
		charS.add('\uD44D');
		charS.add('\uD44E');
		charS.add('\uD44F');
		charS.add('\uD450');
		charS.add('\uD451');
		charS.add('\uD452');
		charS.add('\uD453');
		charS.add('\uD454');
		charS.add('\uD455');
		charS.add('\uD456');
		charS.add('\uD457');
		charS.add('\uD458');
		charS.add('\uD459');
		charS.add('\uD45A');
		charS.add('\uD45B');
		charS.add('\uD45C');
		charS.add('\uD45D');
		charS.add('\uD45E');
		charS.add('\uD45F');
		charS.add('\uD460');
		charS.add('\uD461');
		charS.add('\uD462');
		charS.add('\uD463');
		charS.add('\uD464');
		charS.add('\uD465');
		charS.add('\uD466');
		charS.add('\uD467');
		charS.add('\uD468');
		charS.add('\uD469');
		charS.add('\uD46A');
		charS.add('\uD46B');
		charS.add('\uD46C');
		charS.add('\uD46D');
		charS.add('\uD46E');
		charS.add('\uD46F');
		charS.add('\uD470');
		charS.add('\uD471');
		charS.add('\uD472');
		charS.add('\uD473');
		charS.add('\uD474');
		charS.add('\uD475');
		charS.add('\uD476');
		charS.add('\uD477');
		charS.add('\uD478');
		charS.add('\uD479');
		charS.add('\uD47A');
		charS.add('\uD47B');
		charS.add('\uD47C');
		charS.add('\uD47D');
		charS.add('\uD47E');
		charS.add('\uD47F');
		charS.add('\uD480');
		charS.add('\uD481');
		charS.add('\uD482');
		charS.add('\uD483');
		charS.add('\uD484');
		charS.add('\uD485');
		charS.add('\uD486');
		charS.add('\uD487');
		charS.add('\uD488');
		charS.add('\uD489');
		charS.add('\uD48A');
		charS.add('\uD48B');
		charS.add('\uD48C');
		charS.add('\uD48D');
		charS.add('\uD48E');
		charS.add('\uD48F');
		charS.add('\uD490');
		charS.add('\uD491');
		charS.add('\uD492');
		charS.add('\uD493');
		charS.add('\uD494');
		charS.add('\uD495');
		charS.add('\uD496');
		charS.add('\uD497');
		charS.add('\uD498');
		charS.add('\uD499');
		charS.add('\uD49A');
		charS.add('\uD49B');
		charS.add('\uD49C');
		charS.add('\uD49D');
		charS.add('\uD49E');
		charS.add('\uD49F');
		charS.add('\uD4A0');
		charS.add('\uD4A1');
		charS.add('\uD4A2');
		charS.add('\uD4A3');
		charS.add('\uD4A4');
		charS.add('\uD4A5');
		charS.add('\uD4A6');
		charS.add('\uD4A7');
		charS.add('\uD4A8');
		charS.add('\uD4A9');
		charS.add('\uD4AA');
		charS.add('\uD4AB');
		charS.add('\uD4AC');
		charS.add('\uD4AD');
		charS.add('\uD4AE');
		charS.add('\uD4AF');
		charS.add('\uD4B0');
		charS.add('\uD4B1');
		charS.add('\uD4B2');
		charS.add('\uD4B3');
		charS.add('\uD4B4');
		charS.add('\uD4B5');
		charS.add('\uD4B6');
		charS.add('\uD4B7');
		charS.add('\uD4B8');
		charS.add('\uD4B9');
		charS.add('\uD4BA');
		charS.add('\uD4BB');
		charS.add('\uD4BC');
		charS.add('\uD43D');
		charS.add('\uD4BE');
		charS.add('\uD4BF');
		charS.add('\uD4C0');
		charS.add('\uD4C1');
		charS.add('\uD4C2');
		charS.add('\uD4C3');
		charS.add('\uD4C4');
		charS.add('\uD4C5');
		charS.add('\uD4C6');
		charS.add('\uD4C7');
		charS.add('\uD4C8');
		charS.add('\uD4C9');
		charS.add('\uD4CA');
		charS.add('\uD4CB');
		charS.add('\uD4CC');
		charS.add('\uD4CD');
		charS.add('\uD4CE');
		charS.add('\uD4CF');
		charS.add('\uD4D0');
		charS.add('\uD4D1');
		charS.add('\uD4D2');
		charS.add('\uD4D3');
		charS.add('\uD4D4');
		charS.add('\uD4D5');
		charS.add('\uD4D6');
		charS.add('\uD4D7');
		charS.add('\uD4D8');
		charS.add('\uD4D9');
		charS.add('\uD4DA');
		charS.add('\uD4DB');
		charS.add('\uD4DC');
		charS.add('\uD4DD');
		charS.add('\uD4DE');
		charS.add('\uD4DF');
		charS.add('\uD4E0');
		charS.add('\uD4E1');
		charS.add('\uD4E2');
		charS.add('\uD4E3');
		charS.add('\uD4E4');
		charS.add('\uD4E5');
		charS.add('\uD4E6');
		charS.add('\uD4E7');
		charS.add('\uD4E8');
		charS.add('\uD4E9');
		charS.add('\uD4EA');
		charS.add('\uD4EB');
		charS.add('\uD4EC');
		charS.add('\uD4ED');
		charS.add('\uD4EE');
		charS.add('\uD4EF');
		charS.add('\uD4F0');
		charS.add('\uD4F1');
		charS.add('\uD4F2');
		charS.add('\uD4F3');
		charS.add('\uD4F4');
		charS.add('\uD4F5');
		charS.add('\uD4F6');
		charS.add('\uD4F7');
		charS.add('\uD4F8');
		charS.add('\uD4F9');
		charS.add('\uD4FA');
		charS.add('\uD4FB');
		charS.add('\uD4FC');
		charS.add('\uD4FD');
		charS.add('\uD4FE');
		charS.add('\uD4FF');
		charS.add('\uD500');
		charS.add('\uD501');
		charS.add('\uD502');
		charS.add('\uD503');
		charS.add('\uD504');
		charS.add('\uD505');
		charS.add('\uD506');
		charS.add('\uD507');
		charS.add('\uD508');
		charS.add('\uD509');
		charS.add('\uD50A');
		charS.add('\uD50B');
		charS.add('\uD50C');
		charS.add('\uD50D');
		charS.add('\uD50E');
		charS.add('\uD50F');
		charS.add('\uD510');
		charS.add('\uD511');
		charS.add('\uD512');
		charS.add('\uD513');
		charS.add('\uD514');
		charS.add('\uD515');
		charS.add('\uD516');
		charS.add('\uD517');
		charS.add('\uD518');
		charS.add('\uD519');
		charS.add('\uD51A');
		charS.add('\uD51B');
		charS.add('\uD51C');
		charS.add('\uD51D');
		charS.add('\uD51E');
		charS.add('\uD51F');
		charS.add('\uD520');
		charS.add('\uD521');
		charS.add('\uD522');
		charS.add('\uD523');
		charS.add('\uD524');
		charS.add('\uD525');
		charS.add('\uD526');
		charS.add('\uD527');
		charS.add('\uD528');
		charS.add('\uD529');
		charS.add('\uD52A');
		charS.add('\uD52B');
		charS.add('\uD52C');
		charS.add('\uD52D');
		charS.add('\uD52E');
		charS.add('\uD52F');
		charS.add('\uD530');
		charS.add('\uD531');
		charS.add('\uD532');
		charS.add('\uD533');
		charS.add('\uD534');
		charS.add('\uD535');
		charS.add('\uD536');
		charS.add('\uD537');
		charS.add('\uD538');
		charS.add('\uD539');
		charS.add('\uD53A');
		charS.add('\uD53B');
		charS.add('\uD53C');
		charS.add('\uD53D');
		charS.add('\uD53E');
		charS.add('\uD53F');
		charS.add('\uD540');
		charS.add('\uD541');
		charS.add('\uD542');
		charS.add('\uD543');
		charS.add('\uD544');
		charS.add('\uD545');
		charS.add('\uD546');
		charS.add('\uD547');
		charS.add('\uD548');
		charS.add('\uD549');
		charS.add('\uD54A');
		charS.add('\uD54B');
		charS.add('\uD54C');
		charS.add('\uD54D');
		charS.add('\uD54E');
		charS.add('\uD54F');
		charS.add('\uD550');
		charS.add('\uD551');
		charS.add('\uD552');
		charS.add('\uD553');
		charS.add('\uD554');
		charS.add('\uD555');
		charS.add('\uD556');
		charS.add('\uD557');
		charS.add('\uD558');
		charS.add('\uD559');
		charS.add('\uD55A');
		charS.add('\uD55B');
		charS.add('\uD55C');
		charS.add('\uD55D');
		charS.add('\uD55E');
		charS.add('\uD55F');
		charS.add('\uD560');
		charS.add('\uD561');
		charS.add('\uD562');
		charS.add('\uD563');
		charS.add('\uD564');
		charS.add('\uD565');
		charS.add('\uD566');
		charS.add('\uD567');
		charS.add('\uD568');
		charS.add('\uD569');
		charS.add('\uD56A');
		charS.add('\uD56B');
		charS.add('\uD56C');
		charS.add('\uD56D');
		charS.add('\uD56E');
		charS.add('\uD56F');
		charS.add('\uD570');
		charS.add('\uD571');
		charS.add('\uD572');
		charS.add('\uD573');
		charS.add('\uD574');
		charS.add('\uD575');
		charS.add('\uD576');
		charS.add('\uD577');
		charS.add('\uD578');
		charS.add('\uD579');
		charS.add('\uD57A');
		charS.add('\uD57B');
		charS.add('\uD57C');
		charS.add('\uD57D');
		charS.add('\uD57E');
		charS.add('\uD57F');
		charS.add('\uD580');
		charS.add('\uD581');
		charS.add('\uD582');
		charS.add('\uD583');
		charS.add('\uD584');
		charS.add('\uD585');
		charS.add('\uD586');
		charS.add('\uD587');
		charS.add('\uD588');
		charS.add('\uD589');
		charS.add('\uD58A');
		charS.add('\uD58B');
		charS.add('\uD58C');
		charS.add('\uD58D');
		charS.add('\uD58E');
		charS.add('\uD58F');
		charS.add('\uD590');
		charS.add('\uD591');
		charS.add('\uD592');
		charS.add('\uD593');
		charS.add('\uD594');
		charS.add('\uD595');
		charS.add('\uD596');
		charS.add('\uD597');
		charS.add('\uD598');
		charS.add('\uD599');
		charS.add('\uD59A');
		charS.add('\uD59B');
		charS.add('\uD59C');
		charS.add('\uD59D');
		charS.add('\uD59E');
		charS.add('\uD59F');
		charS.add('\uD5A0');
		charS.add('\uD5A1');
		charS.add('\uD5A2');
		charS.add('\uD5A3');
		charS.add('\uD5A4');
		charS.add('\uD5A5');
		charS.add('\uD5A6');
		charS.add('\uD5A7');
		charS.add('\uD5A8');
		charS.add('\uD5A9');
		charS.add('\uD5AA');
		charS.add('\uD5AB');
		charS.add('\uD5AC');
		charS.add('\uD5AD');
		charS.add('\uD5AE');
		charS.add('\uD5AF');
		charS.add('\uD5B0');
		charS.add('\uD5B1');
		charS.add('\uD5B2');
		charS.add('\uD5B3');
		charS.add('\uD5B4');
		charS.add('\uD5B5');
		charS.add('\uD5B6');
		charS.add('\uD5B7');
		charS.add('\uD5B8');
		charS.add('\uD5B9');
		charS.add('\uD5BA');
		charS.add('\uD5BB');
		charS.add('\uD5BC');
		charS.add('\uD53D');
		charS.add('\uD5BE');
		charS.add('\uD5BF');
		charS.add('\uD5C0');
		charS.add('\uD5C1');
		charS.add('\uD5C2');
		charS.add('\uD5C3');
		charS.add('\uD5C4');
		charS.add('\uD5C5');
		charS.add('\uD5C6');
		charS.add('\uD5C7');
		charS.add('\uD5C8');
		charS.add('\uD5C9');
		charS.add('\uD5CA');
		charS.add('\uD5CB');
		charS.add('\uD5CC');
		charS.add('\uD5CD');
		charS.add('\uD5CE');
		charS.add('\uD5CF');
		charS.add('\uD5D0');
		charS.add('\uD5D1');
		charS.add('\uD5D2');
		charS.add('\uD5D3');
		charS.add('\uD5D4');
		charS.add('\uD5D5');
		charS.add('\uD5D6');
		charS.add('\uD5D7');
		charS.add('\uD5D8');
		charS.add('\uD5D9');
		charS.add('\uD5DA');
		charS.add('\uD5DB');
		charS.add('\uD5DC');
		charS.add('\uD5DD');
		charS.add('\uD5DE');
		charS.add('\uD5DF');
		charS.add('\uD5E0');
		charS.add('\uD5E1');
		charS.add('\uD5E2');
		charS.add('\uD5E3');
		charS.add('\uD5E4');
		charS.add('\uD5E5');
		charS.add('\uD5E6');
		charS.add('\uD5E7');
		charS.add('\uD5E8');
		charS.add('\uD5E9');
		charS.add('\uD5EA');
		charS.add('\uD5EB');
		charS.add('\uD5EC');
		charS.add('\uD5ED');
		charS.add('\uD5EE');
		charS.add('\uD5EF');
		charS.add('\uD5F0');
		charS.add('\uD5F1');
		charS.add('\uD5F2');
		charS.add('\uD5F3');
		charS.add('\uD5F4');
		charS.add('\uD5F5');
		charS.add('\uD5F6');
		charS.add('\uD5F7');
		charS.add('\uD5F8');
		charS.add('\uD5F9');
		charS.add('\uD5FA');
		charS.add('\uD5FB');
		charS.add('\uD5FC');
		charS.add('\uD5FD');
		charS.add('\uD5FE');
		charS.add('\uD5FF');
		charS.add('\uD600');
		charS.add('\uD601');
		charS.add('\uD602');
		charS.add('\uD603');
		charS.add('\uD604');
		charS.add('\uD605');
		charS.add('\uD606');
		charS.add('\uD607');
		charS.add('\uD608');
		charS.add('\uD609');
		charS.add('\uD60A');
		charS.add('\uD60B');
		charS.add('\uD60C');
		charS.add('\uD60D');
		charS.add('\uD60E');
		charS.add('\uD60F');
		charS.add('\uD610');
		charS.add('\uD611');
		charS.add('\uD612');
		charS.add('\uD613');
		charS.add('\uD614');
		charS.add('\uD615');
		charS.add('\uD616');
		charS.add('\uD617');
		charS.add('\uD618');
		charS.add('\uD619');
		charS.add('\uD61A');
		charS.add('\uD61B');
		charS.add('\uD61C');
		charS.add('\uD61D');
		charS.add('\uD61E');
		charS.add('\uD61F');
		charS.add('\uD620');
		charS.add('\uD621');
		charS.add('\uD622');
		charS.add('\uD623');
		charS.add('\uD624');
		charS.add('\uD625');
		charS.add('\uD626');
		charS.add('\uD627');
		charS.add('\uD628');
		charS.add('\uD629');
		charS.add('\uD62A');
		charS.add('\uD62B');
		charS.add('\uD62C');
		charS.add('\uD62D');
		charS.add('\uD62E');
		charS.add('\uD62F');
		charS.add('\uD630');
		charS.add('\uD631');
		charS.add('\uD632');
		charS.add('\uD633');
		charS.add('\uD634');
		charS.add('\uD635');
		charS.add('\uD636');
		charS.add('\uD637');
		charS.add('\uD638');
		charS.add('\uD639');
		charS.add('\uD63A');
		charS.add('\uD63B');
		charS.add('\uD63C');
		charS.add('\uD63D');
		charS.add('\uD63E');
		charS.add('\uD63F');
		charS.add('\uD640');
		charS.add('\uD641');
		charS.add('\uD642');
		charS.add('\uD643');
		charS.add('\uD644');
		charS.add('\uD645');
		charS.add('\uD646');
		charS.add('\uD647');
		charS.add('\uD648');
		charS.add('\uD649');
		charS.add('\uD64A');
		charS.add('\uD64B');
		charS.add('\uD64C');
		charS.add('\uD64D');
		charS.add('\uD64E');
		charS.add('\uD64F');
		charS.add('\uD650');
		charS.add('\uD651');
		charS.add('\uD652');
		charS.add('\uD653');
		charS.add('\uD654');
		charS.add('\uD655');
		charS.add('\uD656');
		charS.add('\uD657');
		charS.add('\uD658');
		charS.add('\uD659');
		charS.add('\uD65A');
		charS.add('\uD65B');
		charS.add('\uD65C');
		charS.add('\uD65D');
		charS.add('\uD65E');
		charS.add('\uD65F');
		charS.add('\uD660');
		charS.add('\uD661');
		charS.add('\uD662');
		charS.add('\uD663');
		charS.add('\uD664');
		charS.add('\uD665');
		charS.add('\uD666');
		charS.add('\uD667');
		charS.add('\uD668');
		charS.add('\uD669');
		charS.add('\uD66A');
		charS.add('\uD66B');
		charS.add('\uD66C');
		charS.add('\uD66D');
		charS.add('\uD66E');
		charS.add('\uD66F');
		charS.add('\uD670');
		charS.add('\uD671');
		charS.add('\uD672');
		charS.add('\uD673');
		charS.add('\uD674');
		charS.add('\uD675');
		charS.add('\uD676');
		charS.add('\uD677');
		charS.add('\uD678');
		charS.add('\uD679');
		charS.add('\uD67A');
		charS.add('\uD67B');
		charS.add('\uD67C');
		charS.add('\uD67D');
		charS.add('\uD67E');
		charS.add('\uD67F');
		charS.add('\uD680');
		charS.add('\uD681');
		charS.add('\uD682');
		charS.add('\uD683');
		charS.add('\uD684');
		charS.add('\uD685');
		charS.add('\uD686');
		charS.add('\uD687');
		charS.add('\uD688');
		charS.add('\uD689');
		charS.add('\uD68A');
		charS.add('\uD68B');
		charS.add('\uD68C');
		charS.add('\uD68D');
		charS.add('\uD68E');
		charS.add('\uD68F');
		charS.add('\uD690');
		charS.add('\uD691');
		charS.add('\uD692');
		charS.add('\uD693');
		charS.add('\uD694');
		charS.add('\uD695');
		charS.add('\uD696');
		charS.add('\uD697');
		charS.add('\uD698');
		charS.add('\uD699');
		charS.add('\uD69A');
		charS.add('\uD69B');
		charS.add('\uD69C');
		charS.add('\uD69D');
		charS.add('\uD69E');
		charS.add('\uD69F');
		charS.add('\uD6A0');
		charS.add('\uD6A1');
		charS.add('\uD6A2');
		charS.add('\uD6A3');
		charS.add('\uD6A4');
		charS.add('\uD6A5');
		charS.add('\uD6A6');
		charS.add('\uD6A7');
		charS.add('\uD6A8');
		charS.add('\uD6A9');
		charS.add('\uD6AA');
		charS.add('\uD6AB');
		charS.add('\uD6AC');
		charS.add('\uD6AD');
		charS.add('\uD6AE');
		charS.add('\uD6AF');
		charS.add('\uD6B0');
		charS.add('\uD6B1');
		charS.add('\uD6B2');
		charS.add('\uD6B3');
		charS.add('\uD6B4');
		charS.add('\uD6B5');
		charS.add('\uD6B6');
		charS.add('\uD6B7');
		charS.add('\uD6B8');
		charS.add('\uD6B9');
		charS.add('\uD6BA');
		charS.add('\uD6BB');
		charS.add('\uD6BC');
		charS.add('\uD63D');
		charS.add('\uD6BE');
		charS.add('\uD6BF');
		charS.add('\uD6C0');
		charS.add('\uD6C1');
		charS.add('\uD6C2');
		charS.add('\uD6C3');
		charS.add('\uD6C4');
		charS.add('\uD6C5');
		charS.add('\uD6C6');
		charS.add('\uD6C7');
		charS.add('\uD6C8');
		charS.add('\uD6C9');
		charS.add('\uD6CA');
		charS.add('\uD6CB');
		charS.add('\uD6CC');
		charS.add('\uD6CD');
		charS.add('\uD6CE');
		charS.add('\uD6CF');
		charS.add('\uD6D0');
		charS.add('\uD6D1');
		charS.add('\uD6D2');
		charS.add('\uD6D3');
		charS.add('\uD6D4');
		charS.add('\uD6D5');
		charS.add('\uD6D6');
		charS.add('\uD6D7');
		charS.add('\uD6D8');
		charS.add('\uD6D9');
		charS.add('\uD6DA');
		charS.add('\uD6DB');
		charS.add('\uD6DC');
		charS.add('\uD6DD');
		charS.add('\uD6DE');
		charS.add('\uD6DF');
		charS.add('\uD6E0');
		charS.add('\uD6E1');
		charS.add('\uD6E2');
		charS.add('\uD6E3');
		charS.add('\uD6E4');
		charS.add('\uD6E5');
		charS.add('\uD6E6');
		charS.add('\uD6E7');
		charS.add('\uD6E8');
		charS.add('\uD6E9');
		charS.add('\uD6EA');
		charS.add('\uD6EB');
		charS.add('\uD6EC');
		charS.add('\uD6ED');
		charS.add('\uD6EE');
		charS.add('\uD6EF');
		charS.add('\uD6F0');
		charS.add('\uD6F1');
		charS.add('\uD6F2');
		charS.add('\uD6F3');
		charS.add('\uD6F4');
		charS.add('\uD6F5');
		charS.add('\uD6F6');
		charS.add('\uD6F7');
		charS.add('\uD6F8');
		charS.add('\uD6F9');
		charS.add('\uD6FA');
		charS.add('\uD6FB');
		charS.add('\uD6FC');
		charS.add('\uD6FD');
		charS.add('\uD6FE');
		charS.add('\uD6FF');
		charS.add('\uD700');
		charS.add('\uD701');
		charS.add('\uD702');
		charS.add('\uD703');
		charS.add('\uD704');
		charS.add('\uD705');
		charS.add('\uD706');
		charS.add('\uD707');
		charS.add('\uD708');
		charS.add('\uD709');
		charS.add('\uD70A');
		charS.add('\uD70B');
		charS.add('\uD70C');
		charS.add('\uD70D');
		charS.add('\uD70E');
		charS.add('\uD70F');
		charS.add('\uD710');
		charS.add('\uD711');
		charS.add('\uD712');
		charS.add('\uD713');
		charS.add('\uD714');
		charS.add('\uD715');
		charS.add('\uD716');
		charS.add('\uD717');
		charS.add('\uD718');
		charS.add('\uD719');
		charS.add('\uD71A');
		charS.add('\uD71B');
		charS.add('\uD71C');
		charS.add('\uD71D');
		charS.add('\uD71E');
		charS.add('\uD71F');
		charS.add('\uD720');
		charS.add('\uD721');
		charS.add('\uD722');
		charS.add('\uD723');
		charS.add('\uD724');
		charS.add('\uD725');
		charS.add('\uD726');
		charS.add('\uD727');
		charS.add('\uD728');
		charS.add('\uD729');
		charS.add('\uD72A');
		charS.add('\uD72B');
		charS.add('\uD72C');
		charS.add('\uD72D');
		charS.add('\uD72E');
		charS.add('\uD72F');
		charS.add('\uD730');
		charS.add('\uD731');
		charS.add('\uD732');
		charS.add('\uD733');
		charS.add('\uD734');
		charS.add('\uD735');
		charS.add('\uD736');
		charS.add('\uD737');
		charS.add('\uD738');
		charS.add('\uD739');
		charS.add('\uD73A');
		charS.add('\uD73B');
		charS.add('\uD73C');
		charS.add('\uD73D');
		charS.add('\uD73E');
		charS.add('\uD73F');
		charS.add('\uD740');
		charS.add('\uD741');
		charS.add('\uD742');
		charS.add('\uD743');
		charS.add('\uD744');
		charS.add('\uD745');
		charS.add('\uD746');
		charS.add('\uD747');
		charS.add('\uD748');
		charS.add('\uD749');
		charS.add('\uD74A');
		charS.add('\uD74B');
		charS.add('\uD74C');
		charS.add('\uD74D');
		charS.add('\uD74E');
		charS.add('\uD74F');
		charS.add('\uD750');
		charS.add('\uD751');
		charS.add('\uD752');
		charS.add('\uD753');
		charS.add('\uD754');
		charS.add('\uD755');
		charS.add('\uD756');
		charS.add('\uD757');
		charS.add('\uD758');
		charS.add('\uD759');
		charS.add('\uD75A');
		charS.add('\uD75B');
		charS.add('\uD75C');
		charS.add('\uD75D');
		charS.add('\uD75E');
		charS.add('\uD75F');
		charS.add('\uD760');
		charS.add('\uD761');
		charS.add('\uD762');
		charS.add('\uD763');
		charS.add('\uD764');
		charS.add('\uD765');
		charS.add('\uD766');
		charS.add('\uD767');
		charS.add('\uD768');
		charS.add('\uD769');
		charS.add('\uD76A');
		charS.add('\uD76B');
		charS.add('\uD76C');
		charS.add('\uD76D');
		charS.add('\uD76E');
		charS.add('\uD76F');
		charS.add('\uD770');
		charS.add('\uD771');
		charS.add('\uD772');
		charS.add('\uD773');
		charS.add('\uD774');
		charS.add('\uD775');
		charS.add('\uD776');
		charS.add('\uD777');
		charS.add('\uD778');
		charS.add('\uD779');
		charS.add('\uD77A');
		charS.add('\uD77B');
		charS.add('\uD77C');
		charS.add('\uD77D');
		charS.add('\uD77E');
		charS.add('\uD77F');
		charS.add('\uD780');
		charS.add('\uD781');
		charS.add('\uD782');
		charS.add('\uD783');
		charS.add('\uD784');
		charS.add('\uD785');
		charS.add('\uD786');
		charS.add('\uD787');
		charS.add('\uD788');
		charS.add('\uD789');
		charS.add('\uD78A');
		charS.add('\uD78B');
		charS.add('\uD78C');
		charS.add('\uD78D');
		charS.add('\uD78E');
		charS.add('\uD78F');
		charS.add('\uD790');
		charS.add('\uD791');
		charS.add('\uD792');
		charS.add('\uD793');
		charS.add('\uD794');
		charS.add('\uD795');
		charS.add('\uD796');
		charS.add('\uD797');
		charS.add('\uD798');
		charS.add('\uD799');
		charS.add('\uD79A');
		charS.add('\uD79B');
		charS.add('\uD79C');
		charS.add('\uD79D');
		charS.add('\uD79E');
		charS.add('\uD79F');
		charS.add('\uD7A0');
		charS.add('\uD7A1');
		charS.add('\uD7A2');
		charS.add('\uD7A3');
		
		return charS;
	}
}