...
public class InteractionEvents
{
	...
	public void interactWithUpgradable(PlayerInteractEvent.RightClickBlock e)
	{
		...
		if(!held.isEmpty())
		{
			...
			if(item != null && tile != null)
			{
				...
				{
					held.shrink(1);
					HammerCore.audioProxy.playSoundAt(world, Info.MOD_ID + ":upgrade", pos, 1F, .8F + world.rand.nextFloat() * .4F, SoundCategory.PLAYERS);
			...
			} else if(held.getItem() == ItemsLT.WAND_REVERSAL)
			{
				if(!world.isRemote && tile != null && tile.dropUpgrade(e.getEntityPlayer()))
				{
					...
					HammerCore.audioProxy.playSoundAt(world, Info.MOD_ID + ":zap", pos, 1F, .8F + world.rand.nextFloat() * .4F, SoundCategory.PLAYERS);
	...
	public void playerTick(PlayerTickEvent e)
	{
		...
		{
			ResourceLocation loc = new ResourceLocation(Info.MOD_ID, "aura/atmosphere_problems");
