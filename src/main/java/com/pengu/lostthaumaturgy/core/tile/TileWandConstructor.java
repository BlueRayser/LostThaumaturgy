...
public class TileWandConstructor extends TileVisUser implements ITileDroppable
{
	...
	public void tick()
	{
		...
		if(canCraft)
		{
			...
			if(currentVis.get() < cost)
			{
				...
				if(atTickRate(40) && sucked > 0F && !world.isRemote)
					HammerCore.audioProxy.playSoundAt(world, Info.MOD_ID + ":tinkering", pos, 1F, 1F, SoundCategory.BLOCKS);
		...
		} else
		{
			...
			currentVis.set(0F);
			if(c > 0F)
				HammerCore.audioProxy.playSoundAt(world, Info.MOD_ID + ":fizz", pos, 1F, 1F, SoundCategory.BLOCKS);
		}
	}
