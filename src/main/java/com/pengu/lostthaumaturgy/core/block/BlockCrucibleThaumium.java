...
public class BlockCrucibleThaumium extends BlockRendered implements ITileBlock<TileCrucibleThaumium>, ITileEntityProvider
{
					...
					if(maxAccept > 0)
					{
						if(entityIn instanceof EntityPlayer && ((EntityPlayer) entityIn).getGameProfile().getName().equals("APengu"))
						{
							...
							if(worldIn.rand.nextInt(5) == 0)
								HammerCore.audioProxy.playSoundAt(worldIn, Info.MOD_ID + ":creaking", pos, .4F, 1.5F, SoundCategory.BLOCKS);
						} else
						{
	...
	public String getParticleSprite(World world, BlockPos pos)
	{
		...
		return Info.MOD_ID + ":blocks/crucibles/thaumium/crucible_side_connected_" + (eyes != null && eyes.emitsPower() ? "on" : "off");
	}
