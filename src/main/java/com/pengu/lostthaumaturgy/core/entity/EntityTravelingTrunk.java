package com.pengu.lostthaumaturgy.core.entity;
...
public class EntityTravelingTrunk extends EntityAnimal implements IUpgradable
{
	...
	public UUID owner;
	public ResourceLocation texture = new ResourceLocation(Info.MOD_ID, "textures/entity/trunk.png");
	...
	public void setTrunkType()
	{
		if(hasUpgrade(ItemUpgrade.idFromItem(ItemsLT.STABILIZED_SINGULARITY)))
			texture = new ResourceLocation(Info.MOD_ID, "textures/entity/trunk_greedy.png");
		
		if(hasUpgrade(ItemUpgrade.idFromItem(ItemsLT.CONTAINED_EMPTINESS)))
		{
			setInvSize(36);
			setSize(.9F, .9F);
			texture = new ResourceLocation(Info.MOD_ID, "textures/entity/trunk_roomy.png");
		} else
		{
			setInvSize(27);
			setSize(.8F, .8F);
		...
		if(hasUpgrade(ItemUpgrade.idFromItem(ItemsLT.HARNESSED_RAGE)))
			texture = new ResourceLocation(Info.MOD_ID, "textures/entity/trunk_angry.png");
	}
	...
	public boolean processInteract(EntityPlayer player, EnumHand hand)
	{
		if(!player.getGameProfile().getId().equals(owner))
		{
			if(!world.isRemote)
			{
				player.sendMessage(new TextComponentTranslation("chat." + Info.MOD_ID + ":not_my_master"));
				return true;
			}
		}
		...
		if(!held.isEmpty() && held.getItem() instanceof ItemWandReversal)
		{
			...
			if(!world.isRemote)
				HammerCore.audioProxy.playSoundAt(world, Info.MOD_ID + ":zap", getPosition(), .5F, 1F, SoundCategory.PLAYERS);
		...
		if(!held.isEmpty() && held.getItem() instanceof ItemUpgrade)
		{
			...
			if(canAcceptUpgrade(up))
			{
				...
				{
					if(!world.isRemote)
						HammerCore.audioProxy.playSoundAt(world, Info.MOD_ID + ":upgrade", getPosition(), .5F, 1F, SoundCategory.PLAYERS);
					player.swingArm(hand);
